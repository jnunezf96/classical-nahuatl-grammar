...
const version = "v20260810-exact-behavior-checkpoint-010";
...
const testFiles = ["classical_checkpoint010_grammar_fact_projection.test.js"];
...
assert(captured.size === 500, `checkpoint exact project-role observation yield drifted: ${captured.size}`);
...
assert(owners.length === 30, `checkpoint owner count drifted: ${owners.length}`);
...
const manifest = {
  schemaVersion: 1, version, status: "validated", baseProgressCommit: "8243d7793f2ac9401fa45104796cea18adb49694",
...
assert(progress.lessonCorpus.exactProofs.exactBehaviorObserved === 6756, "progress ledger delta is wrong");
const report = { schemaVersion: 1, version, status: "passed",
  ...
  progress: { denominator: 18639, before: 6256, after: 6756, remaining: 11883 },
