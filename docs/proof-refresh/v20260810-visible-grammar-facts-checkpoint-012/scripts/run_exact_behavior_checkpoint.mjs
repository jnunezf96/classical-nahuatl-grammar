...
const version = "v20260810-visible-grammar-facts-checkpoint-012";
...
assert(captured.size === 500, `checkpoint exact visible observation yield drifted: ${captured.size}`);
const observations = [...captured.values()].map(item => {
  ...
  assert(tuple[atomIndex.uiRole] === "read-only-grammar-fact", `${item.atomId} is not a grammar-fact presentation obligation`);
  ...
  return {
    ...
    obligation: "normal-user-visible-grammar-fact-presentation",
    assertionId: `${ownerId}:${item.atomId.toLowerCase()}:visible`,
    canonicalPath: item.canonicalPath,
    expectedDigest: digest(stableJson(item.expected)),
    actualDigest: digest(stableJson(item.actual)),
...
assert(owners.length === 18, `checkpoint owner count drifted: ${owners.length}`);
...
for (const ownerId of owners) {
  ...
  const receipt = {
    ...
    version,
    ownerId,
    projectRole: "normal-user-visible-grammar-fact-presentation",
    canonicalResultDigest: digest(stableJson(ownerObservations.map(item => ({ canonicalPath: item.canonicalPath, actualDigest: item.actualDigest })))),
...
const manifest = {
  ...
  preparedRuntimeFacts: {
    ...
    atomCount: 500,
    selectionManifests: ["docs/canvas-progress/checkpoint010_fact_selection.json"],
...
const report = {
  ...
  progress: { denominator: 18639, before: 5756, after: 6256, remaining: 12383 },
  validations: {
