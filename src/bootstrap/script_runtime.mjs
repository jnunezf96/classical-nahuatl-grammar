...
export function createScriptRuntimeApi(targetObject = globalThis) {
    ...
    function getScriptStaticRuntimePaths() {
      const config = typeof globalThis !== "undefined" && globalThis.__CLASSICAL_RUNTIME_CONFIG__ && globalThis.__CLASSICAL_RUNTIME_CONFIG__.paths && typeof globalThis.__CLASSICAL_RUNTIME_CONFIG__.paths === "object" ? globalThis.__CLASSICAL_RUNTIME_CONFIG__.paths : null;
      if (config) {
        return config;
      }
    ...
    function applyStaticOptions(data) {
      ...
      if (data.possessorLabels && typeof data.possessorLabels === "object") {
        ...
        if (data.possessorLabelsNa && typeof data.possessorLabelsNa === "object") {
          Object.entries(data.possessorLabelsNa).forEach(([key, labelEs]) => {
            fallback[key] = {
              ...(fallback[key] || {}),
              labelEs
            };
      ...
      OBJECT_PREFIX_LABELS = new Map(OBJECT_PREFIXES.map(option => {
        ...
        const baseLabelEs = baseLabel ? baseLabel.labelEs || "" : "";
      ...
      targetObject.POSSESSIVE_PREFIX_LABELS = new Map(targetObject.POSSESSIVE_PREFIXES.map(option => {
        ...
        const labelEs = option.labelEs || option.label || personLabel?.labelEs || option.value;
        return [option.value, {
          labelEs
        }];
      }));
    ...
    function applyStaticModes(data) {
      ...
      const normalizeModeMap = map => {
        ...
        Object.entries(map).forEach(([key, value]) => {
          if (value && typeof value === "object") {
            normalized[key] = value.value || value.labelEs || key;
          } else {
      ...
      if (!UnitTenseModeState.mode && targetObject.TENSE_MODE.verbo) {
        UnitTenseModeState.mode = targetObject.TENSE_MODE.verbo;
      }
    ...
    var UI_DENSITY_STORAGE_KEY = "classical_nahuatl_ui_density_mode";
    ...
    var TOGGLE_LOCK_STORAGE_KEY = "classical_nahuatl_toggle_lock_mode";
    var ToggleLockState = {
      enabled: false
    };
    ...
    Object.defineProperty(api, "ClassicalRouteState", {
        configurable: true,
        enumerable: true,
        get() { return ClassicalRouteState; },
        set(value) { ClassicalRouteState = value; },
    });
    ...
    Object.defineProperty(api, "UnitTenseModeState", {
        configurable: true,
        enumerable: true,
        get() { return UnitTenseModeState; },
        set(value) { UnitTenseModeState = value; },
    });
