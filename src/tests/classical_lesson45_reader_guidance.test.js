...
function run(ctx = {}) {
    const s = createSuite("classical_lesson45_reader_guidance");
    const ledger = JSON.parse(fs.readFileSync(path.join(
        ROOT, "docs/canvas-progress/lesson45-review-ledger.json"), "utf8"));
    ...
    const ideas = ctx.LESSON45_READER_GUIDANCE_GROUPS;
    const panel = ctx.ClassicalAuthorityPanel();
    const start = panel.indexOf(
        'data-classical-reader-guidance-lesson="45"'
    );
    const end = panel.indexOf("</details>", start) + "</details>".length;
    const html = panel.slice(start, end);

    s.eq("Lesson 45 guidance covers the exact technical-proof groups", {
    ...
    s.ok(
        "Lesson 45 guidance remains descriptive rather than grammatical authority",
        ...
        && ctx.isLesson45ReaderGuidanceExact(ideas),
    );
