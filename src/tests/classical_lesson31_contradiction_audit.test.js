...
function run() {
    const s = createSuite("classical_lesson31_contradiction_audit");
    const read = name => JSON.parse(fs.readFileSync(path.join(
        ROOT, "docs/canvas-progress", name), "utf8"));
    const audit = read("lesson31-contradiction-audit.json");
    const ledger = read("lesson31-review-ledger.json");
    const proof = read("lesson31-implementation-proof.json");
    ...
    s.eq("all accepted Lesson 31 groups are uncontradicted", {
        scope: audit.scope,
        status: audit.status,
    ...
    s.eq("every accepted Lesson 31 atom has exact routes", {
