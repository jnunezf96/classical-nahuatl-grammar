"use strict";

const { createSuite } = require("./runner");

function buildSource(ctx, stem, subject = "1sg", valence = "mainline-reflexive") {
    return ctx.buildClassicalNahuatlVerbstemClassFrame(stem, {
        subject,
        mood: "indicative",
        tense: "present",
        verbClass: "A",
        perfectiveClass: "A",
        valence,
        transitivity: valence === "intransitive" ? "intransitive" : "transitive",
        objectKind: valence === "mainline-reflexive" ? "reflexive" : "none",
        objectPerson: valence === "mainline-reflexive" ? subject : "",
    });
}

function selectCausative(ctx, source, targetStem, options = {}) {
    const inventory = ctx.getClassicalNahuatlVncDerivationOptionInventory(source, {
        derivationType: "causative",
    });
    const selected = inventory.options.find(option => option.targetStem === targetStem);
    return ctx.deriveClassicalNahuatlVncDerivationOperationFrame(source, {
        derivationType: "causative",
        optionId: selected?.optionId || "",
        ...options,
    });
}

function run(ctx = {}) {
    const s = createSuite("classical_lesson25_reflexive_causative");

    s.eq(
        "Lesson 25.11/25.13 moves a retained source reflexive to shuntline ne",
        (() => {
            const source = buildSource(ctx, "petlāhua");
            const operation = selectCausative(ctx, source, "petlāhua-l-tiā", {
                targetSubject: "2sg",
            });
            const machinery = ctx.buildClassicalNahuatlDerivedVncMachineryFrame(source, operation, {
                mood: "indicative",
                tense: "present",
                targetSubject: "2sg",
            });
            return {
                operationStatus: operation.authorizationStatus,
                operationCanonical: ctx.isClassicalNahuatlVncDerivationOperationFrame(operation),
                targetObjects: operation.targetObjectRequests.map(request => [
                    request.objectKind,
                    request.objectPerson,
                    request.governor,
                    request.derivationalLevel,
                ]),
                rule: operation.participantTransformFrame.retainedSourceReflexiveShuntlineRuleFrame?.ruleId,
                machineryStatus: machinery.authorizationStatus,
                machineryCanonical: ctx.isClassicalNahuatlDerivedVncMachineryFrame(machinery),
                formula: machinery.formulaRealization,
            };
        })(),
        {
            operationStatus: "authorized",
            operationCanonical: true,
            targetObjects: [
                ["reflexive", "nonfirst-common", "directive", 1],
                ["specific-projective", "1sg", "causative", 2],
            ],
            rule: "cn-vnc-retained-source-mainline-reflexive-to-shuntline-ne",
            machineryStatus: "authorized",
            machineryCanonical: true,
            formula: "#ti-0+n-ēch+ne(petlāhua-l-tia)0+0-0#",
        }
    );

    s.eq(
        "Causee Valence carries fixed versus reflexive participant identity without a referent-relation authority",
        (() => {
            const equalSource = buildSource(ctx, "chōca", "3pl", "intransitive");
            const required = selectCausative(ctx, equalSource, "chōc-tiā", { targetSubject: "3pl" });
            const legacyRelation = selectCausative(ctx, equalSource, "chōc-tiā", {
                targetSubject: "3pl",
                causativeReferentRelation: "coreferential",
            });
            const fixed = selectCausative(ctx, equalSource, "chōc-tiā", {
                targetSubject: "3pl",
                causativeObjectKind: "specific-projective",
            });
            const reflexive = selectCausative(ctx, equalSource, "chōc-tiā", {
                targetSubject: "3pl",
                causativeObjectKind: "mainline-reflexive",
            });
            const ineligibleSource = buildSource(ctx, "chōca", "3sg", "intransitive");
            const automatic = selectCausative(ctx, ineligibleSource, "chōc-tiā", {
                targetSubject: "2sg",
            });
            return {
                required: [
                    required.authorizationStatus,
                    required.blockReason,
                    required.participantTransformFrame.causativeObjectKindChoiceEligible,
                    required.participantTransformFrame.allowedCausativeObjectKinds,
                ],
                legacyRelation: [
                    legacyRelation.authorizationStatus,
                    legacyRelation.blockReason,
                    legacyRelation.canonicalSignature === required.canonicalSignature,
                ],
                fixed: [
                    fixed.authorizationStatus,
                    fixed.participantTransformFrame.causativeObjectKind,
                    fixed.participantTransformFrame.addedObjectRequest.objectPerson,
                    fixed.participantTransformFrame.causativeObjectPersonBinding,
                ],
                reflexive: [
                    reflexive.authorizationStatus,
                    reflexive.participantTransformFrame.causativeObjectKind,
                    reflexive.participantTransformFrame.addedObjectRequest.objectPerson,
                    reflexive.participantTransformFrame.causativeObjectPersonBinding,
                ],
                automatic: [
                    automatic.authorizationStatus,
                    automatic.participantTransformFrame.causativeObjectKind,
                    automatic.participantTransformFrame.causativeObjectPersonBinding,
                ],
            };
        })(),
        {
            required: [
                "blocked",
                "classical-vnc-causative-causee-valence-selection-required",
                true,
                ["specific-projective", "reflexive"],
            ],
            legacyRelation: [
                "blocked",
                "classical-vnc-causative-causee-valence-selection-required",
                true,
            ],
            fixed: ["authorized", "specific-projective", "3pl", "fixed-source-subject"],
            reflexive: ["authorized", "reflexive", "3pl", "target-subject-coordinate"],
            automatic: ["authorized", "specific-projective", "fixed-source-subject"],
        }
    );

    return s;
}

module.exports = { run };
