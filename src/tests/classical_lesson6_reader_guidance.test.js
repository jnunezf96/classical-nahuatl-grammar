...
function run(ctx = {}) {
    const s = createSuite("classical_lesson6_reader_guidance");
    const ledger = JSON.parse(fs.readFileSync(path.resolve(
        __dirname, "../../docs/canvas-progress/lesson6-review-ledger.json"
    ), "utf8"));
    const accepted = ledger.records.filter((record) => record.reviewStatus === "ACCEPTED");
    const ideas = ctx.LESSON6_READER_GUIDANCE_GROUPS;
    ...
    for (const record of accepted) {
        ...
        s.no(`${record.atomId} fails if its Lesson 6 reading job is removed`,
            ctx.isLesson6ReaderGuidanceExact(
                ideas.filter((candidate) => candidate.ideaId !== record.reviewGroupId)
            ));
    }
    s.ok("Lesson 6 guidance stays collapsed and does not expose atom bookkeeping",
        visibleGuide.includes('data-classical-reader-guidance-lesson="6"')
        && !/Lesson 6[^]*atom/iu.test(visibleGuide));
