...
function run() {
    ...
    suite.ok(
        "the manufacturer witness is opt-in, local, observational, and records actions plus explicit feelings",
        shell.includes("installClassicalPlayWitness(targetObject, root)")
        && witness.includes('get("manufacturer") === "1"')
        && witness.includes('root.addEventListener("click", observeClick, true)')
        && witness.includes('root.addEventListener("change", observeAction, true)')
        && witness.includes('root.addEventListener("classical:play-feeling", observeFeeling, true)')
        ...
        && witness.includes("projection.textContent = JSON.stringify(snapshot())")
        && witness.includes('observableOutcome:')
