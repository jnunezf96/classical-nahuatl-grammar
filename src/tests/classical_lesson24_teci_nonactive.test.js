...
function run(ctx = {}) {
    ...
    s.eq(
        ...
        [
            typeof ctx.buildClassicalNahuatlNonactiveSurfaceFrame,
            typeof ctx.isClassicalNahuatlNonactiveSurfaceFrame,
        ],
        ["function", "function"]
    );

    const intransitiveInventory = ctx.getClassicalNahuatlNonactiveStemOptions("teci", {
        verbClass: "B",
        sourceValence: "intransitive",
    });
    const transitiveInventory = ctx.getClassicalNahuatlNonactiveStemOptions("teci", {
        verbClass: "B",
    ...
    const projections = projectionFixtures.map(([sourceValence, optionId]) => (
        ctx.buildClassicalNahuatlNonactiveSurfaceFrame("teci", {
            verbClass: "B",
            sourceValence,
            optionId,
        })
    ...
    s.eq(
        ...
        projections.map(frame => ({
            ...
            canonical: ctx.isClassicalNahuatlNonactiveSurfaceFrame(frame, "teci"),
    ...
    s.eq(
        ...
        [forgedWord, forgedLowerRecord, forgedVersion, forgedPolicy, forgedGate]
            .map(frame => ctx.isClassicalNahuatlNonactiveSurfaceFrame(frame, "teci")),
    ...
    s.eq(
        ...
        [
            ctx.buildClassicalNahuatlNonactiveSurfaceFrame("invented", {
                verbClass: "B",
                sourceValence: "intransitive",
            }),
            ctx.buildClassicalNahuatlNonactiveSurfaceFrame("teci", {
    ...
    s.eq(
        ...
        ctx.doesClassicalNahuatlLesson20FinalShapeMatchSuffixFamily(
            ctx.buildClassicalNahuatlStemFinalShapeFrame("tex-o"),
            "ō"
        ),
        false
    );
