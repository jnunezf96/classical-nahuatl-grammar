...
function run(ctx = {}) {
    ...
    if (typeof ctx.refreshInlineComposerControlsFromState === "function") {
        ...
        try {
            ...
            ctx.VerbComposerState.slotCEmbed = "tlaqual";
            ctx.VerbComposerState.slotCStem = "maca";
            ctx.VerbComposerState.valenceEmbedSecondary = "nexti";
            ctx.VerbComposerState.directionalPrefix = "hual";
            ...
            s.eq("programmatic bitransitive state projects incorporated element", controls.embedC.value, "tlaqual");
            s.eq("programmatic bitransitive state projects matrix stem", controls.stemC.value, "maca");
            s.eq("programmatic bitransitive state projects object embed", controls.objectC.value, "nexti");
            s.eq("programmatic bitransitive state projects directional", controls.directional.value, "hual");
        } finally {
            controls.restore();
        }
    } else {
