...
function run(ctx = {}) {
    ...
    const buildPronominalNncCooperationFrame = (subject = "3sg") => (
        ctx.buildClassicalNahuatlPronominalNncFrame({ subtype: "personal-simple", subject })
    ...
    s.eq(
        ...
        (() => {
            const rules = ctx.getClassicalNahuatlIrregularVncRules();
            return {
    ...
    s.eq(
        ...
        (() => {
            const unknown = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("pāca", { tense: "preterit" });
            const ceya = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ce-ya", { tense: "preterit" });
            return {
    ...
    s.eq(
        ...
        (() => {
            const plan = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ye", {
                subject: "3sg",
                mood: "indicative",
                tense: "present",
            });
            const go = ctx.getClassicalNahuatlParadigmRelationFrame("yā");
            const come = ctx.getClassicalNahuatlParadigmRelationFrame("huāl-lā");
            const conditioned = ctx.getClassicalNahuatlParadigmRelationFrame("mati");
            const defective = ctx.getClassicalNahuatlParadigmRelationFrame("ā");
            const zeroRoot = ctx.getClassicalNahuatlParadigmRelationFrame("i-ā");
            const unknown = ctx.getClassicalNahuatlParadigmRelationFrame("pāca");
            return {
    ...
    s.eq(
        ...
        (() => {
            const compound = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ahco-cui", { tense: "preterit" });
            const compoundPresent = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ahco-cui", { tense: "present" });
            const singular = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("mati", { subject: "1sg", mood: "indicative", tense: "preterit" });
            const plural = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("mati", { subject: "1pl", mood: "indicative", tense: "preterit" });
            const distant = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("mati", { subject: "1sg", mood: "indicative", tense: "distant-past" });
            const nearby = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("pati", { subject: "1sg", mood: "indicative", tense: "preterit" });
            return {
                ...
                compoundPresentClass: compoundPresent.selectedClassOverride,
                compoundPresentSelected: compoundPresent.selectedStemOverride,
                compoundPresentAlternatives: compoundPresent.alternatives,
        ...
        })(),
        {
            ...
            compoundPresentClass: "B",
            compoundPresentSelected: "",
            compoundPresentAlternatives: [],
    ...
    s.eq(
        "mani has one Class B identity before irregular application and rejects a caller-forced Class A",
        (() => {
            const inferred = ctx.inferClassicalNahuatlLesson7ClassProfile("mani", {
                valence: "intransitive",
            });
            const forcedA = ctx.inferClassicalNahuatlLesson7ClassProfile("mani", {
                valence: "intransitive",
                verbClass: "A",
            });
            return {
                inferredClass: inferred.classId,
                ruleId: inferred.classGuidelineRuleId,
                perfective: inferred.classGuidelineAuthorityRecord?.perfectiveStemsByClass?.B,
                lexical: inferred.classDeterminedByLexicalException,
                forcedClass: forcedA.classId,
                forcedAllowed: forcedA.classGuidelineSelectedClassAllowed,
                forcedBlocked: forcedA.classGuidelineContradictionBlocked,
            };
        })(),
        {
            inferredClass: "B",
    ...
    s.eq(
        ...
        (() => {
            const ihca = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ih-ca", { tense: "present" });
            const ihcaPast = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ih-ca", { semanticTense: "general-past" });
            const aFuture = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ā", { tense: "future" });
            const itzPresent = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("hui-tz", { tense: "present" });
            const itzFuture = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("hui-tz", { tense: "future" });
            const amiaBlocked = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("am-i-ā", { tense: "present" });
            const amiaAllowed = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("am-i-ā", { tense: "present", construction: "quēn" });
            const zeroBlocked = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("i-ā", { tense: "present" });
            const zeroAllowed = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("i-ā", {
                tense: "present",
                pronominalNncCooperationFrame: buildPronominalNncCooperationFrame("3sg"),
            });
            const mani = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("mani", { tense: "preterit" });
            const nemi = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("nemi", { semanticTense: "general-past" });
            const hostileInternalZero = ctx.buildClassicalNahuatlZeroRootOperationFrame("am-Ø-i-h");
            const hostileNoZero = ctx.buildClassicalNahuatlZeroRootOperationFrame("am-i-h");
            return {
    ...
    s.eq(
        "Lesson 11 irregular tense identities retain paradigm, meaning, and morphology as separate typed facts",
        (() => {
            const preteritAsPresent = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ih-ca", {
                mood: "indicative",
                tense: "preterit-as-present",
            });
            const distantPastAsPast = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ih-ca", {
                mood: "indicative",
                tense: "distant-past-as-past",
            });
            const legacyPresent = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ih-ca", {
                mood: "indicative",
                tense: "present",
            });
            ...
            const project = (plan) => [
                plan.paradigmTense,
                plan.semanticTenseValue,
                plan.morphologicalTense,
                plan.authorizationStatus,
            ];
            return {
                preteritAsPresent: project(preteritAsPresent),
                distantPastAsPast: project(distantPastAsPast),
                legacyPresentCanonicalizes: project(legacyPresent),
                hostileRegular: [hostileRegular.authorizationStatus, hostileRegular.blockReason],
            };
        })(),
        {
            preteritAsPresent: ["preterit-as-present", "present", "preterit", "authorized"],
            distantPastAsPast: ["distant-past-as-past", "general-past", "distant-past", "authorized"],
            legacyPresentCanonicalizes: ["preterit-as-present", "present", "preterit", "authorized"],
    ...
    s.eq(
        ...
        (() => {
            ...
            ].map(([stem, options]) => ctx.buildClassicalNahuatlIrregularVncParadigmPlan(stem, options));
    ...
    s.eq(
        ...
        (() => {
            const bePlural = ctx.buildClassicalNahuatlVerbstemClassFrame("(ye)", {
                valence: "intransitive", subject: "1pl", mood: "indicative", tense: "present",
            });
            const beSingular = ctx.buildClassicalNahuatlVerbstemClassFrame("(ye)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
            });
            const forbiddenPast = ctx.buildClassicalNahuatlVerbstemClassFrame("(ye)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "preterit",
            });
            const antecessivePresent = ctx.buildClassicalNahuatlVerbstemClassFrame("(ih-ca)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present", antecessive: true,
            });
            const ambiguousItz = ctx.buildClassicalNahuatlVerbstemClassFrame("(itz)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
            });
            const alertItz = ctx.buildClassicalNahuatlVerbstemClassFrame("(itz)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "preterit", lexicalReading: "alert-observant",
            });
            const motionItz = ctx.buildClassicalNahuatlVerbstemClassFrame("(itz)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present", lexicalReading: "motion",
            });
            return {
    ...
    s.eq(
        ...
        (() => {
            const quen = ctx.buildClassicalNahuatlVerbstemClassFrame("(am-i-ā)", {
                valence: "intransitive",
                subject: "3sg",
                mood: "indicative",
                tense: "present",
            ...
            const quenMach = ctx.buildClassicalNahuatlVerbstemClassFrame("(am-i-ā)", {
                valence: "intransitive",
                subject: "1sg",
                mood: "indicative",
                tense: "present",
            ...
            const incorporatedQuen = ctx.buildClassicalNahuatlVerbstemClassFrame("(am-i-ā)", {
                valence: "intransitive",
                subject: "2sg",
                mood: "indicative",
                tense: "present",
            ...
            const zeroBlocked = ctx.buildClassicalNahuatlVerbstemClassFrame("(i-ā)", {
                valence: "intransitive", subject: "3pl", mood: "indicative", tense: "present",
            });
            const zeroAuthorized = ctx.buildClassicalNahuatlVerbstemClassFrame("(i-ā)", {
                valence: "intransitive", subject: "3pl", mood: "indicative", tense: "present",
                pronominalNncCooperationFrame: buildPronominalNncCooperationFrame("3pl"),
            });
            const zeroAuthorizedFirstPerson = ctx.buildClassicalNahuatlVerbstemClassFrame("(i-ā)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
                pronominalNncCooperationFrame: buildPronominalNncCooperationFrame("1sg"),
            });
            const explicitZeroSourceAuthorized = ctx.buildClassicalNahuatlVerbstemClassFrame("(0-i-ā)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
                pronominalNncCooperationFrame: buildPronominalNncCooperationFrame("1sg"),
            });
            const arbitraryZeroSourceBlocked = ctx.buildClassicalNahuatlVerbstemClassFrame("(0-mati)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
            });
            const booleanOnlyBlocked = ctx.buildClassicalNahuatlVerbstemClassFrame("(i-ā)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
                pronominalNncCooperation: true,
            });
            const subjectMismatchBlocked = ctx.buildClassicalNahuatlVerbstemClassFrame("(i-ā)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
                pronominalNncCooperationFrame: buildPronominalNncCooperationFrame("3sg"),
            });
            const zeroLesson11Step = zeroAuthorizedFirstPerson.selectedOutputLogicFrame.steps.find((step) => (
                step.operation === "irregular-paradigm-selection"
            ));
            return {
                ...
                zeroLesson11Step: zeroLesson11Step ? [
                    zeroLesson11Step.consumedInputStatus,
                    zeroLesson11Step.consumedRegularStem,
                    zeroLesson11Step.value,
                    zeroLesson11Step.resultOperationId,
                ] : null,
            };
        })(),
        {
            ...
            zeroLesson11Step: ["complete-stem-candidate", "i-h", "0-i-h", "vnc-irregular-paradigm-selection"],
        }
    );

    s.eq(
        "The VNC application obtains Lesson 16 cooperation from semantic construction and rejects caller frames",
        (() => {
            const canonical = buildPronominalNncCooperationFrame("1sg");
            const plainLiteral = {
            ...
            const spreadCopy = { ...canonical };
            const structuredCopy = structuredClone(canonical);
            const jsonCopy = JSON.parse(JSON.stringify(canonical));
            const crossOwner = ctx.deriveClassicalNahuatlNonactiveStemRecord("mayāna", {
                verbClass: "B",
            ...
            const evaluateCallerFrame = (pronominalNncCooperationFrame) => application.evaluate({
                ...semanticRequest,
            ...
            return {
                validator: [
                    ctx.isClassicalNahuatlCooperationFrame(canonical, { subject: "1sg" }),
                    ctx.isClassicalNahuatlCooperationFrame(plainLiteral, { subject: "1sg" }),
                    ctx.isClassicalNahuatlCooperationFrame(spreadCopy, { subject: "1sg" }),
                    ctx.isClassicalNahuatlCooperationFrame(structuredCopy, { subject: "1sg" }),
                    ctx.isClassicalNahuatlCooperationFrame(jsonCopy, { subject: "1sg" }),
                    ctx.isClassicalNahuatlCooperationFrame(crossOwner, { subject: "1sg" }),
                ],
                crossOwnerIssued: ctx.isClassicalNahuatlNonactiveStemRecord(crossOwner, "mayāna"),
                semantic: project(application.evaluate(semanticRequest)),
                missingConstruction: project(application.evaluate({
                    ...semanticRequest,
                    construction: "",
                })),
                canonical: project(evaluateCallerFrame(canonical)),
                plainLiteral: project(evaluateCallerFrame(plainLiteral)),
                spreadCopy: project(evaluateCallerFrame(spreadCopy)),
                structuredCopy: project(evaluateCallerFrame(structuredCopy)),
                jsonCopy: project(evaluateCallerFrame(jsonCopy)),
                crossOwner: project(evaluateCallerFrame(crossOwner)),
                nestedCanonical: project(application.evaluate({
                    ...semanticRequest,
                    sentenceOptions: {
                        construction: "pronominal-nnc",
                        pronominalNncCooperationFrame: canonical,
                    },
                })),
            };
        })(),
        {
            validator: [true, false, false, false, false, false],
            crossOwnerIssued: true,
            semantic: ["authorized", "", "#n-0(0-i-h)0+⎕-0#", "nih"],
            missingConstruction: ["blocked", "zero-i-a-requires-pronominal-nnc-cooperation", "", ""],
            canonical: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
            plainLiteral: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
            spreadCopy: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
            structuredCopy: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
            jsonCopy: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
            crossOwner: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
            nestedCanonical: ["blocked", "classical-vnc-application-caller-authority-rejected", "", ""],
        }
    );
    ...
    s.eq(
        ...
        (() => {
            const onoMatrix = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("on-o", {
                subject: "1sg", mood: "indicative", tense: "present", compoundBoundaryType: "connective-t",
            });
            const huitzCommand = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("hui-tz", {
                subject: "2sg", mood: "indicative", tense: "present",
            });
            const aPositive = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ā", {
                subject: "3sg", mood: "indicative", tense: "present", polarity: "positive",
            });
            const aNegative = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ā", {
                subject: "3sg", mood: "indicative", tense: "present", polarity: "negative",
            });
            const maniMarked = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("mani", {
                subject: "3sg", mood: "indicative", tense: "present", predicateReferentKind: "individual-animate",
            });
            const goImperfect = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("yā", {
                subject: "1sg", mood: "indicative", tense: "imperfect",
            });
            const beThere = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("ye", {
                subject: "3sg", mood: "indicative", tense: "present", directionalPrefix: "on",
            });
            const cenHuiPlural = ctx.buildClassicalNahuatlVerbstemClassFrame("(cen-hui)", {
                valence: "intransitive", subject: "1pl", mood: "indicative", tense: "present",
            });
            const cenHuiSingular = ctx.buildClassicalNahuatlVerbstemClassFrame("(cen-hui)", {
                valence: "intransitive", subject: "1sg", mood: "indicative", tense: "present",
            });
            const nearby = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("cen-huica", {
    ...
    s.eq(
        ...
        (() => {
            const itzPlan = ctx.buildClassicalNahuatlIrregularVncParadigmPlan("hui-tz", {
                subject: "1sg",
                mood: "indicative",
            ...
            const come = ctx.buildClassicalNahuatlVerbstemClassFrame("(huāl-lā)", {
                valence: "intransitive",
                subject: "2sg",
                mood: "indicative",
                tense: "present",
    ...
    s.eq(
        ...
        (() => {
            const witnessed = ctx.buildClassicalNahuatlIdiomFrame("zan-huītz");
            const invented = ctx.buildClassicalNahuatlIdiomFrame("zan-mati");
            return {
    ...
    s.eq(
        ...
        (() => {
            const mati = ctx.buildClassicalNahuatlVerbstemClassFrame("(mati)", {
                valence: "intransitive",
                subject: "1sg",
                mood: "indicative",
                tense: "preterit",
            ...
            const ya = ctx.buildClassicalNahuatlVerbstemClassFrame("(yā)", {
                valence: "intransitive",
                subject: "1pl",
                mood: "indicative",
                tense: "present",
            ...
            const unavailable = ctx.buildClassicalNahuatlVerbstemClassFrame("(ā)", {
                valence: "intransitive",
                subject: "1sg",
                mood: "indicative",
                tense: "future",
    ...
    s.eq(
        ...
        (() => {
            const machinery = ctx.buildClassicalNahuatlVerbstemClassFrame("(mati)", {
                valence: "intransitive",
                subject: "1sg",
                mood: "indicative",
                tense: "preterit",
            ...
            const singleVncAnalysisStart = rendering.indexOf(
                "const singleVncAnalysis = createClassicalResultAnalysisDisclosure("
            );
            const singleVncAnalysisEnd = rendering.indexOf(
                "singleVncSection.append(",
                singleVncAnalysisStart
            );
            const singleVncAnalysis = singleVncAnalysisStart >= 0
                && singleVncAnalysisEnd > singleVncAnalysisStart
                ? rendering.slice(singleVncAnalysisStart, singleVncAnalysisEnd)
                : "";
            return {
                selectedUsesMah: machinery.formulaRealization.includes("(mah)"),
                lexemeId: output.lesson11LexemeId,
                paradigmTense: output.lesson11ParadigmTense,
                semanticTenseValue: output.lesson11SemanticTenseValue,
                ...
                rendersAlternativeAfterFormula:
                    singleVncAnalysis.indexOf("linearFormat") >= 0
                    && singleVncAnalysis.indexOf("lesson11Alternatives")
                        > singleVncAnalysis.indexOf("linearFormat"),
                rendersParadigmInResult: rendering.includes('createGrammarInspectorLine("stem paradigm", surfaceFrame.lesson11ParadigmRelation)'),
                tenseIdentityExplained: rendering.includes("Morphological form:"),
                irregularTensesPresent: shell.includes('<option value="preterit-as-present">preterit-as-present</option>')
                    && shell.includes('<option value="distant-past-as-past">distant-past-as-past</option>'),
                documentaryAuthorityTagsAbsent: !shell.includes("data-classical-authority-option")
                ...
                constructionOptionsReachMachinery:
                    rendering.includes("construction: state.constructionSelection === \"none\" ? \"\" : state.constructionSelection")
                    && rendering.includes("lexicalReading: state.lexicalReadingSelection === \"unspecified\" ? \"\" : state.lexicalReadingSelection")
                    && !rendering.includes("pronominalNncCooperationFrame: state.")
                    && !rendering.includes("lesson11LexicalReading: state."),
        ...
        })(),
        {
            ...
            lexemeId: "mati",
            paradigmTense: "preterit",
            semanticTenseValue: "preterit",
            ...
            tenseIdentityExplained: true,
            irregularTensesPresent: true,
            documentaryAuthorityTagsAbsent: true,
