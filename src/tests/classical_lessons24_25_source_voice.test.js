...
function buildVoiceSource(ctx, activeMachineryFrame, {
    ...
    const nonactiveStemRecord = ctx.deriveClassicalNahuatlNonactiveStemRecord("chihua", {
        verbClass: "A",
        sourceValence,
    });
    const sourceMachineryFrame = ctx.buildClassicalNahuatlDerivedVncFrame(activeMachineryFrame, {
        voice,
        nonactiveStemRecord,
        sourceValence,
        sourceSubject: "3sg",
...
function buildIntransitiveSource(ctx, stem, {
    ...
    return ctx.buildClassicalNahuatlVerbstemClassFrame(stem, {
        ...
        objectKind: "none",
        initialVowelKind: stem === "iuc-ci" ? "supportive" : "",
    });
}
...
function buildIntransitiveImpersonalSource(ctx, stem, {
    ...
    const nonactiveStemRecord = ctx.deriveClassicalNahuatlNonactiveStemRecord(stem, {
        verbClass,
...
function run(ctx = {}) {
    ...
    s.eq(
        ...
        },
        {
            ...
            impersonal: {
                ...
                kind: "classical-nahuatl-nonactive-vnc-derived-vnc-machinery-frame",
                voice: "impersonal",
                formula: "#0-0+tla(chihua-lo)0+0-0#",
                canonical: true,
            },
        }
    );
    ...
    s.eq(
        "The §25.11 written chihua evidence and §25.4 quantity-bearing chīhua Source converge on the canonical long typed lō bridge",
        (() => {
            const summarize = source => {
                const inventory = ctx.getClassicalNahuatlVncDerivationOptionInventory(source, {
                    derivationType: "causative",
                });
                const option = inventory.options[0] || null;
                return {
                    sourceStem: inventory.sourceStem,
                    targets: inventory.options.map(candidate => candidate.targetStem),
                    bridge: option?.typeTwoBridgeStem || "",
            ...
            return {
                short: summarize(passiveActive),
                long: summarize(buildActiveSource(ctx, { stem: "chīhua" })),
            };
        })(),
        {
            short: {
    ...
    s.eq(
        ...
        })(),
        {
            ...
            formula: "#ti-0+c-0+tē(chīhua-l-tia)0+0-0#",
        }
    );
    ...
    s.eq(
        ...
        })(),
        {
            ...
            operationCanonical: true,
            formula: "#ni-0+n-o+tē(chīhua-l-tia)0+0-0#",
        }
    );
    ...
    s.eq(
        ...
        })(),
        {
            ...
            typeTwo: {
                ...
                machineryStatus: "authorized",
                machineryCanonical: true,
                formula: "#ni-0+tē(tom-tia)0+0-0#",
            },
        }
    );

    s.eq(
        "The exact iuc-ci internal bridge supplies a typed nonhuman impersonal source without becoming a generated Lesson 20 result",
        (() => {
            ...
            const request = {
                ...
                requestedSourceVoice: "impersonal",
                sourceSubject: "3sg",
                subject: "3sg",
                mood: "indicative",
                tense: "present",
            ...
            const option = preview.controlFrame?.derivationOptionInventory
                ?.options?.find(candidate => candidate.targetStem === "iuc-xi-tiā") || null;
            const selectedRequest = {
                ...request,
                derivationOptionId: option?.optionId || "missing-iuc-xi-tia-option",
            };
            const scalar = application.evaluate(selectedRequest);
            const operation = scalar.resultFrame?.derivationOperationFrame || null;
            const participants = operation?.participantTransformFrame || {};
            const hostileOperation = jsonClone(operation);
            hostileOperation.selectedOption.implicitAgentObjectKind = "nonspecific-human";
            const hostileActiveChoice = application.evaluate({
                ...selectedRequest,
                requestedSourceVoice: "active",
                causativeObjectKind: "nonspecific-nonhuman",
            });
            const hostileInternalBridge = application.evaluate({
                ...selectedRequest,
                nonactiveStemRecord: {
            ...
            const coordinate = application.projectParadigmCoordinates(plan, [{
                subject: "3sg",
                mood: "indicative",
                tense: "present",
            }])[0] || null;
            const internalBridge = operation?.selectedOption?.typeTwoInternalBridgeFrame || null;
            return {
                applicationCanonical: ctx.isClassicalNahuatlVncApplicationFrame(scalar),
                actualSourceStem: scalar.resultFrame?.sourceMachineryFrame?.stem || "",
                optionImplicitAgentObjectKind: operation?.selectedOption?.implicitAgentObjectKind || "",
                internalBridge: {
                    stem: internalBridge?.nonactiveStem || "",
                    internalOnly: internalBridge?.internalPrerequisiteOnly === true,
                    userSelectable: internalBridge?.userSelectable === true,
                    lesson20Authority: internalBridge?.lesson20OperationAuthority === true,
                },
                ...
                paradigm: {
                    ...
                    sameTypedResult: coordinate?.typedSlotFrame
                ...
                hostileActiveChoice: [
                    hostileActiveChoice.authorizationStatus,
                    hostileActiveChoice.blockReason,
                ],
                hostileInternalBridge: [
                    hostileInternalBridge.authorizationStatus,
                    hostileInternalBridge.blockReason,
                    hostileInternalBridge.rejectedAuthorityFields,
                ],
            };
        })(),
        {
            applicationCanonical: true,
            actualSourceStem: "iuc-x-o-hua",
            optionImplicitAgentObjectKind: "nonspecific-nonhuman",
            internalBridge: {
                stem: "iuc-xi-hua",
                internalOnly: true,
            ...
            operationCanonical: true,
            targetObjects: [["nonspecific-nonhuman", "", "causative", 1]],
            formula: "#0-0+tla(uc-xi-tia)0+0-0#",
            written: "tlaucxitia",
            paradigm: {
                status: "authorized",
    ...
    s.eq(
        ...
        (() => {
            ...
            const oneObject = deriveCausativeTarget(ctx, tomiImpersonal, "tom-tiā", "1sg");
            const twoObjects = deriveCausative(ctx, impersonal.sourceMachineryFrame, "2sg");

            const lower = ctx.buildClassicalNahuatlVerbstemClassFrame("chihua", {
                subject: "3sg",
                mood: "indicative",
                tense: "present",
                verbClass: "A",
            ...
            const twoObjectActive = ctx.buildClassicalNahuatlMultipleObjectVncFrame(lower, {
                objectRequests: [
            ...
            const twoObjectPassive = ctx.buildClassicalNahuatlDerivedVncFrame(twoObjectActive, {
                voice: "passive",
                nonactiveStemRecord,
        ...
        })(),
        {
            ...
            threeObjects: {
                ...
                operationCanonical: true,
                machineryCanonical: true,
                formula: "#ti-0+c-0+tē+tē(chīhua-l-tia)0+0-0#",
            },
        }
    );
    ...
    s.eq(
        ...
        (() => {
            ...
            const impersonalSource = ctx.buildClassicalNahuatlDerivedVncFrame(twoObjectActive, {
                voice: "impersonal",
                nonactiveStemRecord,
                sourceObjectClusterFrame: twoObjectActive.multipleObjectClusterFrame,
                sourceValence: "multiple-object",
        ...
        {
            activeSource: ["authorized", "#0-0+tē+tla(chihua)0+0-0#"],
            nonactiveRecord: ["authorized", "chihua-lō"],
            impersonalSource: ["authorized", "", "#0-0+tē+tla(chihua-lo)0+0-0#", true],
            ...
            machinery: ["authorized", "", "#ti-0+tē+tē+tla(chīhua-l-tia)0+0-0#", true],
        }
    );
