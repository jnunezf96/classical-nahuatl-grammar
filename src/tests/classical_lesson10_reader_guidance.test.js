...
function run(ctx = {}) {
    const s = createSuite("classical_lesson10_reader_guidance");
    ...
    for (const record of accepted) {
        ...
        s.no(`mutation:${record.atomId} fails if its Lesson 10 reading job is removed`,
            ctx.isLesson10ReaderGuidanceExact(ideas.filter((candidate) => candidate.ideaId !== record.reviewGroupId)));
    }
    s.ok("Lesson 10 guidance stays collapsed, non-authorizing, and free of atom bookkeeping",
        visible.includes('data-classical-reader-guidance-lesson="10"')
        && !/Lesson 10[^]*atom/iu.test(visible)
