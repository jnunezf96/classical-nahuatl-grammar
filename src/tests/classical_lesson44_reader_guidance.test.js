...
function run(ctx = {}) {
    const s = createSuite("classical_lesson44_reader_guidance");
    const ledger = JSON.parse(fs.readFileSync(path.join(
        ROOT, "docs/canvas-progress/lesson44-review-ledger.json"), "utf8"));
    ...
    const ideas = ctx.LESSON44_READER_GUIDANCE_GROUPS;
    const panel = ctx.ClassicalAuthorityPanel();
    const start = panel.indexOf(
        'data-classical-reader-guidance-lesson="44"'
    );
    const end = panel.indexOf("</details>", start) + "</details>".length;
    const html = panel.slice(start, end);

    s.eq("Lesson 44 guidance covers the exact technical-proof groups", {
    ...
    s.ok(
        "Lesson 44 guidance remains descriptive rather than grammatical authority",
        ...
        && ctx.isLesson44ReaderGuidanceExact(ideas),
    );
