...
function run(ctx = {}) {
            ...
            const missingMachineryTagIds = items
                .filter((item) => {
                    const implementationRefs =
                        item.ruleLogicTag?.implementationRefs || [];
                    return implementationRefs.length === 0
                        || implementationRefs.some(relativePath => (
    ...
    s.eq(
        "Andrews tense slot inventory matches runtime VNC tense frames",
        {
            ...
            frame: ctx.getVncTenseMorphFrame().andrewsMoodTenseInventory,
            profiles: Object.fromEntries(
                ["presente", "presente-habitual", "imperfecto", "futuro", "preterito", "pasado-remoto", "optativo"]
                    .map((tense) => {
                        const profile = ctx.getVncTenseProfile(tense);
                        return [tense, {
                            morph: profile?.morph,
                            mood: profile?.mood,
                            tense: profile?.tense,
            ...
            nonAndrewsFiniteExtensionsKeptOutOfPredicateNominalSources: [
        ...
        },
        {
            ...
            profiles: {
                presente: { morph: "\u00d8", mood: "indicative", tense: "present" },
                "presente-habitual": { morph: "ni", mood: "indicative", tense: "customary-present" },
                imperfecto: { morph: "yā", mood: "indicative", tense: "imperfect" },
                futuro: { morph: "z", mood: "indicative", tense: "future" },
                preterito: { morph: "\u00d8", mood: "indicative", tense: "preterit" },
                "pasado-remoto": { morph: "cā", mood: "indicative", tense: "distant-past" },
                optativo: { morph: "\u00d8", mood: "optative", tense: "nonpast" },
            },
    ...
    s.eq(
        "core NNC inventory keeps Andrews structural Classical dyads",
        {
            ...
            runtimeLesson12Dyads: ctx.getNncAbsolutiveSubjectPositionFrame().num1Num2Rule.singularCommonDyads,
        },
        {
    ...
    s.eq(
        "documentary formula index covers derivational families without exposing a workbench runtime lane",
        {
            ...
            derivationalSchemaNotExecutable: ctx.renderAndrewsFormulaTemplate("derivational-routes"),
            obsoleteWorkbenchApis: [
                typeof ctx.getDerivationalRouteFormulaSchema,
                typeof ctx.buildDerivationalRouteFormulaWorkbenchExamples,
                typeof ctx.buildNominalizationFormulaWorkbenchExamples,
            ],
        },
        {
