...
const selection = require("../../docs/canvas-progress/checkpoint010_fact_selection.json");
...
function run(ctx = {}) {
    const s = createSuite("classical_checkpoint010_grammar_fact_projection");
    s.eq("checkpoint 010 selects exactly 500 read-only grammar facts", selection.counts.selectedAtoms, 500);
    s.eq("checkpoint 010 reuses fewer than 50 shared typed owners", selection.counts.selectedOwners, 30);
