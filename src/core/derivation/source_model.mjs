...
export function createDerivationSourceModelContext(targetObject = globalThis, installationContext = null) {
    ...
    function buildActiveActionCompoundEmbedTargetContinuationFrame({
      ...
      const matrixRoot = String(matrix.root || "").trim();
    ...
    function getDerivationContinuationMatrixFrame(record = null) {
      ...
      return {
        ...
        root: String(matrix.root || framedMatrix.root || source.matrixRoot || "").trim(),
    ...
    function buildFallbackDerivationSourceModel(verbMeta = {}, verb = "", analysisVerb = "") {
      ...
      const outerPieces = parseLanguage === "current-regex" && Array.isArray(structuralOuterPieces) ? structuralOuterPieces.map(piece => normalizeDerivationSourceOuterPiece(piece)).filter(Boolean) : (() => {
        ...
        const impersonalPieces = meta?.hasImpersonalTlaPrefix === true ? [{
          type: "impersonal",
          value: "ta"
        }] : [];
    ...
    function buildSupportivePrecedingSurfaceFromMorphologyInput({
      ...
      return [targetObject.normalizeComposerStem(directionalPrefix || ""), lexicalPrefix, ...explicitPrefixes, hasImpersonalTlaPrefix ? "tla" : ""].filter(Boolean).join("");
    }
    ...
    const PATIENTIVO_PRELOCATIVE_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      status: "andrews-canvas-source-backed",
      sourceStates: ["absolutive"],
      evidence: ["Andrews 30.15.2", "Andrews 39.7"]
    }), Object.freeze({
      id: "tla-nequi",
      ...
      status: "andrews-canvas-source-backed",
      sourceStates: ["absolutive", "possessive"],
      evidence: ["Andrews 39.7", "Andrews 39.8"]
    }), Object.freeze({
      id: "tla-ih-tlani",
    ...
    const DEFAULT_PATIENTIVO_COMPOUND_EMBED_MATRIX_ROOT = "miqui";
    const PATIENTIVO_COMPOUND_EMBED_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      evidence: ["Andrews 39.6"]
    })]);
    const DEFAULT_PATIENTIVO_NOMINAL_COMPOUND_MATRIX_ROOT = "cal";
    ...
    const PATIENTIVO_CHARACTERISTIC_PROPERTY_SUFFIX = "yōtl";
    const PATIENTIVO_CHARACTERISTIC_PROPERTY_POSSESSIVE_SUFFIX = "yō";
    const DEFAULT_PATIENTIVO_CHARACTERISTIC_PROPERTY_MATRIX_ROOT = "chicāhua";
    const PATIENTIVO_CHARACTERISTIC_PROPERTY_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      evidence: ["Andrews 39.9"]
    })]);
    const DEFAULT_ACTIVE_ACTION_COMPOUND_EMBED_MATRIX_ROOT = "tzahtzi";
    const ACTIVE_ACTION_COMPOUND_EMBED_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      status: "andrews-canvas-source-backed",
      matrixValency: "intransitive",
      evidence: ["Andrews 37.5.4"]
    })]);
    const DEFAULT_ACTIVE_ACTION_NOMINAL_COMPOUND_MATRIX_ROOT = "cal";
    ...
    const DEFAULT_CUSTOMARY_AGENTIVE_COMPOUND_EMBED_MATRIX_ROOT = "toca";
    const DEFAULT_CUSTOMARY_AGENTIVE_NOMINAL_COMPOUND_MATRIX_ROOT = DEFAULT_ACTIVE_ACTION_NOMINAL_COMPOUND_MATRIX_ROOT;
    const DEFAULT_PRETERIT_AGENTIVE_OWNERHOOD_MATRIX_ROOT = "huā";
    ...
    const CUSTOMARY_AGENTIVE_COMPOUND_EMBED_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      evidence: ["Andrews 36.3: fully nominalized customary-present agentive stems can fill compound embed position"]
    })]);
    const PRETERIT_AGENTIVE_OWNERHOOD_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      evidence: ["Andrews 35.9: general-use preterit-agentive stems incorporate into *tla-(-hua)"]
    }), Object.freeze({
      ...
      evidence: ["Andrews 35.10: abundant ownerhood uses *tla-(-yo-a)"]
    })]);
    const PRETERIT_AGENTIVE_COMPLEMENT_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      evidence: ["Andrews 35.12: incorporated-complement VNC"]
    }), Object.freeze({
      id: "te-cahua",
    ...
    const PRETERIT_AGENTIVE_ADVERBIAL_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      evidence: ["Andrews 35.12: preterit-agentive nounstem as incorporated adverb of manner"]
    })]);
    const ORDINARY_NOUN_OWNERHOOD_MATRIX_SPECS = Object.freeze([Object.freeze({
      ...
      status: "andrews-canvas-source-backed",
      matrixValency: "transitive",
      defaultForNounClasses: ["tl"],
      label: "ownerhood matrix for compatible t-class nouns",
      evidence: ["Andrews 35.9: *tla-(-ē) incorporates tl-class nounstems"]
    }), Object.freeze({
      id: "tla-hua-ownerhood",
      classicalMatrix: "*tla-(-hua)",
      root: "huā",
      ...
      evidence: ["Andrews 35.9: *tla-(-hua) incorporates in and zero nounstems"]
    }), Object.freeze({
      id: "tla-yo-a-abundant-ownerhood",
      classicalMatrix: "*tla-(-yo-a)",
      root: "yoā",
      ...
      status: "andrews-canvas-source-backed",
      matrixValency: "transitive",
      defaultForNounClasses: ["tl", "tli", "in", "zero"],
      label: "abundant ownerhood matrix",
      evidence: ["Andrews 35.10: abundant ownerhood uses *tla-(-yo-a)"]
    })]);
    ...
    function resolvePatientivoPrelocativeMatrixSpec(matrixRoot = DEFAULT_PATIENTIVO_PRELOCATIVE_MATRIX_ROOT) {
      ...
      const spec = PATIENTIVO_PRELOCATIVE_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || Array.isArray(entry.aliases) && entry.aliases.includes(normalizedRoot));
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolvePatientivoCompoundEmbedMatrixSpec(matrixRoot = DEFAULT_PATIENTIVO_COMPOUND_EMBED_MATRIX_ROOT) {
      ...
      const spec = PATIENTIVO_COMPOUND_EMBED_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || Array.isArray(entry.aliases) && entry.aliases.includes(normalizedRoot));
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolvePatientivoNominalCompoundMatrixSpec(matrixRoot = DEFAULT_PATIENTIVO_NOMINAL_COMPOUND_MATRIX_ROOT) {
      ...
      const spec = PATIENTIVO_NOMINAL_COMPOUND_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || Array.isArray(entry.aliases) && entry.aliases.includes(normalizedRoot));
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolvePatientivoCharacteristicPropertyMatrixSpec(matrixRoot = DEFAULT_PATIENTIVO_CHARACTERISTIC_PROPERTY_MATRIX_ROOT) {
      ...
      const spec = PATIENTIVO_CHARACTERISTIC_PROPERTY_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || Array.isArray(entry.aliases) && entry.aliases.includes(normalizedRoot));
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolveActiveActionCompoundEmbedMatrixSpec(matrixRoot = DEFAULT_ACTIVE_ACTION_COMPOUND_EMBED_MATRIX_ROOT) {
      ...
      const spec = ACTIVE_ACTION_COMPOUND_EMBED_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || Array.isArray(entry.aliases) && entry.aliases.includes(normalizedRoot));
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolveActiveActionNominalCompoundMatrixSpec(matrixRoot = DEFAULT_ACTIVE_ACTION_NOMINAL_COMPOUND_MATRIX_ROOT) {
      ...
      const spec = ACTIVE_ACTION_NOMINAL_COMPOUND_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || Array.isArray(entry.aliases) && entry.aliases.includes(normalizedRoot));
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolveCustomaryAgentiveCompoundEmbedMatrixSpec(matrixRoot = DEFAULT_CUSTOMARY_AGENTIVE_COMPOUND_EMBED_MATRIX_ROOT) {
      ...
      const spec = CUSTOMARY_AGENTIVE_COMPOUND_EMBED_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || entry.id === normalizedRoot || entry.classicalMatrix === normalizedRoot);
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolvePreteritAgentiveOwnerhoodMatrixSpec(matrixRoot = DEFAULT_PRETERIT_AGENTIVE_OWNERHOOD_MATRIX_ROOT) {
      ...
      const spec = PRETERIT_AGENTIVE_OWNERHOOD_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || entry.id === normalizedRoot);
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolvePreteritAgentiveComplementMatrixSpec(matrixRoot = DEFAULT_PRETERIT_AGENTIVE_COMPLEMENT_MATRIX_ROOT) {
      ...
      const spec = PRETERIT_AGENTIVE_COMPLEMENT_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || entry.id === normalizedRoot || entry.classicalMatrix === normalizedRoot);
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function resolvePreteritAgentiveAdverbialMatrixSpec(matrixRoot = DEFAULT_PRETERIT_AGENTIVE_ADVERBIAL_MATRIX_ROOT) {
      ...
      const spec = PRETERIT_AGENTIVE_ADVERBIAL_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || entry.id === normalizedRoot || entry.classicalMatrix === normalizedRoot);
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function normalizeOrdinaryNounOwnerhoodNounClass(nounClass = "") {
      return typeof targetObject.normalizeOrdinaryNncNounClass === "function"
        ? targetObject.normalizeOrdinaryNncNounClass(nounClass)
        : "";
    }
    function resolveOrdinaryNounOwnerhoodMatrixSpec(matrixRoot = "") {
      const normalizedRoot = String(matrixRoot || "").trim().toLowerCase();
      const spec = ORDINARY_NOUN_OWNERHOOD_MATRIX_SPECS.find(entry => entry.root === normalizedRoot || entry.id === normalizedRoot);
      return spec ? {
        ...spec,
        supported: true,
        diagnostics: []
    ...
    function buildPatientivoCompoundEmbedVerbInput({
      ...
      const normalizedMatrixRoot = matrixSpec.supported ? matrixSpec.root : "";
    ...
    function buildPatientivoCompoundEmbedOperationFrame({
      ...
      const sourceRoot = String(sourcePayload?.patientivoStem || "").trim();
      const matrix = matrixSpec && typeof matrixSpec === "object" ? matrixSpec : {};
      const matrixRoot = matrix.supported ? String(matrix.root || "").trim() : "";
    ...
    function resolvePatientivoCompoundEmbedFormationFrame({
      ...
      return {
        ...
        matrix: {
          ...
          root: matrixSpec?.supported ? matrixSpec.root : "",
    ...
    function buildPatientivoCharacteristicPropertyEmbedContinuationContract({
      ...
      if (!resolvedTargetContinuationFrame) {
        ...
        if (String(targetFrame.stem || "").trim() !== `${incorporatedRoot}${matrixSpec.supported ? matrixSpec.root : ""}`) {
      ...
      return attachDerivationContinuationGrammarContract({
        ...
        incorporatedRoot,
        matrixRoot: matrixSpec.supported ? matrixSpec.root : String(matrixRoot || "").trim(),
        matrix: matrixSpec,
        compoundVerbInput,
    ...
    function buildPreteritAgentiveCompoundEmbedContinuationContract({
      ...
      if (!resolvedTargetContinuationFrame) {
        ...
        if (String(targetFrame.stem || "").trim() !== `${normalizedPreteritAgentiveStem}${matrixSpec.supported ? matrixSpec.root : ""}`) {
    ...
    function buildCustomaryAgentiveCompoundEmbedContinuationContract({
      ...
      if (!resolvedTargetContinuationFrame) {
        ...
        if (String(targetFrame.stem || "").trim() !== `${normalizedCustomaryAgentiveStem}${matrixSpec.supported ? matrixSpec.root : ""}`) {
    ...
    function buildPreteritAgentiveOwnerhoodContinuationContract({
      ...
      if (!resolvedTargetContinuationFrame) {
        ...
        const expectedTargetStem = `${normalizedPreteritAgentiveStem}${matrixSpec.supported ? matrixSpec.root : ""}`;
        if (targetSourceFrame && targetSourceFrame !== sourceContinuationFrame) {
    ...
    function buildOrdinaryNounOwnerhoodContinuationContract({
      ...
      if (!resolvedTargetContinuationFrame) {
        ...
        const targetSourceFrame = resolvedTargetContinuationFrame.sourceFrame || null;
        const targetFrameStem = String(targetFrame.stem || "").trim();
        const expectedTargetStem = `${normalizedNounStem}${matrixSpec.supported ? matrixSpec.root : ""}`;
        if (targetSourceFrame && targetSourceFrame !== sourceContinuationFrame) {
      ...
      return attachDerivationContinuationGrammarContract({
        ...
        sourceContinuationFrame: isGeneratedOutputTypedDerivationContinuationFrame(sourceContinuationFrame) ? sourceContinuationFrame : null,
        targetContinuationFrame: resolvedTargetContinuationFrame || null,
        matrixRoot: matrixSpec.supported ? matrixSpec.root : resolvedMatrixRoot,
        surfaceMatrix: matrixSpec.surfaceMatrix || "",
...
export function installDerivationSourceModelGlobals(targetObject = globalThis, installationContext = null) {
    const api = createDerivationSourceModelContext(targetObject, installationContext);
