...
export function createClassicalComparisonValidationSemanticOperationsApi(
  ...
  function ordinary(stem, { state = "absolutive", possessor = "", sourceClass = "", subject = "3sg" } = {}) {
    const sourceFrame = target.buildClassicalNahuatlOrdinaryNncSourceFrame({ stem, ...(sourceClass ? { sourceClass } : {}) });
  ...
  function sources() {
    return {
      cal: ordinary("cal", { subject: "3common" }),
      mich: ordinary("mich"),
      teuc: ordinary("tēuc"),
      possessiveCal: ordinary("cal", { state: "possessive", possessor: "3sg", subject: "3common" }),
  ...
  function evaluateProfile(profileId, source) {
    if (profileId === "superlative-example-variants") {
      const slots = {
        topic: ordinary("ichpōch", { sourceClass: "tli-1" }),
        predicate: ordinary("cualnēzqui", { sourceClass: "zero" }),
      };
      // The nine ordinary examples in Andrews 53.7, not the following
      // honorific series. All forms are generated by the comparison owner.
      return [
        ...["cencah", "cencah-huel", "za-cencah-huel"].map(value => ({
          routeId: "superlative-adverbial",
          choices: { superlativeAdverbial: value, copula: true },
        })),
        ...["cem", "cenquizca", "cemahcica"].map(value => ({
          routeId: "superlative-incorporated",
          choices: { incorporatedSuperlative: value },
        })),
        ...["ahcic", "cemahcic", "tlapanahuia"].map(value => ({
          routeId: "superlative-principal-ic",
          choices: { superlativePrincipal: value, adjunctorIn: true },
        })),
      ].map(request => target.evaluateClassicalNahuatlComparison({
        ...request, slots, choices: { ...request.choices, sentenceType: "declarative" },
      }));
    }
    if (profileId === "ordinary-superlative") {
      return target.evaluateClassicalNahuatlComparison({
        ...
        choices: { superlativeAdverbial: "cencah", copula: true, sentenceType: "declarative" },
      });
    }
  ...
  function buildClassicalComparisonValidationFrame(
    ...
    const frame = deepFreeze({
      ...
      result: {
        ...
        projectionDifferences: results.map(result => observeFormulaProjectionDifference(
          result.formulaRecord, result.formulaRealizationRecord)),
        liveResult: primary,
        liveResults: results,
      },
      ...
      analysis: {
        semanticBoundary: allCanonical ? results.map(result => ({
          routeId: result.routeId,
          relation: result.relation,
          operation: result.operation,
          choices: result.operationFrame?.choices,
        })) : null,
        ...(profileId === "superlative-example-variants" ? {
          semanticBoundary: allCanonical ? results.map(result => ({
            relation: result.relation,
            routeId: result.routeId,
            topicStem: result.operationFrame?.sourceSlots?.topic?.sourceIdentityStem,
            predicateStem: result.operationFrame?.sourceSlots?.predicate?.sourceIdentityStem,
            predicateHonorific: result.operationFrame?.sourceSlots?.predicate?.honorific,
            choices: result.operationFrame?.choices,
          })) : null,
        } : {}),
        ...(profileId === "ordinary-superlative" ? {
          // A measured witness, not the selected profile's name. Honorific
          // classification belongs to the comparison owner, not this recipe.
          semanticBoundary: allCanonical ? {
            relation: primary.relation,
            routeId: primary.routeId,
            topicStem: primary.operationFrame?.sourceSlots?.topic?.sourceIdentityStem,
            predicateStem: primary.operationFrame?.sourceSlots?.predicate?.sourceIdentityStem,
            predicateHonorific: primary.operationFrame?.sourceSlots?.predicate?.honorific,
            superlativeAdverbial: primary.operationFrame?.choices?.superlativeAdverbial,
          } : null,
        } : {}),
