...
export function createClassicalNahuatlParticlesApi(
  ...
  installationContext = null
    ...
    const CLASSICAL_NAHUATL_LESSON3_NONPARTICLE_RANKS = Object.freeze({
      om: Object.freeze({
        candidate: "om",
        unitKind: "numeral-combination-morpheme",
        particleAllowed: false,
        meaning: "plus",
        construction: "numeral-combination"
      }),
      iuh: Object.freeze({
        candidate: "iuh",
        unitKind: "adverbialized-verbal-nuclear-clause",
        particleAllowed: false,
        meaning: "thus",
        sourceVerbstem: "(ihui)"
      }),
...
l3-ca-no-zo|ca no zo|collocation|collocation-sequence|3.5
...
l3-ca-no-zo|thus it is|base collocation licensed for particle honorificization
...
`.trim().split("\n").map(row => Object.freeze(row.split("|"))));
    ...
    function classifyClassicalNahuatlLesson3ParticleCandidate(candidate = "") {
      const requested = String(candidate == null ? "" : candidate).trim();
      const normalized = normalizeClassicalNahuatlParticleLookupValue(requested);
      const entry = findClassicalNahuatlLesson3ParticleEntry(requested);
      if (entry) {
        return freezeClassicalNahuatlLesson3ParticleResult({
          kind: "classical-nahuatl-lesson3-lexical-rank-frame",
          candidate: requested,
          unitKind: "particle",
          particleAllowed: true,
          particleId: entry.id,
          lexicalRank: "minor",
          monomorphemic: entry.formulaSegments.length === 1,
          internalMorphologicalStructure: entry.formulaSegments.length === 1
            ? "none"
            : "ordered-particle-collocation"
        });
      }
      const distinction = CLASSICAL_NAHUATL_LESSON3_NONPARTICLE_RANKS[
        normalized === "ihui" && /^\(/u.test(requested) ? "(ihui)" : normalized
      ] || null;
      return freezeClassicalNahuatlLesson3ParticleResult({
        kind: "classical-nahuatl-lesson3-lexical-rank-frame",
        candidate: requested,
        unitKind: distinction?.unitKind || "unlicensed",
        particleAllowed: false,
        particleId: "",
        lexicalRank: distinction ? "nonparticle" : "unknown",
        meaning: distinction?.meaning || "",
        construction: distinction?.construction || "",
        sourceVerbstem: distinction?.sourceVerbstem || ""
      });
    }
    function buildClassicalNahuatlParticleStructureFrame(entry = null) {
      return freezeClassicalNahuatlLesson3ParticleResult({
        kind: "classical-nahuatl-particle-structure-frame",
        particleId: entry?.id || "",
        unitKind: "particle",
        lexicalRank: "minor",
        monomorphemic: entry?.formulaSegments?.length === 1,
        ...
        normalSyllableCount: 1,
        exceptionalSyllableCount: 2,
        clauseDependence: entry?.placement?.scope === "bound-to-following"
          || entry?.placement?.scope === "bound-to-previous"
          || entry?.placement?.scope === "written-suffixed-to-preceding-nominal-nuclear-clause"
          ? "required"
          : "contextual",
        stressAllowedWhenDependent: true,
        degenerateClauseDefaultClassification: "particle",
        reclassificationRequiresClearProof: true,
        derivedFromTypedParticleIdentity: true,
        userSelectable: false
      });
    }
    function buildClassicalNahuatlParticleStructuralRoleFrame(entry = null, structuralRole = "") {
      const requestedRole = String(structuralRole || "").trim();
      if (!requestedRole) return null;
      const functionScope = entry?.functionScope || "";
      const permitted = requestedRole === "independent-utterance"
        ? functionScope === "interjection" || entry?.placement?.scope === "independent-utterance"
        : requestedRole === "principal-clause"
          ? false
          : requestedRole === "compound-embed"
            ? entry?.id === "l3-ah-negative"
            : requestedRole === "matrix-stem"
              ? false
              : requestedRole === "adverbial-modifier"
                ? functionScope === "adverbial-modifier"
                : requestedRole === "exclamation"
                  ? functionScope === "interjection"
                  : requestedRole === "clause-introducer"
                    ? functionScope === "clause-introducer"
                    : false;
      const frame = freezeClassicalNahuatlLesson3ParticleResult({
        kind: "classical-nahuatl-particle-structural-role-frame",
        particleId: entry?.id || "",
        requestedRole,
        permitted,
        functionScope,
        matrixStemAllowed: false,
        particleRemainsOutsideNuclearClause: true,
        decisionMadeByApplication: true,
        userSelectableRestriction: false
      });
      issuedParticleStructuralRoleFrames.set(frame, Object.freeze({
        particleId: frame.particleId,
        requestedRole,
        permitted,
        functionScope
      }));
    ...
    function isClassicalNahuatlParticleStructuralRoleFrame(frame = null) {
      const receipt = frame && typeof frame === "object"
        ? issuedParticleStructuralRoleFrames.get(frame)
        : null;
      return Boolean(receipt
        && frame.kind === "classical-nahuatl-particle-structural-role-frame"
        && frame.particleId === receipt.particleId
        && frame.requestedRole === receipt.requestedRole
        && frame.permitted === receipt.permitted
        && frame.functionScope === receipt.functionScope
        && frame.matrixStemAllowed === false
        && frame.particleRemainsOutsideNuclearClause === true
        && frame.decisionMadeByApplication === true
        && frame.userSelectableRestriction === false
        && Object.isFrozen(frame));
    }
    function selectClassicalNahuatlNegativeParticleFrame(request = {}) {
      ...
      const frame = freezeClassicalNahuatlLesson3ParticleResult({
        kind: "classical-nahuatl-negative-particle-selection-frame",
        version: CLASSICAL_NAHUATL_LESSON3_PARTICLES_VERSION,
          ...
          : unexpectedKey
            ...
            : polarity !== "negative"
              ...
              : particleResultFrame?.blockReason
                || "classical-negative-particle-result-required",
        polarity,
        precedingParticleId,
        sentenceKind,
        ...
        selectedByApplication: true,
        userSelectableAllomorph: false,
        callerSuppliedSurfaceAuthority: false,
        callerSuppliedFormulaAuthority: false
      });
      issuedNegativeParticleSelectionFrames.set(frame, Object.freeze({
        authorized,
    ...
    function isClassicalNahuatlNegativeParticleSelectionFrame(frame = null) {
      const receipt = frame && typeof frame === "object"
        ? issuedNegativeParticleSelectionFrames.get(frame)
        : null;
      return Boolean(receipt
        && frame.kind === "classical-nahuatl-negative-particle-selection-frame"
        ...
        && frame.selectedByApplication === true
        && frame.userSelectableAllomorph === false
    ...
    function buildClassicalNahuatlParticleLexicalFactFrame(entry = null) {
      ...
      const antecessiveProfile = entry.id === "l3-o-antecessive"
        ...
        : null;
      const speakerProfile = entry.id === "l3-e-vocative"
        ? freezeClassicalNahuatlLesson3ParticleResult({
            requiredSpeakerGender: "male",
            restrictionEnforcedByApplication: true,
          })
        : entry.id === "l3-no-interjection"
          ? freezeClassicalNahuatlLesson3ParticleResult({
              requiredSpeakerGender: "female",
              restrictionEnforcedByApplication: true,
            })
          : null;
      const frame = freezeClassicalNahuatlLesson3ParticleResult({
      ...
      issuedParticleLexicalFactFrames.set(
        ...
        Object.freeze({
          ...
          speakerProfile: frame.speakerProfile,
        })
      );
      return frame;
    }
    function isClassicalNahuatlParticleLexicalFactFrame(frame = null) {
      ...
      return Boolean(
        ...
        && frame.speakerProfile === receipt.speakerProfile
    ...
    function buildClassicalNahuatlParticleResultFrame(
      ...
      const allowedOptionKeys = new Set(["precedingParticleSourceFrame", "structuralRole", "speakerGender"]);
      ...
      const lexicalFactFrame = entry
        ...
        : null;
      const structureFrame = entry
        ? buildClassicalNahuatlParticleStructureFrame(entry)
        : null;
      const structuralRoleFrame = entry
        ? buildClassicalNahuatlParticleStructuralRoleFrame(
            entry,
            options.structuralRole
          )
        : null;
      const structuralRoleLicensed = !options.structuralRole
        || Boolean(
          isClassicalNahuatlParticleStructuralRoleFrame(structuralRoleFrame)
          && structuralRoleFrame.permitted
        );
      const requestedSpeakerGender = String(options.speakerGender || "").trim();
      const requiredSpeakerGender = entry?.id === "l3-e-vocative"
        ? "male"
        : entry?.id === "l3-no-interjection"
          ? "female"
          : "";
      const speakerContextLicensed = !requiredSpeakerGender
        ...
        || requestedSpeakerGender === requiredSpeakerGender;
                ...
                : rawNegativeParticle && !rawNegativeLicensed
                  ...
                  : caContextRequired && !precedingSourceAuthorized
                    ...
                    : !caContextLicensed
                      ...
                      : !structuralRoleLicensed
                        ? `classical-particle-structural-role-not-licensed:${String(options.structuralRole || "")}`
                        : !speakerContextLicensed
                          ? `classical-particle-speaker-context-required:${requiredSpeakerGender}`
                      : missingCapability
      ...
      issuedParticleResultFrames.set(
        ...
        Object.freeze({
          ...
          structureFrame: issued.structureFrame,
          structuralRoleFrame: issued.structuralRoleFrame,
    ...
    function isClassicalNahuatlParticleResultFrame(frame = null) {
      ...
      return Boolean(
        ...
        && frame.structureFrame === receipt.structureFrame
        && frame.structuralRoleFrame === receipt.structuralRoleFrame
        && frame.formula === receipt.formula
        && frame.surface === receipt.surface
        && (
          authorized
    ...
    const CLASSICAL_NAHUATL_SENTENCE_PARTICLE_HONORIFIC_IDS = Object.freeze({
      ...
      "l3-auh-interjection": "l3-auhtzin",
      "l3-ca-no-zo": "l3-ca-no-zotzin"
    });
    function getClassicalNahuatlSentenceParticleEntries() {
      const seen = new Set();
      return getClassicalNahuatlParticleSourceEntries().filter(entry => {
        const directlySelectableInterjection = entry.functionScope === "interjection"
          && entry.id !== "l3-e-vocative";
        const directlySelectableNegative = entry.functionScope === "negation"
          && !["l3-ah-negative", "l3-ca-negative"].includes(entry.id);
        const directlySelectableCollocation = entry.functionScope === "collocation"
          && (entry.curriculumCoordinate === "3.4" || entry.id === "l3-ca-no-zo");
        const composable = (
          entry.clauseComposable === true
          || directlySelectableInterjection
          || directlySelectableNegative
          || directlySelectableCollocation
    ...
    function isClassicalNahuatlSentenceParticleOutputFrame(frame = null) {
      return isClassicalNahuatlParticleResultFrame(frame)
        || isClassicalNahuatlParticleHonorificResultFrame(frame);
    }
    ...
    function buildClassicalNahuatlSentenceParticleLayerFrame(options = {}) {
      ...
      const speakerGender = String(options.speakerGender || "unspecified").trim();
      const canonicalInput =
        resolveClassicalNahuatlParticleCanonicalSentenceInput(options);
      const nuclearClauseKind = canonicalInput.nuclearClauseKind;
      const nuclearFormula = canonicalInput.nuclearFormula;
      ...
      const placementAuthorized = [
        "clause-initial",
        "independent-utterance",
        "collocation-sequence",
        "floating"
      ].includes(inventoryEntry?.placement?.scope);
      ...
      const selectedEntry =
        isClassicalNahuatlSentenceParticleOutputFrame(particleResultFrame)
          ? getClassicalNahuatlParticleSourceEntries()
            .find(entry => entry.id === (
              honorificizedEntryId || particleResultFrame.particleId
            ))
            || null
          : null;
      ...
      return issueClassicalNahuatlParticleSentenceLayerFrame({
          ...
          ? authorityPath
            ...
            : !nuclearFormula
              ...
              : inventoryEntry && !placementAuthorized
                ...
                : !speakerContextLicensed
                  ? `sentence-particle-speaker-context-required:${requiredSpeakerGender}`
        ...
        particleResultFrame:
          authorized ? particleResultFrame : null,
        honorificSourceFrame:
          honorificizedAuthorized ? honorificSourceFrame : null,
        selectedEntry,
        baseSelectedEntry,
        specializedSentenceControlOwnsEntry,
        ...
        honorificizedEntryId,
        sourceForm,
        functionScope: syntacticEntry?.functionScope || "",
        functionClass: syntacticEntry?.functionClass || null,
        placement: syntacticEntry?.placement || null,
        placementAuthorized,
        speakerGender,
    ...
    function installClassicalNahuatlParticlesClassicGlobals() {
      ...
      Object.assign(globalTarget, {
        ...
        classifyClassicalNahuatlLesson3ParticleCandidate,
        ...
        isClassicalNahuatlParticleResultFrame,
        isClassicalNahuatlParticleStructuralRoleFrame,
    ...
    api.classifyClassicalNahuatlLesson3ParticleCandidate = classifyClassicalNahuatlLesson3ParticleCandidate;
    ...
    api.isClassicalNahuatlParticleStructuralRoleFrame = isClassicalNahuatlParticleStructuralRoleFrame;
    api.selectClassicalNahuatlNegativeParticleFrame = selectClassicalNahuatlNegativeParticleFrame;
    api.isClassicalNahuatlNegativeParticleSelectionFrame = isClassicalNahuatlNegativeParticleSelectionFrame;
