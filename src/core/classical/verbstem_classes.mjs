...
export function createClassicalNahuatlVerbstemClassesRuntime(targetObject = globalThis) {
    ...
    const classicalNahuatlLesson7IssuedSourceSelectionFrames = new WeakSet();
    const classicalNahuatlLesson7IssuedVerbstemClassFrames = new WeakSet();
    ...
    const CLASSICAL_NAHUATL_LESSON7_STRUCTURE_ACTIONS = Object.freeze({
      ...
      RECORD_DOCUMENTARY_EMBED_MATRIX: "record-documentary-embed-matrix-witness-without-source-authority"
    });
    const CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS = Object.freeze({
      SELECT_WHOLE_STEM: "select-typed-whole-stem-source",
      SELECT_INTERNAL_MORPHEMES: "select-typed-internal-morpheme-source",
      SELECT_EMBED_MATRIX: "select-typed-embed-matrix-source",
      ACCEPT_USER_TYPED_SELECTION: "accept-user-source-selection-from-matching-typed-constituents",
      BLOCK_INVALID_TYPED_SELECTION: "block-user-source-selection-that-contradicts-typed-constituents",
    ...
    const CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS = Object.freeze({
      ...
      APPLY_OPTIONAL_ON_ITTA_CONTRACTION: "apply-optional-on-itta-directional-contraction",
    ...
    const CLASSICAL_NAHUATL_LESSON9_SENTENCE_ACTIONS = Object.freeze({
      MARK_LOWER_VNC_PROVISIONAL: "consume-complete-optative-vnc-in-sentence-composition",
      ...
      ADD_PREFACE_PARTICLE_OUTSIDE_VNC: "add-optative-preface-particle-outside-vnc",
      ADD_INTRODUCTORY_MODIFIER_OUTSIDE_VNC: "add-optative-introductory-modifier-outside-vnc",
      ...
      CARRY_SENTENCE_FINALIZER_TO_SELECTED_OUTPUT: "carry-composed-optative-sentence-to-selected-output"
    });
    const CLASSICAL_NAHUATL_LESSON10_SENTENCE_ACTIONS = Object.freeze({
      MARK_LOWER_VNC_PROVISIONAL: "consume-complete-admonitive-vnc-in-sentence-composition",
      ...
      CARRY_SENTENCE_FINALIZER_TO_SELECTED_OUTPUT: "carry-composed-admonitive-sentence-to-selected-output"
    });
    ...
    const CLASSICAL_NAHUATL_LESSON7_SOURCE_BOUNDARY_ROLE_RECORDS = Object.freeze({
      ...
      "ahco-cui": Object.freeze({
        sourceKind: "compound-vnc",
        stemRelationshipKind: "compound verbstem with verbal embed",
        embedMatrixAuthorized: true,
        embedStem: "ahco",
        matrixStem: "cui",
        canonicalStemVariant: "ahco-cui",
        boundaryRole: "verbal-embed-plus-vnc-matrix",
        lineStart: 4001,
        lineEnd: 4005,
        exactWitness: "tla-(ahco-cui) > tla-(ahco-uc) ~ tla-(ahco-c) = to lifts.th.",
        notes: "Canvas explicitly treats tla-(ahco-cui) as a compound verbstem whose Class A to Class B shift is governed by the matrix stem cui."
      }),
      "ce-cui": Object.freeze({
        sourceKind: "compound-vnc",
        stemRelationshipKind: "compound verbstem with verbal embed",
        embedMatrixAuthorized: true,
        embedStem: "ce",
        matrixStem: "cui",
        canonicalStemVariant: "ce-cui",
        boundaryRole: "verbal-embed-plus-vnc-matrix",
        lineStart: 4001,
        lineEnd: 4006,
        exactWitness: "(ce-cui) > (ce-uc) = pers, to be cold [The perfective stem is usually misspelled (ce-cu).]",
        notes: "Canvas treats ce-cui as a compound verbstem whose class behavior follows the matrix stem cui."
      }),
      ...
      "chico-mati": Object.freeze({
        sourceKind: "compound-vnc",
        stemRelationshipKind: "lexicalized tla-fusion compound verbstem",
        embedMatrixAuthorized: true,
        embedStem: "chico",
        matrixStem: "mati",
        ...
        boundaryRole: "lexical-exception-chico-tla-fusion-source",
        sourceEvidenceBoundary: "test-only-not-imported-by-lesson7-rule-inventory",
        notes: "This lexical target remains a semantic exception. Its later-lesson witness is kept out of the Lesson 7 production rule inventory."
      }),
      ...
      "zaca-mo-a": Object.freeze({
        sourceKind: "compound-vnc",
        stemRelationshipKind: "incorporated-object compound verbstem with polymorphemic matrix",
        embedMatrixAuthorized: true,
        embedStem: "zaca",
        matrixStem: "mo-ā",
        canonicalStemVariant: "zaca-mo-ā",
        boundaryRole: "incorporated-object-embed-plus-polymorphemic-vnc-matrix",
        lineStart: 8747,
        lineEnd: 8750,
        exactWitness: "(zaca-mo-ā) = to clear away weeds, to clear ground for cultivation > tē-(zaca-mo-l-huiā) = to clear ground for cultivation for s.o. [The source is a compound stem with (zaca)-tl-, “grass/weed,” as the embed and the unattested verbstem *tla-(mo-ā) as the matrix (for compound stems, see § 30.3.)]",
        notes: "Canvas fixes the top-level compound boundary after zaca. The internal mo-ā boundary remains inside the polymorphemic matrix; external tla- belongs to the matrix VNC source, not to the matrix stem."
      }),
    ...
    const CLASSICAL_NAHUATL_LESSON7_TLA_FUSION_RULES = Object.freeze([Object.freeze({
      ...
      lineEnd: 3285,
      exactWitness: "if an adverb precedes tla in the VNC, the tla is part of the verbstem.",
      rule: "When typed Source supplies an embed plus a matrix, tla fusion builds the derived stem inside that compound boundary as embed plus contextually realized tla plus matrix; documentary examples do not supply the source or target.",
    ...
    function makeClassicalNahuatlLesson76GuidelineAuthorityRecord({
      ...
      const normalizedDefault = normalizeClassicalNahuatlClassId(defaultClassId);
      const normalizedAllowed = Array.from(new Set((allowedClassIds.length ? allowedClassIds : [normalizedDefault, ...alternativeClassIds]).map(normalizeClassicalNahuatlClassId).filter(Boolean)));
      const normalizedOptions = Array.from(new Set((classOptions.length ? classOptions : normalizedAllowed).map(normalizeClassicalNahuatlClassId).filter(Boolean)));
      return Object.freeze({
        ...
        alternativeClassIds: Object.freeze(alternativeClassIds.map(normalizeClassicalNahuatlClassId).filter(Boolean)),
    ...
    const CLASSICAL_NAHUATL_LESSON7_GUIDELINE_AUTHORITY_STEMS = Object.freeze({
      ...
      mani: makeClassicalNahuatlLesson76GuidelineAuthorityRecord({
        sourceStem: "mani",
        citedForm: "(mani) > (man)",
        section: "11.4.8",
        lineStart: 4156,
        lineEnd: 4162,
        exactWitness: "(mani) > (man) = to extend, to be",
        relationRuleId: "cn-l11-1148-mani",
        defaultClassId: "B",
        allowedClassIds: ["B"],
        perfectiveStemsByClass: {
          B: "man"
        },
        conditionKind: "canvas-stated-lexical-class-membership",
        exceptionKind: "lesson-11-exact-lexical-membership"
      })
    });
    ...
    const CLASSICAL_NAHUATL_LESSON7_EXACT_PERFECTIVE_STEM_OVERRIDES = Object.freeze({
      ...
      "C|cui-tiā": Object.freeze({
        perfectiveStem: "cui-tih",
    ...
    function getClassicalNahuatlVerbstemRuntimeTarget() {
      return typeof targetObject !== "undefined" && targetObject || (typeof globalThis !== "undefined" ? globalThis : null);
    }
    function buildClassicalNahuatlProgressiveBoundaryFrameForVerbstem(stem = "") {
      const runtimeTarget = getClassicalNahuatlVerbstemRuntimeTarget();
      if (
        typeof runtimeTarget?.buildClassicalNahuatlProgressiveAssimilationFrame
          !== "function"
        || typeof runtimeTarget?.isClassicalNahuatlTranscriptionAnalysisFrame
          !== "function"
      ) {
        return null;
      }
      const frame =
        runtimeTarget.buildClassicalNahuatlProgressiveAssimilationFrame(stem);
      return runtimeTarget.isClassicalNahuatlTranscriptionAnalysisFrame(frame)
        ? frame
        : null;
    }
    ...
    function getClassicalNahuatlStructureRules() {
      return CLASSICAL_NAHUATL_LESSON7_STRUCTURE_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlCitationRules() {
      return CLASSICAL_NAHUATL_LESSON7_CITATION_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlClassRules() {
      return CLASSICAL_NAHUATL_LESSON7_CLASS_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlTypeOneCausativeRules() {
      return CLASSICAL_NAHUATL_LESSON24_TYPE_ONE_CAUSATIVE_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlClassBChangeRules() {
      return CLASSICAL_NAHUATL_LESSON7_CLASS_B_CHANGE_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlVariableClassRules() {
      return CLASSICAL_NAHUATL_LESSON7_VARIABLE_CLASS_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlGuidelineRules() {
      return CLASSICAL_NAHUATL_LESSON7_GUIDELINE_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlPredicateRules() {
      return CLASSICAL_NAHUATL_LESSON7_PREDICATE_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlAnalysisRules() {
      return CLASSICAL_NAHUATL_LESSON7_ANALYSIS_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlObjectRelationshipRules() {
      return CLASSICAL_NAHUATL_LESSON7_OBJECT_RELATIONSHIP_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlTlaFusionRules() {
      return CLASSICAL_NAHUATL_LESSON7_TLA_FUSION_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlVerbstemReceiptRules() {
      return CLASSICAL_NAHUATL_LESSON7_RECEIPT_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlOptionalIrregularPerfectiveRules() {
      return CLASSICAL_NAHUATL_LESSON11_OPTIONAL_IRREGULAR_PERFECTIVE_RULES.map(cloneClassicalNahuatlLesson7Rule);
    }
    function getClassicalNahuatlVerbstemRuleLogicRules() {
      return {
        structure: getClassicalNahuatlStructureRules(),
        citation: getClassicalNahuatlCitationRules(),
        classes: getClassicalNahuatlClassRules(),
        classBChanges: getClassicalNahuatlClassBChangeRules(),
        variableClass: getClassicalNahuatlVariableClassRules(),
        guidelines: getClassicalNahuatlGuidelineRules(),
        predicateFormation: getClassicalNahuatlPredicateRules(),
        analysis: getClassicalNahuatlAnalysisRules(),
        objectRelationship: getClassicalNahuatlObjectRelationshipRules(),
        tlaFusion: getClassicalNahuatlTlaFusionRules(),
        optionalIrregularPerfective: getClassicalNahuatlOptionalIrregularPerfectiveRules(),
        receipt: getClassicalNahuatlVerbstemReceiptRules()
      };
    }
    function normalizeClassicalNahuatlVerbstemToken(value = "") {
      return String(value == null ? "" : value).trim().toLowerCase();
    }
    function normalizeClassicalNahuatlVerbstem(value = "") {
    ...
    function getClassicalNahuatlExplicitTlaFusionDerivedStem(options = {}) {
      return normalizeClassicalNahuatlVerbstem(options.derivedStem || options.fusedStem || options.tlaFusionDerivedStem || "");
    }
    function removeClassicalNahuatlLesson7Marks(value = "") {
      return String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/gu, "").toLowerCase();
    }
    function getClassicalNahuatlStemForShape(value = "") {
      return removeClassicalNahuatlLesson7Marks(normalizeClassicalNahuatlVerbstem(value)).replace(/[^a-z-]/gu, "").replace(/-/gu, "");
    }
    function wrapClassicalNahuatlLesson7Stem(value = "") {
      return `(${normalizeClassicalNahuatlVerbstem(value) || "STEM"})`;
    }
    function getClassicalNahuatlStructureLookupKey(stem = "") {
      return removeClassicalNahuatlLesson7Marks(normalizeClassicalNahuatlVerbstem(stem)).replace(/[^a-z-]/gu, "").toLowerCase();
    }
    function getClassicalNahuatlTlaFusionCanvasBoundaryExample(stem = "") {
      const normalized = normalizeClassicalNahuatlVerbstem(stem);
      const boundaryRecord = getClassicalNahuatlSourceBoundaryRoleRecord(normalized);
      if (!boundaryRecord?.embedMatrixAuthorized) {
        return null;
      }
      const embedStem = normalizeClassicalNahuatlVerbstem(boundaryRecord.embedStem || "");
      const matrixStem = normalizeClassicalNahuatlVerbstem(boundaryRecord.matrixStem || "");
      const derivedStem = normalizeClassicalNahuatlVerbstem(boundaryRecord.tlaFusionDerivedStem || "");
      const adverbialBoundary = /adverb/iu.test(boundaryRecord.boundaryRole || "");
      return {
        kind: "classical-nahuatl-verbstem-tla-fusion-canvas-boundary-example",
        tagId: "cn-l7-tla-fusion",
        ...
        embedStem,
        matrixStem,
        fusedTlaSegment: adverbialBoundary ? getClassicalNahuatlFusedTlaSegment({
    ...
    function getClassicalNahuatlUnknownInternalMorphRecords(stem = "") {
      const key = getClassicalNahuatlStructureLookupKey(stem);
      return cloneClassicalNahuatlLesson7Record(CLASSICAL_NAHUATL_LESSON7_UNKNOWN_INTERNAL_MORPH_RECORDS[key] || []);
    }
    function getClassicalNahuatlContrastiveBoundaryRecord(stem = "") {
      const key = getClassicalNahuatlStructureLookupKey(stem);
      return cloneClassicalNahuatlLesson7Record(CLASSICAL_NAHUATL_LESSON7_CONTRASTIVE_BOUNDARY_STEMS[key] || null);
    }
    function buildClassicalNahuatlUserDefinedEmbedMatrixBoundaryRecord(stem = "", options = {}) {
      const key = getClassicalNahuatlStructureLookupKey(stem);
      const solidKey = key.replace(/-/gu, "");
      const embedStem = normalizeClassicalNahuatlVerbstem(getClassicalNahuatlExplicitTlaFusionEmbedStem(options));
      const matrixStem = normalizeClassicalNahuatlVerbstem(getClassicalNahuatlExplicitTlaFusionMatrixStem(options));
      const embedKey = getClassicalNahuatlStructureLookupKey(embedStem);
      const matrixKey = getClassicalNahuatlStructureLookupKey(matrixStem);
      const matrixSolidKey = matrixKey.replace(/-/gu, "");
      if (!embedKey || !matrixKey) {
        return null;
      }
      const matchingStaticRecordEntry = Object.entries(CLASSICAL_NAHUATL_LESSON7_SOURCE_BOUNDARY_ROLE_RECORDS).find(([_recordKey, record]) => record?.embedMatrixAuthorized === true && getClassicalNahuatlStructureLookupKey(record.embedStem || "") === embedKey && getClassicalNahuatlStructureLookupKey(record.matrixStem || "") === matrixKey);
    ...
    function getClassicalNahuatlSourceBoundaryRoleRecord(stem = "", options = {}) {
      const key = getClassicalNahuatlStructureLookupKey(stem);
      ...
      const matchedEntry = Object.entries(CLASSICAL_NAHUATL_LESSON7_SOURCE_BOUNDARY_ROLE_RECORDS).find(([recordKey]) => getClassicalNahuatlStructureLookupKey(recordKey) === key);
      if (matchedEntry) {
        return {
      ...
      const compactMatchedEntry = Object.entries(CLASSICAL_NAHUATL_LESSON7_SOURCE_BOUNDARY_ROLE_RECORDS).find(([recordKey, record]) => record?.embedMatrixAuthorized === true && getClassicalNahuatlStructureLookupKey(recordKey).replace(/-/gu, "") === compactKey);
      ...
      const userDefinedEmbedMatrixRecord = buildClassicalNahuatlUserDefinedEmbedMatrixBoundaryRecord(stem, options);
      if (userDefinedEmbedMatrixRecord) {
        return userDefinedEmbedMatrixRecord;
      }
      return null;
    }
    function getClassicalNahuatlNearbySourceBoundaryRecordKey(stem = "") {
      const key = getClassicalNahuatlStructureLookupKey(stem);
      const compactKey = key.replace(/-/gu, "");
      if (!compactKey) {
        return "";
      ...
      const nearbyEntry = Object.entries(CLASSICAL_NAHUATL_LESSON7_SOURCE_BOUNDARY_ROLE_RECORDS).find(([recordKey, record]) => {
        ...
        const compactRecordKey = getClassicalNahuatlStructureLookupKey(recordKey).replace(/-/gu, "");
    ...
    function buildClassicalNahuatlSourceBoundaryRoleFrame(stem = "", options = {}) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem || options.source || "");
      const structureRuleFrame = buildClassicalNahuatlVerbstemStructureRuleFrame(normalizedStem);
      const boundaryRecord = getClassicalNahuatlSourceBoundaryRoleRecord(normalizedStem);
      const internalMorphs = structureRuleFrame.internalMorphs || [];
      const hasInternalHyphen = internalMorphs.length > 1;
      const documentaryEmbedMatrixWitness = boundaryRecord?.embedMatrixAuthorized === true;
      const sourceKind = hasInternalHyphen ? "analyzed-verbstem" : "simple-verbstem";
      const structureActions = [...(structureRuleFrame.structureActions || [])];
      if (hasInternalHyphen) {
        structureActions.push(CLASSICAL_NAHUATL_LESSON7_STRUCTURE_ACTIONS.BLOCK_HYPHEN_AS_EMBED_MATRIX);
      }
      if (documentaryEmbedMatrixWitness) {
        structureActions.push(CLASSICAL_NAHUATL_LESSON7_STRUCTURE_ACTIONS.RECORD_DOCUMENTARY_EMBED_MATRIX);
      }
      return {
        kind: "classical-nahuatl-verbstem-source-boundary-role-frame",
        lesson: "Andrews Lesson 7",
        section: boundaryRecord?.lineStart ? "Documentary source-boundary example" : "7.1",
        ruleLogicRole: "source-boundary-documentary-evidence",
        ...
        stem: normalizedStem,
        sourceKind,
        stemRelationshipKind: "",
        boundaryRole: hasInternalHyphen ? "position-internal-morph-boundary" : "none",
        boundaryRoleKnown: false,
        boundaryRecordKey: "",
        inputBoundarySpelling: hasInternalHyphen ? "hyphenated" : "solid",
        solidBoundaryInferred: false,
        witnessUsePolicy: boundaryRecord?.witnessUsePolicy || "proof-anchor-not-whitelist",
        canonicalStemVariant: normalizedStem,
        internalMorphs,
        ...
        hyphenOnlyEmbedMatrixInferenceBlocked: hasInternalHyphen,
        hyphenOnlyCannotPopulateEmbedMatrix: hasInternalHyphen,
        embedMatrixAuthorized: false,
        compoundStemAuthorized: false,
        embedStem: "",
        matrixStem: "",
        tlaFusionSourceStemVariant: "",
        tlaFusionDerivedStem: "",
        tlaFusionDerivedTargetWitnessed: false,
        explicitSlashMaySeparateEmbedMatrix: true,
        sourceReadoutRole: hasInternalHyphen
          ? "one polymorphemic verbstem; no embed/matrix from hyphen"
          : "one whole verbstem",
        documentaryEvidenceOnly: true,
        documentaryEvidenceAuthorizesSource: false,
        documentaryEvidenceAuthorizesResult: false,
        documentaryBoundaryRoleKnown: Boolean(boundaryRecord),
        documentaryBoundaryRecordKey: boundaryRecord?.boundaryRecordKey || "",
        documentaryStemRelationshipKind: boundaryRecord?.stemRelationshipKind || "",
        documentaryBoundaryRole: boundaryRecord?.boundaryRole || "",
        documentaryEmbedMatrixWitness,
        documentaryEmbedStem: boundaryRecord?.embedStem || "",
        documentaryMatrixStem: boundaryRecord?.matrixStem || "",
        documentaryTlaFusionDerivedStem: boundaryRecord?.tlaFusionDerivedStem || "",
        documentaryInputBoundarySpelling: boundaryRecord?.inputBoundarySpelling || "",
        documentarySolidBoundaryInferred: boundaryRecord?.solidBoundaryInferred === true,
        ...
        ruleRefs: dedupeClassicalNahuatlLesson7RuleRefs([...(structureRuleFrame.ruleRefs || []), boundaryRecord ? {
          id: "cn-l7-source-boundary-role-documentary-witness",
    ...
    function normalizeClassicalNahuatlSourceSelectionKind(value = "") {
    ...
    function buildClassicalNahuatlFuenteSourceSelectionFrame(stem = "", options = {}) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem || options.source || "");
      const structureRuleFrame = buildClassicalNahuatlVerbstemStructureRuleFrame(normalizedStem);
      const documentaryBoundaryFrame = buildClassicalNahuatlSourceBoundaryRoleFrame(normalizedStem);
      const explicitEmbedStem = getClassicalNahuatlExplicitTlaFusionEmbedStem(options);
      const explicitMatrixStem = getClassicalNahuatlExplicitTlaFusionMatrixStem(options);
      const rawRequestedSelectionKind = String(options.sourceSelectionKind || options.fuenteSourceSelectionKind || options.sourceKind || options.userSourceSelectionKind || "").trim();
      const requestedSelectionKind = normalizeClassicalNahuatlSourceSelectionKind(rawRequestedSelectionKind);
      const explicitPartsPresent = Boolean(explicitEmbedStem || explicitMatrixStem);
      const explicitPartsComplete = Boolean(explicitEmbedStem && explicitMatrixStem);
      const normalizedStemKey = getClassicalNahuatlStructureLookupKey(normalizedStem);
      ...
      const selectedSourceKind = requestedSelectionKind
        || (explicitPartsPresent
          ? "embed-matrix"
          : structureRuleFrame.internalMorphs.length > 1
            ? "internal-morphemes"
            : "whole-stem");
      let userSelectionContradictionReason = "";
      if (!normalizedStem) {
        userSelectionContradictionReason = "typed-source-stem-required";
      } else if (rawRequestedSelectionKind && !requestedSelectionKind) {
        userSelectionContradictionReason = "unknown-typed-source-selection-kind";
      } else if (selectedSourceKind === "embed-matrix" && !explicitPartsComplete) {
        userSelectionContradictionReason = "typed-embed-matrix-requires-both-source-constituents";
      } else if (selectedSourceKind === "embed-matrix" && !typedPartsMatchStem) {
        userSelectionContradictionReason = "typed-embed-matrix-constituents-mismatch-source-stem";
      } else if (selectedSourceKind !== "embed-matrix" && explicitPartsPresent) {
        userSelectionContradictionReason = "typed-noncompound-source-cannot-carry-embed-matrix-constituents";
      } else if (selectedSourceKind === "internal-morphemes" && structureRuleFrame.internalMorphs.length < 2) {
        userSelectionContradictionReason = "typed-internal-morpheme-source-requires-explicit-boundary";
      }
      const userSelectionContradictsTypedSource = Boolean(userSelectionContradictionReason);
      const explicitUserSelection = Boolean(rawRequestedSelectionKind || explicitPartsPresent);
      const userSelectionTypedSourceAuthorized = Boolean(explicitUserSelection && !userSelectionContradictsTypedSource);
      const selectedBy = userSelectionContradictsTypedSource
        ? "none"
        : explicitUserSelection
          ? "typed-user-source"
          : "typed-source-constituents";
      const rejectedAuthorityFields = [
        options.sourceBoundaryRoleFrame ? "sourceBoundaryRoleFrame" : "",
        options.sourceBoundaryRecord ? "sourceBoundaryRecord" : "",
        options.canvasBoundaryExample ? "canvasBoundaryExample" : ""
      ].filter(Boolean);
      const sourceSelectionActions = Array.from(new Set([
        selectedSourceKind === "embed-matrix"
          ? CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS.SELECT_EMBED_MATRIX
          : selectedSourceKind === "internal-morphemes"
            ? CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS.SELECT_INTERNAL_MORPHEMES
            : CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS.SELECT_WHOLE_STEM,
        ...(userSelectionTypedSourceAuthorized
          ? [CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS.ACCEPT_USER_TYPED_SELECTION]
          : []),
        ...(userSelectionContradictsTypedSource
          ? [CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS.BLOCK_INVALID_TYPED_SELECTION]
          : []),
        CLASSICAL_NAHUATL_LESSON7_SOURCE_SELECTION_ACTIONS.CARRY_TO_SELECTED_OUTPUT
      ]));
      const sourceSelectionFrame = {
        kind: "classical-nahuatl-verbstem-fuente-source-selection-frame",
        lesson: "Andrews Lesson 7",
        section: "7.1 typed Source structure",
        ...
        selectedSourceKind,
        selectedBy,
        typedSourceSelection: true,
        typedSourceConstituentsAuthorizeSelection: true,
        canvasExamplesAuthorizeSelection: false,
        sourceBoundaryMetadataAuthorizesSelection: false,
        typedSourceSelectsWholeStem: selectedSourceKind === "whole-stem",
        typedSourceSelectsInternalMorphemes: selectedSourceKind === "internal-morphemes",
        typedSourceSelectsEmbedMatrix: selectedSourceKind === "embed-matrix",
        typedSourceSelectionAvailable: Boolean(normalizedStem),
        explicitUserSelection,
        userSelectionKind: selectedSourceKind,
        ...
        selectedEmbedStem: !userSelectionContradictsTypedSource && selectedSourceKind === "embed-matrix" ? explicitEmbedStem : "",
        selectedMatrixStem: !userSelectionContradictsTypedSource && selectedSourceKind === "embed-matrix" ? explicitMatrixStem : "",
        selectedInternalMorphs: !userSelectionContradictsTypedSource && selectedSourceKind === "internal-morphemes" ? structureRuleFrame.internalMorphs || [] : [],
        selectedWholeStem: !userSelectionContradictsTypedSource && selectedSourceKind === "whole-stem" ? normalizedStem : "",
        sourceReadoutRole: selectedSourceKind === "embed-matrix"
          ? `compound stem · embed: ${explicitEmbedStem || "required"} | matrix: ${explicitMatrixStem || "required"}`
          : selectedSourceKind === "internal-morphemes"
            ? "one polymorphemic verbstem; no embed/matrix from hyphen"
            : "one whole verbstem",
        rejectedAuthorityFields,
        callerBoundaryMetadataAccepted: false,
        documentaryBoundaryRecordKey: documentaryBoundaryFrame.documentaryBoundaryRecordKey || "",
        documentaryWitnessUsePolicy: documentaryBoundaryFrame.witnessUsePolicy || "proof-anchor-not-whitelist",
        sourceSelectionActions,
        ruleRefs: dedupeClassicalNahuatlLesson7RuleRefs(documentaryBoundaryFrame.ruleRefs || []),
        authorizationStatus: userSelectionContradictsTypedSource ? "blocked" : "authorized",
        grammarGenerationAllowed: false,
        surfaceGenerationAllowed: false,
      };
      classicalNahuatlLesson7IssuedSourceSelectionFrames.add(
        sourceSelectionFrame,
      );
      return sourceSelectionFrame;
    }
    function isClassicalNahuatlFuenteSourceSelectionFrame(
      frame = null,
    ) {
      return Boolean(
        frame
        && classicalNahuatlLesson7IssuedSourceSelectionFrames.has(frame)
        && frame.kind
          === "classical-nahuatl-verbstem-fuente-source-selection-frame"
      );
    }
    function getClassicalNahuatlClassBSilentCausativeCarrierRecord(stem = "") {
    ...
    function getClassicalNahuatlTraditionalSpellingWarningRecord(stem = "") {
      const key = getClassicalNahuatlStructureLookupKey(stem);
      const compactKey = key.replace(/-/gu, "");
    ...
    function getClassicalNahuatlShapeConsonantUnits(stem = "") {
      const beforeFinalVowel = getClassicalNahuatlStemForShape(stem).replace(/[aeio]$/u, "");
    ...
    function isClassicalNahuatlFinalVowelAfterConsonantCluster(stem = "") {
      const units = getClassicalNahuatlShapeConsonantUnits(stem);
      return units.length >= 2;
    }
    function isClassicalNahuatlMonosyllabicStem(stem = "") {
      const normalized = normalizeClassicalNahuatlVerbstem(stem).replace(/[^a-zāēīō-]/giu, "").replace(/-/gu, "");
    ...
    function makeClassicalNahuatlLesson7GeneralClassAuthorityRecord({
      ...
      return makeClassicalNahuatlLesson76GuidelineAuthorityRecord({
        sourceStem: normalizeClassicalNahuatlVerbstem(stem),
        section,
        lineStart,
        lineEnd,
        exactWitness,
    ...
    function getClassicalNahuatlGuidelineAuthorityRecord(stem = "", options = {}) {
      const normalized = normalizeClassicalNahuatlVerbstem(stem);
      const key = getClassicalNahuatlStructureLookupKey(stem);
      const compactKey = key.replace(/-/gu, "");
      const shapeStem = getClassicalNahuatlStemForShape(normalized);
      ...
      if (getClassicalNahuatlFinalVowelInfo(normalized).finalVowel && isClassicalNahuatlMonosyllabicStem(normalized)) {
      ...
      if (isClassicalNahuatlFinalVowelAfterConsonantCluster(normalized)) {
        return makeClassicalNahuatlLesson7GeneralClassAuthorityRecord({
          stem: normalized,
          section: "7.6.2",
          lineStart: 3001,
    ...
    function getClassicalNahuatlFinalVowelInfo(stem = "") {
      const normalized = normalizeClassicalNahuatlVerbstem(stem);
    ...
    function normalizeClassicalNahuatlValence(value = "") {
      const raw = normalizeClassicalNahuatlVerbstemToken(value || "intransitive").replace(/\s+/gu, "-");
      const aliases = {
        intr: "intransitive",
    ...
    function getClassicalNahuatlVerbstemInitialVowelKind(stem = "", options = {}) {
      const normalized = normalizeClassicalNahuatlVerbstem(stem);
      if (!/^[aeioāēīō]/iu.test(normalized)) {
        return "consonant-or-supportive-i";
      }
      const explicit = normalizeClassicalNahuatlVerbstemToken(options.initialVowelKind || "");
    ...
    function omitClassicalNahuatlLesson7InitialSupportiveI(stem = "") {
      return normalizeClassicalNahuatlVerbstem(stem).replace(/^[iī]/iu, "");
    }
    function buildClassicalNahuatlVerbstemInitialSupportiveIFrame(stem = "", options = {}) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const valence = normalizeClassicalNahuatlValence(options.valence || options.transitivity || options.objectKind || options.object || "");
      const initialVowelKind = getClassicalNahuatlVerbstemInitialVowelKind(normalizedStem, options);
      ...
      return {
        kind: "classical-nahuatl-verbstem-initial-supportive-i-boundary-frame",
        sourceAuthority: "Andrews transcription",
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlAnalysisRules().filter(rule => rule.tagId === "cn-l7-supportive-initial-i"),
    ...
    function getClassicalNahuatlProjectiveCitationRepresentative(options = {}) {
      const explicit = normalizeClassicalNahuatlVerbstemToken(options.citationRepresentative || options.projectiveCitationRepresentative || options.projectiveObjectRepresentative || "");
      ...
      const animacy = normalizeClassicalNahuatlVerbstemToken(options.objectAnimacy || options.citationObjectAnimacy || "");
      ...
      const objectKind = normalizeClassicalNahuatlVerbstemToken(options.objectKind || options.object || "");
      ...
      const objectPerson = normalizeClassicalNahuatlVerbstemSubject(options.objectPerson || "");
    ...
    function buildClassicalNahuatlCitationValenceFrame(stem = "", options = {}) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const valence = normalizeClassicalNahuatlValence(options.valence || options.transitivity || options.objectKind || options.object || "");
      const initialSupportiveIFrame = buildClassicalNahuatlVerbstemInitialSupportiveIFrame(normalizedStem, {
        ...options,
        valence
      });
      ...
      if (valence === "shuntline-reflexive") {
        ...
        const representative = getClassicalNahuatlProjectiveCitationRepresentative(options);
      ...
      return {
        kind: "classical-nahuatl-verbstem-citation-valence-frame",
    ...
    function normalizeClassicalNahuatlClassId(value = "") {
      const raw = normalizeClassicalNahuatlVerbstemToken(value).replace(/^class\s*/u, "").replace(/[^a-d12]/gu, "").toUpperCase();
      if (raw === "A1") {
        return "A-1";
      }
      if (raw === "A2") {
    ...
    function getClassicalNahuatlStemRelationship(stem = "") {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
    ...
    function getClassicalNahuatlOptionalIrregularPerfectiveRecord(stem = "") {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
    ...
    function normalizeClassicalNahuatlVerbstemSubject(value = "") {
      const raw = normalizeClassicalNahuatlVerbstemToken(value || "3sg").replace(/[^a-z0-9]/gu, "");
      const aliases = {
        "1": "1sg",
        "1s": "1sg",
        "1sg": "1sg",
    ...
    function isClassicalNahuatlOptionalIrregularEnvironment({
      subject = "3sg",
      mood = "indicative",
      tense = "present",
      aspect = ""
    } = {}) {
      const normalizedSubject = normalizeClassicalNahuatlVerbstemSubject(subject);
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
      const normalizedTense = normalizeClassicalNahuatlVerbstemTense(tense, normalizedMood);
    ...
    function inferClassicalNahuatlLesson7ClassProfile(stem = "", options = {}) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const shapeStem = getClassicalNahuatlStemForShape(normalizedStem);
      const stemRelationship = getClassicalNahuatlStemRelationship(normalizedStem);
      const optionalIrregularRecord = getClassicalNahuatlOptionalIrregularPerfectiveRecord(normalizedStem);
      const traditionalSpellingWarningRecord = getClassicalNahuatlTraditionalSpellingWarningRecord(normalizedStem);
      const valence = normalizeClassicalNahuatlValence(options.valence || options.transitivity || "");
      const guidelineAuthorityRecord = getClassicalNahuatlGuidelineAuthorityRecord(normalizedStem, {
        ...options,
        valence
      });
      const explicit = normalizeClassicalNahuatlClassId(options.verbClass || options.stemClass || options.classId || options.perfectiveClass || "");
      const finalInfo = getClassicalNahuatlFinalVowelInfo(normalizedStem);
      const classOptions = [];
    ...
    function stripClassicalNahuatlLesson7FinalVowel(stem = "") {
      return normalizeClassicalNahuatlVerbstem(stem).replace(/[aeioāēīō]$/iu, "");
    }
    ...
    function doesClassicalNahuatlLesson7FinalCRepresentS(stem = "") {
      const shape = removeClassicalNahuatlLesson7Marks(normalizeClassicalNahuatlVerbstem(stem));
    ...
    function getClassicalNahuatlClassCTruncatedStem(value = "") {
      return normalizeClassicalNahuatlVerbstem(value).replace(/-?[aā]$/iu, "");
    }
    function isClassicalNahuatlSingularSubject(subject = "") {
      return normalizeClassicalNahuatlVerbstemSubject(subject).endsWith("sg");
    }
    function buildClassicalNahuatlImperfectiveShapeEligibilityFrame({
      stem = "",
      classProfile = null,
      mood = "indicative",
      tense = "present",
      ...
      const normalizedTense = normalizeClassicalNahuatlVerbstemTense(tense, normalizedMood);
      const normalizedSubject = normalizeClassicalNahuatlVerbstemSubject(subject);
      const singularSubject = isClassicalNahuatlSingularSubject(normalizedSubject);
      ...
      if (profile.classId === "C" && aspect === "imperfective") {
        const truncatedShort = getClassicalNahuatlClassCTruncatedStem(normalizedStem);
        ...
        if (fullShort !== normalizedStem && isPresentIndicative) {
          ...
          const shortFutureDrinkCausative =
            isFutureIndicative && normalizedStem === "ī-tiā";
          const useShortTruncated =
            isNonpastOptative && singularSubject
            || shortFutureDrinkCausative;
          selectedStemVariant = shortFutureDrinkCausative
            ? "i-ti"
            : useShortTruncated
              ? truncatedShort
              : truncatedLong;
          ...
          selectedShape = shortFutureDrinkCausative
            ? "class-c-drink-causative-short-imperfective-before-future-z"
            : isFutureIndicative
              ? "class-c-truncated-long-imperfective-before-future-z"
              : useShortTruncated
                ? "class-c-truncated-short-imperfective-vocable-final"
                : "class-c-truncated-long-imperfective-before-plural-0-h";
          selectedShapeReason = shortFutureDrinkCausative
            ? "lexically-conditioned-drink-causative-short-vowels-before-future-z"
            : isFutureIndicative
              ? "class-c-future-truncated-long-stem-from-7.7-paradigm"
              : useShortTruncated
                ? "class-c-singular-nonpast-optative-short-vocable-final"
                : "class-c-plural-nonpast-optative-long-before-0-h";
          vowelLengthOperation = shortFutureDrinkCausative
            ? "select-short-drink-causative-vowels-before-future-z"
            : useShortTruncated
              ? "select-truncated-short-vowel-at-vocable-boundary"
              : "select-truncated-long-vowel-before-following-morph";
      ...
      return {
        kind: "classical-nahuatl-verbstem-imperfective-shape-eligibility-frame",
        lesson: "Andrews Lesson 7",
        section: "7.3.2",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlClassRules().filter(rule => rule.id === "cn-l7-73-imperfective-shape-counts" || rule.id === "cn-l7-73-cd-imperfective-shapes-tense-correlated" || rule.id === "cn-l7-73-class-c-truncated-nonpast-optative-singular" || rule.id === "cn-l7-73-a2-short-imperfective-admonitive" || rule.id === "cn-l7-73-class-c-truncated-silent-carrier"),
        stem: normalizedStem,
        ...
        sourceFinalVowel: getClassicalNahuatlFinalVowelInfo(normalizedStem).finalVowel,
        selectedFinalVowel: getClassicalNahuatlFinalVowelInfo(selectedStemVariant).finalVowel,
        selectedFinalVowelLong: getClassicalNahuatlFinalVowelInfo(selectedStemVariant).long,
    ...
    function getClassicalNahuatlExactPerfectiveStemOverride(stem = "", classId = "") {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const key = `${normalizeClassicalNahuatlClassId(classId) || classId}|${normalizedStem}`;
    ...
    function getClassicalNahuatlClassBPerfectiveCarrierFrame(stem = "", classProfile = null, perfective = null) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const profile = classProfile || inferClassicalNahuatlLesson7ClassProfile(normalizedStem);
      const perfectiveFrame = perfective || getClassicalNahuatlPerfectiveStem(normalizedStem, profile);
      ...
      const silentCarrierRecord = getClassicalNahuatlClassBSilentCausativeCarrierRecord(normalizedStem);
    ...
    function getClassicalNahuatlPerfectiveStem(stem = "", classProfile = null) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const profile = classProfile || inferClassicalNahuatlLesson7ClassProfile(normalizedStem);
      const exactOverride = getClassicalNahuatlExactPerfectiveStemOverride(normalizedStem, profile.classId);
      if (exactOverride) {
    ...
    function buildClassicalNahuatlOptionalIrregularPerfectiveFrame({
      stem = "",
      classProfile = null,
      perfective = null,
      subject = "3sg",
      ...
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const profile = classProfile || inferClassicalNahuatlLesson7ClassProfile(normalizedStem);
      const record = profile.optionalIrregularPerfectiveRecord || getClassicalNahuatlOptionalIrregularPerfectiveRecord(normalizedStem);
      const normalizedSubject = normalizeClassicalNahuatlVerbstemSubject(subject);
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
      const normalizedTense = normalizeClassicalNahuatlVerbstemTense(tense, normalizedMood);
      const regularPerfectiveStem = perfective?.perfectiveStem || record?.regularPerfectiveStem || "";
      const conditionPassed = Boolean(record && isClassicalNahuatlOptionalIrregularEnvironment({
        subject: normalizedSubject,
        mood: normalizedMood,
        tense: normalizedTense,
        aspect
      ...
      return {
        kind: "classical-nahuatl-irregular-vnc-optional-irregular-perfective-frame",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlOptionalIrregularPerfectiveRules(),
        stem: normalizedStem,
        classId: profile.classId,
        subject: normalizedSubject,
        mood: normalizedMood,
    ...
    function normalizeClassicalNahuatlVerbstemMood(value = "") {
      const normalized = normalizeClassicalNahuatlVerbstemToken(value || "indicative");
      if (normalized === "optative" || normalized === "optativo") return "optative";
      if (normalized === "admonitive" || normalized === "admonitivo") return "admonitive";
      return "indicative";
    }
    function normalizeClassicalNahuatlVerbstemTense(value = "", mood = "indicative") {
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
      const rawTense = normalizeClassicalNahuatlVerbstemToken(value || "");
      ...
      const aliases = {
        ...
        "distant-past-as-past": "distant-past",
        "general-past": "distant-past",
        "preterit-as-present": "preterit",
        nonpast: "nonpast",
        "non-past": "nonpast",
        past: "past",
        pasado: "past",
      };
      return aliases[rawTense]
        || (normalizedMood === "indicative" ? "present" : "nonpast");
    }
    function getClassicalNahuatlBorrowedIndicativeFormUse({
      mood = "indicative",
      tense = "present"
    } = {}) {
      const requestedMood = normalizeClassicalNahuatlVerbstemToken(mood || "indicative");
      const requestedTense = normalizeClassicalNahuatlVerbstemToken(tense || "");
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
      const normalizedTense = normalizeClassicalNahuatlVerbstemTense(tense, normalizedMood);
      const borrowed = normalizedMood === "optative" && ["future", "preterit"].includes(normalizedTense);
      const recognizedMood = ["", "indicative", "indicativo", "optative", "optativo", "admonitive", "admonitivo"]
        .includes(requestedMood);
      const recognizedTense = !requestedTense || [
        "present", "presente", "customary-present", "habitual",
        "imperfect", "imperfecto", "future", "futuro", "preterit",
        "preterito", "pretérito", "distant-past", "remoto",
        "distant-past-as-past", "general-past", "preterit-as-present",
        "nonpast", "non-past", "past", "pasado"
      ].includes(requestedTense);
      const moodCompatible = normalizedMood === "indicative"
        ? ["present", "customary-present", "imperfect", "future", "preterit", "distant-past"]
          .includes(normalizedTense)
        : normalizedMood === "optative"
          ? ["nonpast", "past", "future", "preterit"].includes(normalizedTense)
          : normalizedTense === "nonpast";
      const semanticSelectionAuthorized = recognizedMood && recognizedTense && moodCompatible;
      const safeFallbackTense = normalizedMood === "indicative" ? "present" : "nonpast";
      const formMood = borrowed ? "indicative" : normalizedMood;
      const formTense = semanticSelectionAuthorized || borrowed
        ? normalizedTense
        : safeFallbackTense;
      const paradigmFormDiffers = [
        "preterit-as-present",
      ...
      return {
        ...
        semanticTense: requestedTense === "preterit-as-present"
          ? "present"
          : ["distant-past-as-past", "general-past"].includes(requestedTense)
            ? "general-past"
            : normalizedTense,
        formMood,
        formTense,
        finiteFormDiffersFromSemanticSelection:
          borrowed || paradigmFormDiffers || !semanticSelectionAuthorized,
        semanticSelectionAuthorized,
        borrowedFormRule: borrowed ? normalizedTense === "future" ? "future-optative-borrows-future-indicative-form-by-use" : "preterit-optative-borrows-preterit-indicative-form-by-use" : "",
        antecessiveObligatory: borrowed && normalizedTense === "preterit"
      };
    }
    function getClassicalNahuatlAspectForTense({
      mood = "indicative",
      tense = "present"
    } = {}) {
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
    ...
    function normalizeClassicalNahuatlPredicateCarrier(value = "") {
      return String(value || "").trim().replace(/\s+/gu, "").replace(/Ø/gu, "0").replace(/\u2395/gu, "⎕");
    }
    function getClassicalNahuatlPredicateRuleId(classProfile = {}) {
    ...
    function getClassicalNahuatlPredicateTableSide({
      mood = "indicative"
    } = {}) {
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
    ...
    function buildClassicalNahuatlPredicateTableFrame({
      stem = "",
      classProfile = null,
      ...
      const predicateRules = getClassicalNahuatlPredicateRules();
      const ruleId = getClassicalNahuatlPredicateRuleId(profile);
      const rule = predicateRules.find(entry => entry.id === ruleId) || predicateRules.find(entry => entry.id === "cn-l7-77-core-tense-constituents");
      const requestedStemVariant = normalizeClassicalNahuatlVerbstem(options.predicateStemVariantOverride || options.requestedPredicateStemVariant || options.hostilePredicateStemVariant || "");
      ...
      const expectedCarrierKey = normalizeClassicalNahuatlPredicateCarrier(expectedPredicateCarrier);
      const requestedCarrierKey = normalizeClassicalNahuatlPredicateCarrier(requestedPredicateCarrier);
      ...
      return {
        kind: "classical-nahuatl-verbstem-predicate-table-frame",
        lesson: "Andrews Lesson 7",
        section: "7.7",
        ...
        tense: normalizedTense,
        subject: normalizedSubject,
        tableSide: getClassicalNahuatlPredicateTableSide({
          mood: normalizedMood
        }),
    ...
    function getClassicalNahuatlStemVariant({
      stem = "",
      classProfile = null,
      mood = "indicative",
      tense = "present",
      ...
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const profile = classProfile || inferClassicalNahuatlLesson7ClassProfile(normalizedStem);
      const perfective = getClassicalNahuatlPerfectiveStem(normalizedStem, profile);
      const classBPerfectiveCarrierFrame = getClassicalNahuatlClassBPerfectiveCarrierFrame(normalizedStem, profile, perfective);
      const selectedPerfectiveStem = classBPerfectiveCarrierFrame.classBUnderlyingPerfectiveStem || perfective.perfectiveStem;
      const aspect = getClassicalNahuatlAspectForTense({
        mood,
        tense
      });
      const imperfectiveShapeEligibilityFrame = buildClassicalNahuatlImperfectiveShapeEligibilityFrame({
        stem: normalizedStem,
      ...
      const optionalIrregularPerfectiveFrame = buildClassicalNahuatlOptionalIrregularPerfectiveFrame({
        stem: normalizedStem,
        classProfile: profile,
        perfective: {
          ...perfective,
      ...
      return {
        kind: "classical-nahuatl-verbstem-stem-variant-frame",
        stem: normalizedStem,
        stemVariant,
        aspect,
        classId: profile.classId,
    ...
    function buildClassicalNahuatlVerbstemStructureRuleFrame(stem = "") {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const internalMorphs = normalizedStem.split(/-/u).filter(Boolean);
      const isPolymorphemic = internalMorphs.length > 1;
      const unknownButAuthorizedMorphs = getClassicalNahuatlUnknownInternalMorphRecords(normalizedStem);
      const unknownMorphKeys = new Set(unknownButAuthorizedMorphs.map(record => getClassicalNahuatlStructureLookupKey(record.morph || "")).filter(Boolean));
      const contrastiveBoundaryRecord = getClassicalNahuatlContrastiveBoundaryRecord(normalizedStem);
      const internalMorphFrames = internalMorphs.map(morph => {
        const morphKey = getClassicalNahuatlStructureLookupKey(morph);
        const unknownRecord = unknownButAuthorizedMorphs.find(record => getClassicalNahuatlStructureLookupKey(record.morph || "") === morphKey) || null;
        return {
          morph,
      ...
      return {
        kind: "classical-nahuatl-verbstem-verbstem-structure-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.1",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlStructureRules(),
        stem: normalizedStem,
    ...
    function buildClassicalNahuatlCitationRuleFrame(stem = "", options = {}) {
      const citationValenceFrame = buildClassicalNahuatlCitationValenceFrame(stem, options);
      ...
      return {
        ...
        kind: "classical-nahuatl-verbstem-citation-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.2",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlCitationRules(),
    ...
    function buildClassicalNahuatlVerbstemClassRuleFrame(stem = "", options = {}) {
      const classProfile = inferClassicalNahuatlLesson7ClassProfile(stem, options);
      const perfective = getClassicalNahuatlPerfectiveStem(stem, classProfile);
      const classBPerfectiveCarrierFrame = getClassicalNahuatlClassBPerfectiveCarrierFrame(stem, classProfile, perfective);
      ...
      const ruleRefs = [...getClassicalNahuatlClassRules(), ...getClassicalNahuatlVariableClassRules(), ...getClassicalNahuatlGuidelineRules(), ...(classProfile.classId === "B" ? getClassicalNahuatlClassBChangeRules() : []), ...(classBPerfectiveCarrierFrame.classBSilentCausativeCarrierPresent ? getClassicalNahuatlTypeOneCausativeRules() : []), ...(hasOptionalIrregularPerfective ? getClassicalNahuatlOptionalIrregularPerfectiveRules() : [])];
      return {
        kind: "classical-nahuatl-verbstem-verbstem-class-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.3-7.6",
    ...
    function buildClassicalNahuatlPredicateFormationRuleFrame(stem = "", options = {}) {
      const requestedMood = options.mood || options.sentenceMood || "";
      const requestedTense = options.tense || options.tenseKey || "";
      const mood = normalizeClassicalNahuatlVerbstemMood(requestedMood);
      const tense = normalizeClassicalNahuatlVerbstemTense(requestedTense, mood);
      const borrowedIndicativeFormUse = getClassicalNahuatlBorrowedIndicativeFormUse({
        mood: requestedMood,
        tense: requestedTense
      });
      const formMood = borrowedIndicativeFormUse.formMood;
      const formTense = borrowedIndicativeFormUse.formTense;
      const subject = normalizeClassicalNahuatlVerbstemSubject(options.subject || options.subjectPerson || options.subj || "");
      const classProfile = options.classProfile || inferClassicalNahuatlLesson7ClassProfile(stem, options);
      const stemVariantFrame = getClassicalNahuatlStemVariant({
        stem,
        classProfile,
        mood: formMood,
        tense: formTense,
      ...
      const tenseBuilder = getClassicalNahuatlVerbstemRuntimeTarget()?.getClassicalNahuatlFiniteMoodTenseFrame;
      const tenseFrame = typeof tenseBuilder === "function" ? tenseBuilder({
        mood: formMood,
        tense: formTense,
        verbClass: classProfile.classId
      }) : null;
      const predicateTableFrame = buildClassicalNahuatlPredicateTableFrame({
        stem,
        classProfile,
        stemVariantFrame,
        tenseFrame,
      ...
      return {
        kind: "classical-nahuatl-verbstem-predicate-formation-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.7",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: [...getClassicalNahuatlPredicateRules(), ...(stemVariantFrame.imperfectiveShapeEligibilityFrame?.ruleRefs || []), ...(predicateTableFrame.ruleRefs || []), ...(stemVariantFrame.optionalIrregularPerfectiveFrame?.regularPerfectiveRemainsLegal ? getClassicalNahuatlOptionalIrregularPerfectiveRules() : [])],
        consumesPriorLesson: "Andrews Lesson 5",
        ...
        finiteFormDiffersFromSemanticSelection:
          borrowedIndicativeFormUse.finiteFormDiffersFromSemanticSelection,
        semanticSelectionAuthorized:
          borrowedIndicativeFormUse.semanticSelectionAuthorized,
        formMood,
        formTense,
    ...
    function buildClassicalNahuatlAnalysisRuleFrame(stem = "", options = {}) {
      const initialVowelKind = getClassicalNahuatlVerbstemInitialVowelKind(stem, options);
      return {
        kind: "classical-nahuatl-verbstem-analysis-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.8",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlAnalysisRules(),
        stem: normalizeClassicalNahuatlVerbstem(stem),
        requiredDivision: "subject-plus-predicate",
    ...
    function normalizeClassicalNahuatlObjectRelationshipToken(value = "") {
      return removeClassicalNahuatlLesson7Marks(value).replace(/ø/gu, "0").replace(/[^a-z0-9āēīō⎕-]+/gu, "-").replace(/-+/gu, "-").replace(/^-|-$/gu, "");
    }
    function normalizeClassicalNahuatlIndefiniteObject(value = "") {
      const raw = normalizeClassicalNahuatlObjectRelationshipToken(value);
    ...
    function normalizeClassicalNahuatlObjectRelationshipKind(value = "") {
      const raw = normalizeClassicalNahuatlObjectRelationshipToken(value);
      const aliases = {
    ...
    function getClassicalNahuatlObjectRelationshipGroup(relationshipKind = "") {
    ...
    function getClassicalNahuatlObjectRelationshipWitness(relationshipGroup = "") {
      const rules = getClassicalNahuatlObjectRelationshipRules();
    ...
    function getClassicalNahuatlSpecificProjectiveRelationshipKind({
      ...
      const person = normalizeClassicalNahuatlObjectRelationshipToken(objectPerson);
      const humanness = normalizeClassicalNahuatlObjectRelationshipToken(objectHumanness);
    ...
    function getClassicalNahuatlSelectedObjectRelationship({
      valence = "",
      objectFrame = null,
      options = {}
    } = {}) {
      const normalizedValence = normalizeClassicalNahuatlValence(valence);
      ...
      if (normalizedValence === "projective-human" || objectKind === "nonspecific-human") {
        ...
        relationshipKind = getClassicalNahuatlSpecificProjectiveRelationshipKind({
          objectPerson,
          objectHumanness
        });
      }
      const relationshipGroup = getClassicalNahuatlObjectRelationshipGroup(relationshipKind);
    ...
    function buildClassicalNahuatlObjectRelationshipRuleFrame(stem = "", options = {}, priorVncFrame = null) {
      ...
      const requestedRelationshipKind = normalizeClassicalNahuatlObjectRelationshipKind(options.requestedObjectRelationshipKind || options.objectRelationshipKind || options.hostileObjectRelationshipKind || "");
      const requestedIndefiniteObject = normalizeClassicalNahuatlIndefiniteObject(options.requestedIndefiniteObject || options.indefiniteObject || options.hostileIndefiniteObject || "");
      const requestedRelationshipGroup = getClassicalNahuatlObjectRelationshipGroup(requestedRelationshipKind);
    ...
    function normalizeClassicalNahuatlTlaFusionAnalysisKind(value = "") {
      const raw = normalizeClassicalNahuatlObjectRelationshipToken(value);
      const aliases = {
    ...
    function normalizeClassicalNahuatlAdverbPosition(value = "") {
      const raw = normalizeClassicalNahuatlObjectRelationshipToken(value);
      const aliases = {
        before: "before-tla",
    ...
    function getClassicalNahuatlFusedTlaSegment({
      incorporatedAdverb = ""
    } = {}) {
      return removeClassicalNahuatlLesson7Marks(incorporatedAdverb).endsWith("l") ? "la" : "tla";
    }
    function getClassicalNahuatlExplicitTlaFusionEmbedStem(options = {}) {
      return normalizeClassicalNahuatlVerbstem(options.embedStem || options.embeddedStem || options.sourceEmbedStem || options.sourceEmbed || options.tlaFusionEmbedStem || "");
    }
    function getClassicalNahuatlExplicitTlaFusionMatrixStem(options = {}) {
      return normalizeClassicalNahuatlVerbstem(options.matrixStem || options.matrixStemVariant || options.sourceMatrixStem || options.sourceMatrix || options.tlaFusionMatrixStem || "");
    }
    function buildClassicalNahuatlConstructiveTlaFusionTargetFrame(stem = "", options = {}) {
      const stemVariant = normalizeClassicalNahuatlVerbstem(options.stemVariant || stem);
      const sourceSelectionFrame = buildClassicalNahuatlFuenteSourceSelectionFrame(stem, options);
      const embedStem = sourceSelectionFrame.authorizationStatus === "authorized"
        && sourceSelectionFrame.selectedSourceKind === "embed-matrix"
        ? normalizeClassicalNahuatlVerbstem(sourceSelectionFrame.selectedEmbedStem)
        : "";
      const matrixStem = sourceSelectionFrame.authorizationStatus === "authorized"
        && sourceSelectionFrame.selectedSourceKind === "embed-matrix"
      ...
      return {
        ...
        exactWitnessOverridesConstructedShape: false,
        typedSourceSelectionFrame: sourceSelectionFrame,
        documentaryExamplesAuthorizeTarget: false,
        callerDerivedStemAuthorizesTarget: false,
        buildLogic: embedStem ? "embed + fused-tla + matrix" : "tla + matrix"
      };
    }
    function buildClassicalNahuatlTlaFusionDerivedStem(stem = "", options = {}) {
      return buildClassicalNahuatlConstructiveTlaFusionTargetFrame(stem, options).derivedStem;
    }
    function buildClassicalNahuatlTlaFusionSourceStem(stem = "", options = {}) {
      const stemVariant = normalizeClassicalNahuatlVerbstem(options.stemVariant || stem);
      const sourceSelectionFrame = buildClassicalNahuatlFuenteSourceSelectionFrame(stem, options);
      if (
        sourceSelectionFrame.authorizationStatus === "authorized"
        && sourceSelectionFrame.selectedSourceKind === "embed-matrix"
      ) {
        return [
          sourceSelectionFrame.selectedEmbedStem,
          sourceSelectionFrame.selectedMatrixStem
        ].filter(Boolean).join("-");
      }
      return stemVariant;
    }
    function buildClassicalNahuatlTlaFusionRuleFrame(stem = "", options = {}) {
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const stemVariant = normalizeClassicalNahuatlVerbstem(options.stemVariant || normalizedStem);
      const typedSourceSelectionFrame = buildClassicalNahuatlFuenteSourceSelectionFrame(normalizedStem, options);
      const documentaryBoundaryRecord = getClassicalNahuatlSourceBoundaryRoleRecord(normalizedStem);
      const explicitFusionRequested = options.fused === true || options.tlaFusion === true;
      const explicitEmbedStem = getClassicalNahuatlExplicitTlaFusionEmbedStem(options);
      const explicitMatrixStem = getClassicalNahuatlExplicitTlaFusionMatrixStem(options);
      const explicitIncorporatedAdverb = normalizeClassicalNahuatlVerbstem(options.incorporatedAdverb || options.adverb || "");
      const explicitAdverbPosition = normalizeClassicalNahuatlAdverbPosition(options.adverbPosition || "");
      const typedSourceHasEmbedMatrix = Boolean(
        typedSourceSelectionFrame.authorizationStatus === "authorized"
        && typedSourceSelectionFrame.selectedSourceKind === "embed-matrix"
        && typedSourceSelectionFrame.selectedEmbedStem
        && typedSourceSelectionFrame.selectedMatrixStem
      );
      const typedEmbedStem = typedSourceHasEmbedMatrix
        ? normalizeClassicalNahuatlVerbstem(typedSourceSelectionFrame.selectedEmbedStem)
        : "";
      const typedMatrixStem = typedSourceHasEmbedMatrix
        ? normalizeClassicalNahuatlVerbstem(typedSourceSelectionFrame.selectedMatrixStem)
        : stemVariant;
      const incorporatedAdverb = explicitIncorporatedAdverb;
      const adverbPosition = explicitAdverbPosition;
      const adverbPrecedesTla = adverbPosition === "before-tla";
      const adverbFollowsTla = adverbPosition === "after-tla";
      const typedAdverbMatchesEmbed = Boolean(
        !typedSourceHasEmbedMatrix
        || (
          incorporatedAdverb
          && getClassicalNahuatlStructureLookupKey(incorporatedAdverb)
            === getClassicalNahuatlStructureLookupKey(typedEmbedStem)
        )
      );
      let typedSourceContextBlockReason = typedSourceSelectionFrame.authorizationStatus === "authorized"
        ? ""
        : typedSourceSelectionFrame.userSelectionContradictionReason || "typed-source-selection-blocked";
      if (!typedSourceContextBlockReason && explicitFusionRequested && typedSourceHasEmbedMatrix && !incorporatedAdverb) {
        typedSourceContextBlockReason = "typed-embed-matrix-tla-fusion-requires-incorporated-adverb";
      } else if (!typedSourceContextBlockReason && explicitFusionRequested && typedSourceHasEmbedMatrix && !typedAdverbMatchesEmbed) {
        typedSourceContextBlockReason = "typed-incorporated-adverb-mismatches-source-embed";
      } else if (!typedSourceContextBlockReason && explicitFusionRequested && typedSourceHasEmbedMatrix && !adverbPrecedesTla) {
        typedSourceContextBlockReason = "typed-embed-matrix-tla-fusion-requires-before-tla-context";
      }
      const fused = Boolean(explicitFusionRequested && !adverbFollowsTla && !typedSourceContextBlockReason);
      const requestedSourceValence = normalizeClassicalNahuatlValence(options.requestedSourceValence || options.requestedValence || options.sourceValence || options.valence || options.transitivity || options.objectKind || options.object || "");
      const valence = explicitFusionRequested ? "projective-nonhuman" : normalizeClassicalNahuatlValence(options.valence || options.transitivity || options.objectKind || options.object || "");
      ...
      const matrixStemVariant = normalizeClassicalNahuatlVerbstem(typedMatrixStem || stemVariant);
      const constructiveTlaFusionTargetFrame = buildClassicalNahuatlConstructiveTlaFusionTargetFrame(normalizedStem, {
        ...options,
        stemVariant: sourceStemVariant || stemVariant,
        embedStem: typedEmbedStem,
        matrixStem: matrixStemVariant
      });
      const explicitDerivedStem = getClassicalNahuatlExplicitTlaFusionDerivedStem(options);
      const expectedDerivedStem = normalizeClassicalNahuatlVerbstem(constructiveTlaFusionTargetFrame.derivedStem || "");
      ...
      const requestedAnalysisKind = normalizeClassicalNahuatlTlaFusionAnalysisKind(options.requestedTlaFusionAnalysis || options.tlaFusionAnalysis || options.hostileTlaFusionAnalysis || (typeof options.requestedTlaFusion === "boolean" ? options.requestedTlaFusion ? "fused" : "unfused" : "") || (typeof options.hostileTlaFusion === "boolean" ? options.hostileTlaFusion ? "fused" : "unfused" : ""));
      const requestedAnalysisContradicts = Boolean(requestedAnalysisKind && selectedAnalysisKind !== "not-applicable" && requestedAnalysisKind !== selectedAnalysisKind);
      const contradictionBlocked = Boolean(typedSourceContextBlockReason || requestedAnalysisContradicts);
      ...
      const sourceBoundaryProofAnchor = documentaryBoundaryRecord ? {
        id: "cn-l7-source-boundary-proof-anchor",
        tagId: "cn-l7-source-boundary-role",
        section: documentaryBoundaryRecord.sourceKind || "",
        lineStart: documentaryBoundaryRecord.lineStart || 0,
        lineEnd: documentaryBoundaryRecord.lineEnd || 0,
        exactWitness: documentaryBoundaryRecord.exactWitness || "",
        rule: documentaryBoundaryRecord.notes || "",
        authorityRole: "documentary-only"
      } : null;
      const tlaFusionProofAnchors = dedupeClassicalNahuatlLesson7RuleRefs([...getClassicalNahuatlTlaFusionRules(), sourceBoundaryProofAnchor].filter(Boolean));
      const ruleVariables = {
        ...
        typedSourceSelectionKind: typedSourceSelectionFrame.selectedSourceKind || "",
        typedSourceSelectionSource: typedSourceSelectionFrame.selectedBy || "",
        typedSourceHasEmbedMatrix,
        typedSourceContextComplete: !typedSourceContextBlockReason,
        documentaryBoundaryRecordKey: documentaryBoundaryRecord?.boundaryRecordKey || "",
        documentaryBoundaryAuthorizesSource: false,
        documentaryBoundaryAuthorizesTarget: false
      };
      const ruleInputs = {
        stem: normalizedStem,
        stemVariant,
        ...
        adverbPosition,
        typedSourceSelectionKind: typedSourceSelectionFrame.selectedSourceKind || "",
        typedSourceEmbedStem: typedEmbedStem,
        typedSourceMatrixStem: typedMatrixStem,
        callerDerivedStemRejected: Boolean(explicitDerivedStem),
        sourceFormula
      };
      const ruleOutputs = {
      ...
      return {
        kind: "classical-nahuatl-verbstem-tla-fusion-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.10",
        ...
        explicitFusionRequested,
        tlaEligible,
        typedSourceSelectionFrame,
        typedSourceHasEmbedMatrix,
        typedEmbedStem,
        typedMatrixStem,
        typedSourceContextBlockReason,
        documentaryBoundaryRecordKey: documentaryBoundaryRecord?.boundaryRecordKey || "",
        documentaryExamplesAuthorizeSource: false,
        documentaryExamplesAuthorizeTarget: false,
        sourceBoundaryMetadataAuthorizesSource: false,
        sourceBoundaryMetadataAuthorizesTarget: false,
        ...
        adverbBoundaryDecision: fused
          ? constructiveTlaFusionTargetFrame.buildKind === "embed-matrix-plus-tla-fusion"
            ? "typed-embed-matrix-context-builds-tla-fusion"
            : "typed-matrix-builds-tla-fusion"
          : adverbFollowsTla
            ? "typed-after-tla-context-preserves-transitive-tla-object"
            : typedSourceContextBlockReason,
        sourceFormula,
        targetFormula,
        selectedAnalysis: fused ? targetFormula : tlaEligible ? sourceFormula : "",
        ...
        tlaFusionContradictionReason: typedSourceContextBlockReason || (requestedAnalysisContradicts ? "requested-tla-fusion-analysis-contradicts-typed-operation" : ""),
        hostileRejectedTlaFusionAnalyses: requestedAnalysisContradicts ? [requestedAnalysisKind].filter(Boolean) : [],
        hostileRejectedDerivedStems: [explicitDerivedStem].filter(Boolean),
        rejectedAuthorityFields: [
          explicitDerivedStem ? "derivedStem" : "",
          options.sourceBoundaryRecord ? "sourceBoundaryRecord" : "",
          options.sourceBoundaryRoleFrame ? "sourceBoundaryRoleFrame" : "",
          options.canvasBoundaryExample ? "canvasBoundaryExample" : "",
          options.fusedTlaSegment || options.tlaFusionSegment || options.tlaInsideStem ? "fusedTlaSegment" : ""
        ].filter(Boolean),
    ...
    function getClassicalNahuatlExpandedVncRules() {
      return [{
        ...
        tagId: "cn-l8-811-optional-on-itta-contraction",
        section: "8.1.1",
        transcriptionLineStart: 3346,
        transcriptionLineEnd: 3349,
        exactWitness: "nocon-, tocon-, and xocon- have the option of deleting the /n/ of on. The resulting noco-, toco-, and xoco- entail the dismissal of the supportive [i] of the verbstem",
        ruleAction: CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.APPLY_OPTIONAL_ON_ITTA_CONTRACTION
      }, {
    ...
    function normalizeClassicalNahuatlDirectionalPrefix(value = "") {
    ...
    function normalizeClassicalNahuatlOutsidePrefix(value = "") {
      const raw = String(value || "").trim();
      const normalized = raw.normalize("NFD").replace(/[\u0300-\u036f]/gu, "").toLowerCase().replace(/[#\s_-]/gu, "");
      if (normalized === "o") {
        return "ō#";
    ...
    function getClassicalNahuatlOutsidePrefixes(options = {}) {
      ...
      return Array.from(new Set(requested.map(normalizeClassicalNahuatlOutsidePrefix).filter(Boolean)));
    }
    function getClassicalNahuatlHostileFormulaSlots(options = {}) {
    ...
    function getClassicalNahuatlDirectionalPlacement({
      valence = "",
    ...
    function isClassicalNahuatlThirdSingularProjectiveOnObjectSlot(objectSlot = "") {
      return ["c-0", "qu-0", "qui-0"].includes(String(objectSlot || "").trim());
    }
    function normalizeClassicalNahuatlDirectionalIttaContractionSelection(value = "") {
      if (value === true) {
        return "rare-contracted";
      }
      if (value === false || value == null || value === "") {
        return value === false ? "ordinary" : "";
      }
      const normalized = String(value).trim().toLowerCase().replace(/[\s_]/gu, "-");
      if (["ordinary", "full", "retain-on", "uncontracted"].includes(normalized)) {
        return "ordinary";
      }
      if (["rare", "contracted", "rare-contracted", "delete-n", "on-itta-contraction"].includes(normalized)) {
        return "rare-contracted";
      }
      return "invalid";
    }
    function buildClassicalNahuatlDirectionalIttaContractionFrame({
      stem = "",
      directionalPrefix = "",
      objectRelationshipRuleFrame = null,
      selection = ""
      ...
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const normalizedStemKey = removeClassicalNahuatlLesson7Marks(normalizedStem).replace(/[^a-z]/gu, "");
      const normalizedDirectional = normalizeClassicalNahuatlDirectionalPrefix(directionalPrefix);
      const normalizedSelection = normalizeClassicalNahuatlDirectionalIttaContractionSelection(selection);
      const requested = Boolean(normalizedSelection);
      const rareRequested = normalizedSelection === "rare-contracted";
      const sourceStemAuthorized = normalizedStemKey === "itta";
      const selectedObjectSlot = String(objectRelationshipRuleFrame?.selectedObjectSlot || "").trim();
      const objectContextAuthorized = isClassicalNahuatlThirdSingularProjectiveOnObjectSlot(selectedObjectSlot);
      const contextAuthorized = normalizedDirectional === "on" && sourceStemAuthorized && objectContextAuthorized;
      const blocked = normalizedSelection === "invalid" || rareRequested && !contextAuthorized;
      const applies = rareRequested && contextAuthorized;
      return Object.freeze({
        kind: "classical-nahuatl-directional-itta-contraction-frame",
        operationId: "optional-on-itta-directional-contraction",
        sourceAuthority: "Andrews transcription",
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        ruleRefs: getClassicalNahuatlExpandedVncRules().filter(rule => rule.tagId === "cn-l8-811-optional-on-itta-contraction"),
        requested,
        ...
        sourceDirectionalPrefix: normalizedDirectional,
        sourceObjectSlot: selectedObjectSlot,
        selectedStem: applies ? "tt-a" : normalizedStem,
        selectedDirectionalPrefix: applies ? "o" : normalizedDirectional,
        deletedDirectionalSegment: applies ? "n" : "",
        dismissedSupportiveStemSegment: applies ? "i" : "",
        supportiveInitialIAction: applies ? "dismiss" : "retain",
        contractionApplies: applies,
        formulaStringAuthority: false,
        surfaceStringAuthority: false,
        callerSuppliedFormulaSurfaceIgnored: true,
        authorizationStatus: blocked ? "blocked" : requested ? "authorized" : "not-requested",
        blockReason: normalizedSelection === "invalid" ? "unknown-on-itta-contraction-selection" : rareRequested && normalizedDirectional !== "on" ? "on-itta-contraction-requires-on-directional" : rareRequested && !sourceStemAuthorized ? "on-itta-contraction-requires-itta-stem" : rareRequested && !objectContextAuthorized ? "on-itta-contraction-requires-third-singular-specific-object" : "",
        operationActions: applies ? [CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.APPLY_OPTIONAL_ON_ITTA_CONTRACTION] : []
      });
    }
    function getClassicalNahuatlObjectMorphIdentityFrame(priorVncFrame = null) {
    ...
    function getClassicalNahuatlFirstSound(value = "") {
      const normalized = normalizeClassicalNahuatlVerbstem(value).normalize("NFD").replace(/[\u0300-\u036f]/gu, "").toLowerCase();
      const match = normalized.match(/[a-z]/u);
      return match ? match[0] : "";
    }
    function getClassicalNahuatlLastSound(value = "") {
      const normalized = normalizeClassicalNahuatlVerbstem(value).normalize("NFD").replace(/[\u0300-\u036f]/gu, "").toLowerCase();
      const matches = normalized.match(/[a-z]/gu);
      return matches?.length ? matches[matches.length - 1] : "";
    }
    function isClassicalNahuatlVncBoundaryVowelSound(sound = "") {
      return /^[aeio]$/u.test(String(sound || "").toLowerCase());
    }
    function getClassicalNahuatlPers1Carrier(formula = "") {
    ...
    function getClassicalNahuatlPers1CarrierFamily(carrier = "") {
    ...
    function getClassicalNahuatlNextCarrierAfterPers1(formula = "") {
    ...
    function applyClassicalNahuatlLesson81Pers1FinalSupport({
      ...
      const carrier = getClassicalNahuatlPers1Carrier(sourceFormula);
      const family = getClassicalNahuatlPers1CarrierFamily(carrier);
      if (!family) {
        return {
      ...
      const nextCarrier = getClassicalNahuatlNextCarrierAfterPers1(sourceFormula);
      const nextSound = getClassicalNahuatlFirstSound(nextCarrier);
      const nextNeedsSupport = Boolean(nextSound && !isClassicalNahuatlVncBoundaryVowelSound(nextSound));
    ...
    function getClassicalNahuatlSpecificObjectSlotFromFormula(formula = "") {
      const match = String(formula || "").match(/^#[^+()]+\+([^+()]+)(?:\+(?:on|huāl))?\(/u);
      return match ? match[1] : "";
    }
    function getClassicalNahuatlFormulaStemInitialAfterObject(formula = "") {
      const match = String(formula || "").match(/^#[^+()]+\+[^+()]+(?:\+(?:on|huāl))?\(([^)]*)\)/u);
      return match ? getClassicalNahuatlFirstSound(match[1]) : "";
    }
    function getClassicalNahuatlSoundBeforeObject(formula = "") {
      const carrier = getClassicalNahuatlPers1Carrier(formula);
      if (!carrier) {
        return "";
      }
      return getClassicalNahuatlFirstSound(carrier.slice(-1));
    }
    function applyClassicalNahuatlLesson81ThirdPersonObjectFinalRealization({
      ...
      const sourceFormula = String(formula || "");
      const sourceObjectSlot = String(selectedObjectSlot || getClassicalNahuatlSpecificObjectSlotFromFormula(sourceFormula) || "");
      ...
      if (String(objectMorphIdentityFrame.va2 || "") === "im" || /^qu-i[mn]$/u.test(sourceObjectSlot)) {
        ...
        return {
          formula: realized,
          selectedObjectSlotBeforeFinalBoundary: sourceObjectSlot,
          finalObjectSlot: getClassicalNahuatlSpecificObjectSlotFromFormula(realized) || sourceObjectSlot,
      ...
      if (/\+(?:qu|qui)-0\+on\(/u.test(realized)) {
        ...
        const leftSound = getClassicalNahuatlSoundBeforeObject(realized);
        const leftVowel = isClassicalNahuatlVncBoundaryVowelSound(leftSound);
        ...
        const leftSound = getClassicalNahuatlSoundBeforeObject(realized);
        const rightSound = getClassicalNahuatlFormulaStemInitialAfterObject(realized);
        const finalVa1 = isClassicalNahuatlVncBoundaryVowelSound(leftSound) || isClassicalNahuatlVncBoundaryVowelSound(rightSound) ? rightSound === "e" || rightSound === "i" ? "qu" : "c" : "qui";
    ...
    function getClassicalNahuatlNumberDyadFrame(priorVncFrame = null) {
    ...
    function parseClassicalNahuatlLesson81FinalPredicateNumber(formula = "") {
      ...
      return {
        stem: normalizeClassicalNahuatlVerbstem(match[1] || ""),
        tns: String(match[2] || ""),
        num1: String(match[3] || ""),
        num2: String(match[4] || "")
      };
    }
    function isClassicalNahuatlSilentCarrier(value = "") {
      const normalized = String(value || "").trim();
      return !normalized || normalized === "0" || normalized === "Ø" || normalized === CLASSICAL_NAHUATL_LESSON7_SQUARE_ZERO;
    }
    function getClassicalNahuatlNum1LeftSound(parsed = {}) {
      if (!isClassicalNahuatlSilentCarrier(parsed.tns || "")) {
        return {
          source: "tns",
          carrier: parsed.tns || "",
          sound: getClassicalNahuatlLastSound(parsed.tns || "")
        };
      }
      return {
        source: "stem",
        carrier: parsed.stem || "",
        sound: getClassicalNahuatlLastSound(parsed.stem || "")
      };
    }
    function isClassicalNahuatlNum1KConnectorContext(parsed = {}, numberDyadFrame = null) {
    ...
    function applyClassicalNahuatlLesson81Num1FinalRealization({
      ...
      const numberDyadFrame = getClassicalNahuatlNumberDyadFrame(priorVncFrame);
      const kConnectorContext = Boolean(parsed.num1 && parsed.num2 && isClassicalNahuatlNum1KConnectorContext(parsed, numberDyadFrame));
      const leftCarrier = getClassicalNahuatlNum1LeftSound(parsed);
      ...
      if (parsed.num2 === "eh") {
        ...
        if (isClassicalNahuatlVncBoundaryVowelSound(leftCarrier.sound)) {
          finalNum1 = "c";
    ...
    function buildClassicalNahuatlLegacyStringBoundaryRealizationFrame({
      formula = "",
      ...
      const objectMorphIdentityFrame = getClassicalNahuatlObjectMorphIdentityFrame(priorVncFrame);
      ...
      const finalSubjectCarrier = getClassicalNahuatlPers1Carrier(withDirectional) || subjectRealization.finalSubjectCarrier;
    ...
    function buildClassicalNahuatlFinalBoundaryRealizationFrame({
      vncSlotFrame = null,
      expandedVncBoundaryFrame = null,
      objectRelationshipRuleFrame = null,
      priorVncFrame = null
    ...
    function applyClassicalNahuatlLesson81DirectionalToFormula(formula = "", expandedVncBoundaryFrame = null) {
      return buildClassicalNahuatlFinalBoundaryRealizationFrame({
        formulaArtifact: formula,
        expandedVncBoundaryFrame
      }).formulaRealization;
    }
    function buildClassicalNahuatlExpandedVncBoundaryFrame({
      stem = "",
      valence = "",
      mood = "indicative",
      tense = "",
      ...
      const normalizedStem = normalizeClassicalNahuatlVerbstem(stem);
      const normalizedMood = normalizeClassicalNahuatlVerbstemMood(mood);
      const normalizedTense = normalizeClassicalNahuatlVerbstemTense(tense);
      const normalizedDirectional = normalizeClassicalNahuatlDirectionalPrefix(directionalPrefix);
      const directionalIttaContractionFrame = buildClassicalNahuatlDirectionalIttaContractionFrame({
        stem: normalizedStem,
        directionalPrefix: normalizedDirectional,
        objectRelationshipRuleFrame,
        selection: directionalIttaContraction
      });
      const normalizedOutsidePrefixes = Array.from(new Set(outsidePrefixes.map(normalizeClassicalNahuatlOutsidePrefix).filter(Boolean)));
      const hostileSlots = getClassicalNahuatlHostileFormulaSlots({
        hostileLesson8FormulaSlots: hostileFormulaSlots
      });
      const hostileOutsideSlots = hostileSlots.filter(slot => normalizeClassicalNahuatlOutsidePrefix(slot));
      const boundaryApplies = Boolean(normalizedDirectional || normalizedOutsidePrefixes.length || hostileOutsideSlots.length || directionalIttaContractionFrame.requested);
      const directionalPlacement = normalizedDirectional ? getClassicalNahuatlDirectionalPlacement({
        valence,
        objectRelationshipRuleFrame
      }) : "";
      ...
      const supportiveOContext = Boolean(normalizedDirectional === "on" && directionalPlacement === "after-specific-projective-valence" && isClassicalNahuatlThirdSingularProjectiveOnObjectSlot(objectRelationshipRuleFrame?.selectedObjectSlot));
      const actions = [...directionalActions, ...directionalIttaContractionFrame.operationActions, ...(supportiveOContext ? [CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.REPLACE_PERS1_SUPPORTIVE_I_WITH_O] : []), ...(antecessivePresent ? [CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.KEEP_ANTECESSIVE_OUTSIDE_VNC, CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.PRESERVE_OBJECT_SHAPE_UNDER_OUTSIDE_PREFIX] : []), ...(negativePresent ? [CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.KEEP_NEGATIVE_OUTSIDE_VNC] : []), ...(negativeAttractedToAntecessive ? [CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.ATTRACT_NEGATIVE_TO_ANTECESSIVE] : []), ...(hostileOutsideSlots.length ? [CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.BLOCK_OUTSIDE_PREFIX_AS_FORMULA_SLOT] : []), CLASSICAL_NAHUATL_LESSON8_EXPANDED_VNC_ACTIONS.CARRY_BOUNDARY_TO_SELECTED_OUTPUT];
      const blocked = hostileOutsideSlots.length > 0 || !antecessiveTenseAuthorized || !caNegativeTriggerAuthorized || directionalIttaContractionFrame.authorizationStatus === "blocked";
      return {
        kind: "classical-nahuatl-vnc-sentence-expanded-vnc-boundary-frame",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: getClassicalNahuatlExpandedVncRules(),
        boundaryApplies,
        stem: normalizedStem,
        mood: normalizedMood,
        tense: normalizedTense,
        valence: normalizeClassicalNahuatlValence(valence),
        directionalPrefix: normalizedDirectional,
        directionalIttaContractionFrame,
        directionalStemOperationFrame: directionalIttaContractionFrame,
        selectedDirectionalPrefix: directionalIttaContractionFrame.selectedDirectionalPrefix,
        selectedDirectionalStem: directionalIttaContractionFrame.selectedStem,
        ...
        blockReason: hostileOutsideSlots.length ? "outside-prefix-cannot-be-vnc-formula-slot" : antecessivePresent && !antecessiveTenseAuthorized ? "antecessive-prefix-requires-past-tense-vnc" : caNegativePresent && !caNegativeTriggerAuthorized ? "ca-negative-prefix-requires-ma-tla-or-mah-trigger" : directionalIttaContractionFrame.blockReason,
        grammarGenerationAllowed: false,
        surfaceGenerationAllowed: false,
      };
    }
    function getClassicalNahuatlSentenceSurfaceRules() {
      return [{
    ...
    function normalizeClassicalNahuatlVncSentenceType(options = {}) {
    ...
    function normalizeClassicalNahuatlParticleSentenceType(options = {}) {
      const raw = String(options.sentenceType || options.sentenceKind || options.wishKind || options.commandKind || "").trim();
      const normalized = raw.toLowerCase().replace(/[\s_]/gu, "-");
    ...
    function isClassicalNahuatlNegativeSentenceRequested(options = {}) {
      const raw = String(options.sentenceType || options.sentenceKind || "").trim();
      const normalized = raw.toLowerCase().replace(/[\s_]/gu, "-");
    ...
    function normalizeClassicalNahuatlIntroductoryParticle(value = "") {
    ...
    function getClassicalNahuatlIntroductoryParticle(options = {}) {
      return normalizeClassicalNahuatlIntroductoryParticle(options.introductoryParticle || options.introParticle || options.lesson9IntroductoryParticle || options.wishParticle || options.commandParticle || (options.useTlaIntroductoryParticle === true ? "tlā" : "") || (options.useMaIntroductoryParticle === true ? "mā" : ""));
    }
    function normalizeClassicalNahuatlPrefaceParticle(value = "") {
      const normalized = String(value || "").trim().normalize("NFD").replace(/[\u0300-\u036f]/gu, "").toLowerCase().replace(/[#\s_-]/gu, "");
      if (normalized === "ihyo") {
        return "ihyo";
      }
    ...
    function normalizeClassicalNahuatlIntroductoryModifier(value = "") {
    ...
    function getClassicalNahuatlPrefaceParticle(options = {}) {
      return normalizeClassicalNahuatlPrefaceParticle(options.prefaceParticle || options.lesson9PrefaceParticle || options.beforeIntroductoryParticle || "");
    }
    function getClassicalNahuatlIntroductoryModifier(options = {}) {
      return normalizeClassicalNahuatlIntroductoryModifier(options.introductoryModifier || options.lesson9IntroductoryModifier || options.afterIntroductoryModifier || "");
    }
    function isClassicalNahuatlPrefaceParticleAuthorized({
      particle = "",
    ...
    function isClassicalNahuatlIntroductoryModifierAuthorized({
      modifier = "",
      introductoryParticle = "",
      tense = ""
    } = {}) {
    ...
    function isClassicalNahuatlNenAuthorized({
      modifier = "",
      negative = false
    } = {}) {
      if (!modifier) {
    ...
    function getClassicalNahuatlSubjectPersonClass(options = {}) {
    ...
    function deriveClassicalNahuatlSentenceTypeFromMood({
    ...
    function getClassicalNahuatlIntroductoryParticleCanvasSentenceRole(sentenceType = "", subjectPersonClass = "") {
    ...
    function getClassicalNahuatlIntroductoryParticleCanvasSentenceRoleNotice(sentenceRole = "", subjectPersonClass = "") {
    ...
    function getClassicalNahuatlContrastCanvasSentenceRole(subjectPersonClass = "") {
      if (subjectPersonClass === "second-person") {
    ...
    function getClassicalNahuatlContrastCanvasSentenceRoleNotice(sentenceRole = "") {
    ...
    function getClassicalNahuatlSubjectNumberClass(options = {}) {
      const subject = normalizeClassicalNahuatlVerbstemSubject(options.subject || options.subjectPerson || "");
      return subject.endsWith("pl") ? "plural" : "singular";
    }
    function getClassicalNahuatlContrastReadingRequest(options = {}) {
      const normalized = normalizeClassicalNahuatlVerbstemToken(options.admonitiveContrastReading || options.contrastReading || options.requestedContrastReading || "");
      const aliases = {
        "": "",
    ...
    function getClassicalNahuatlClassContrastProfile({
      classId = "",
      subjectPersonClass = "",
      subjectNumberClass = ""
    } = {}) {
    ...
    function getClassicalNahuatlOptativeVncRules() {
      return [{
        tagId: "cn-l9-91-optative-semantic-category",
        section: "9.1",
        transcriptionLineStart: 3508,
        transcriptionLineEnd: 3512,
        exactWitness: "The optative mood is characteristically used to express wishes or hopes.",
        ruleAction: "select-optative-semantic-category",
        callablePath: "buildClassicalNahuatlPredicateFormationRuleFrame"
      }, {
        tagId: "cn-l9-92-borrowed-indicative-form-by-use",
        section: "9.2",
        transcriptionLineStart: 3513,
        transcriptionLineEnd: 3519,
        exactWitness: "A preterit or future optative VNC is identified by use, not by form. Both are borrowed from the indicative mood",
        ruleAction: "borrow-indicative-form-by-optative-use",
        callablePath: "getClassicalNahuatlBorrowedIndicativeFormUse"
      }, {
        tagId: "cn-l9-92-preterit-optative-obligatory-antecessive",
        section: "9.2",
        transcriptionLineStart: 3513,
        transcriptionLineEnd: 3519,
        exactWitness: "the antecessive-order particle ... is obligatorily attached to a preterit optative VNC",
        ruleAction: "require-antecessive-for-preterit-optative",
        callablePath: "getClassicalNahuatlBorrowedIndicativeFormUse"
      }, {
        tagId: "cn-l9-93-imperfective-optative-basis",
        section: "9.3",
        transcriptionLineStart: 3520,
        transcriptionLineEnd: 3529,
        exactWitness: "both nonpast and past optative VNCs are formed on the imperfective stem",
        ruleAction: "build-nonpast-past-optative-on-imperfective-stem",
        callablePath: "getClassicalNahuatlAspectForTense"
      }, {
        tagId: "cn-l9-93-second-person-x-xi",
        section: "9.3",
        transcriptionLineStart: 3530,
        transcriptionLineEnd: 3533,
        exactWitness: "All nonpast and past optative VNCs use the morphs x or xi as second-person fillers",
        ruleAction: "select-x-xi-second-person-subject",
        callablePath: "buildClassicalNahuatlPredicateFormationRuleFrame"
      }, {
        tagId: "cn-l9-93-nonpast-plural-c-an",
        section: "9.3",
        transcriptionLineStart: 3538,
        transcriptionLineEnd: 3539,
        exactWitness: "All nonpast optative VNCs use the morphic dyad c-ān ... when the subject personal pronoun is plural",
        ruleAction: "select-c-an-nonpast-optative-plural-number",
        callablePath: "buildClassicalNahuatlPredicateFormationRuleFrame"
      }, {
        tagId: "cn-l9-93-class-c-truncation-quantity",
        section: "9.3",
        transcriptionLineStart: 3540,
        transcriptionLineEnd: 3543,
        exactWitness: "All nonpast optative VNCs built on Class C verbstems use truncated stem morphs",
        ruleAction: "select-class-c-truncated-nonpast-optative-stem",
        callablePath: "buildClassicalNahuatlImperfectiveShapeEligibilityFrame"
      }, {
        tagId: "cn-l9-93-class-d-vowel-quantity",
        section: "9.3",
        transcriptionLineStart: 3544,
        transcriptionLineEnd: 3546,
        exactWitness: "All nonpast optative VNCs built on Class D verbstems use a stem morph with a short vowel when followed only by silent morphs and a long vowel when followed by at least one sounded morph",
        ruleAction: "select-class-d-conditioned-vowel-quantity",
        callablePath: "buildClassicalNahuatlImperfectiveShapeEligibilityFrame"
      }, {
        tagId: "cn-l9-94-optative-indicative-form-identity",
        section: "9.4",
        transcriptionLineStart: 3548,
        transcriptionLineEnd: 3594,
        exactWitness: "When optative and indicative VNCs have the same shape, the optative can be distinguished from the indicative by syntactical criteria",
        ruleAction: "preserve-optative-indicative-form-identity-and-syntactic-disambiguation",
        callablePath: "evaluateClassicalNahuatlVncApplication"
      }];
    }
    function getClassicalNahuatlWishCommandSentenceRules() {
      return [{
        ...
        ruleAction: "compose-optative-vnc-as-wish-or-command-sentence"
      }, {
        ...
        tagId: "cn-l9-sentence-composition-consumes-complete-vnc",
        section: "9.5-9.9",
        transcriptionLineStart: 3595,
        transcriptionLineEnd: 3728,
    ...
    function getClassicalNahuatlAdmonitionSentenceRules() {
      return [{
    ...
    function normalizeClassicalNahuatlQuestionMode(options = {}) {
    ...
    function getClassicalNahuatlSentenceHostileFormulaSlots(options = {}) {
    ...
    function buildClassicalNahuatlSentencePrefixalStack({
      sentenceParticles = [],
      expandedVncBoundaryFrame = null
    } = {}) {
      const normalizedSentenceParticles = Array.isArray(sentenceParticles) ? sentenceParticles.map(particle => {
        ...
        return normalizeClassicalNahuatlOutsidePrefix(raw);
      }).filter(Boolean) : [];
    ...
    function buildClassicalNahuatlVncSentenceSurfaceFrame({
      ...
      const normalizedTense = normalizeClassicalNahuatlVerbstemTense(options.tense || options.sentenceTense || "", normalizedMood);
      const requestedTenseToken = normalizeClassicalNahuatlVerbstemToken(options.tense || options.sentenceTense || "");
      ...
      const subjectPersonClass = getClassicalNahuatlSubjectPersonClass(options);
      const subjectNumberClass = getClassicalNahuatlSubjectNumberClass(options);
      const explicitLesson9SentenceType = normalizeClassicalNahuatlParticleSentenceType(options);
      const introductoryParticleForDerivation = getClassicalNahuatlIntroductoryParticle(options);
      const requestedOptativePrefaceParticle = getClassicalNahuatlPrefaceParticle(options);
      const requestedOptativeIntroductoryModifier = getClassicalNahuatlIntroductoryModifier(options);
      const lesson9PrefaceParticleAuthorized = isClassicalNahuatlPrefaceParticleAuthorized({
        particle: requestedOptativePrefaceParticle,
        introductoryParticle: introductoryParticleForDerivation
      });
      const lesson9IntroductoryModifierAuthorized = isClassicalNahuatlIntroductoryModifierAuthorized({
        modifier: requestedOptativeIntroductoryModifier,
        introductoryParticle: introductoryParticleForDerivation,
        tense: normalizedTense
      });
      const lesson9PrefaceParticleForDerivation = lesson9PrefaceParticleAuthorized ? requestedOptativePrefaceParticle : "";
      const lesson9IntroductoryModifierForDerivation = lesson9IntroductoryModifierAuthorized ? requestedOptativeIntroductoryModifier : "";
      const lesson10Applies = normalizedMood === "admonitive";
      const lesson10NegativeRequested = Boolean(lesson10Applies && isClassicalNahuatlNegativeSentenceRequested(options));
      const requestedNen = requestedOptativeIntroductoryModifier === "nēn" || lesson10NegativeRequested ? "nēn" : "";
      const lesson10NenAuthorized = isClassicalNahuatlNenAuthorized({
        modifier: requestedNen,
        negative: lesson10NegativeRequested
      });
      const lesson10Nen = lesson10NenAuthorized ? requestedNen : "";
      const requestedTranslationReadingToken = lesson10Applies ? normalizeClassicalNahuatlVerbstemToken(options.admonitiveTranslationReading || options.translationReading || options.requestedTranslationReading || "") : "";
      const lesson10RequestedTranslationReading = {
        ...
        wish: "may-not-wish"
      }[requestedTranslationReadingToken] || requestedTranslationReadingToken || (lesson10NegativeRequested ? "reject-caution-sense" : "warning-sense");
      ...
      const lesson9SentenceType = deriveClassicalNahuatlSentenceTypeFromMood({
      ...
      const lesson9CanvasSentenceRoleNotice = getClassicalNahuatlIntroductoryParticleCanvasSentenceRoleNotice(lesson9CanvasSentenceRole, subjectPersonClass);
      ...
      const lesson10ClassContrastProfile = lesson10Applies ? getClassicalNahuatlClassContrastProfile({
        classId: lesson10ClassId,
        subjectPersonClass,
        subjectNumberClass
      }) : {};
      const requestedAdmonitiveContrastReading = lesson10Applies ? getClassicalNahuatlContrastReadingRequest(options) : "";
      const lesson10ContrastReadingAuthorized = Boolean(!lesson10Applies || !requestedAdmonitiveContrastReading || requestedAdmonitiveContrastReading === "admonitive");
      ...
      const lesson10CanvasSentenceRole = lesson10Applies ? getClassicalNahuatlContrastCanvasSentenceRole(subjectPersonClass) : "";
      const lesson10CanvasSentenceRoleNotice = lesson10Applies ? getClassicalNahuatlContrastCanvasSentenceRoleNotice(lesson10CanvasSentenceRole) : "";
      ...
      const requestedOutsidePrefixes = Array.isArray(options.outsidePrefixes) ? options.outsidePrefixes.map(normalizeClassicalNahuatlOutsidePrefix).filter(Boolean) : [];
      ...
      const lesson10CaNegativeRequested = Boolean(lesson10Applies && (boundaryNegativePrefixes.includes("ca#") || options.caNegative === true || normalizeClassicalNahuatlOutsidePrefix(options.negativePrefix || "") === "ca#" || normalizeClassicalNahuatlOutsidePrefix(options.requestedNegativePrefix || "") === "ca#"));
      ...
      return {
        kind: lesson10Applies ? "classical-nahuatl-admonitive-admonitive-sentence-surface-frame" : lesson9Applies ? "classical-nahuatl-optative-sentence-sentence-surface-frame" : "classical-nahuatl-vnc-sentence-sentence-surface-frame",
        ...
        sourceDocument: CLASSICAL_NAHUATL_LESSON7_SOURCE_DOCUMENT,
        legalWitnessAuthority: CLASSICAL_NAHUATL_LESSON7_LEGAL_WITNESS_AUTHORITY,
        ruleRefs: lesson10Applies ? getClassicalNahuatlAdmonitionSentenceRules() : lesson9Applies ? [...getClassicalNahuatlOptativeVncRules(), ...getClassicalNahuatlWishCommandSentenceRules()] : getClassicalNahuatlSentenceSurfaceRules(),
        ...
        consumedVncRole: "complete-typed-vnc-input-to-sentence-composition",
        sentenceCompositionOperationId: applies ? "vnc-sentence-composition" : "none",
        ...
        admonitiveRequestedContrastReading: requestedAdmonitiveContrastReading,
        ...
        requestedPrefaceParticle: requestedOptativePrefaceParticle,
        ...
        requestedIntroductoryModifier: requestedOptativeIntroductoryModifier,
    ...
    function getClassicalNahuatlPriorVncFrame(stemVariant = "", options = {}) {
      const valence = normalizeClassicalNahuatlValence(options.valence || options.transitivity || options.objectKind || options.object || "");
      const runtimeTarget = getClassicalNahuatlVerbstemRuntimeTarget();
      try {
        if (valence === "intransitive") {
          const builder = runtimeTarget?.buildClassicalNahuatlFiniteVncResult;
          return typeof builder === "function" ? builder(stemVariant, {
            subject: options.subject || options.subjectPerson || options.subj || "",
            mood: options.mood || options.sentenceMood || "",
            tense: options.tense || options.tenseKey || "",
            verbClass: options.verbClass || options.perfectiveClass || "",
          }) : null;
        }
        const builder = runtimeTarget?.buildClassicalNahuatlTransitiveVncObjectFrame;
        ...
        // An invalid intermediate UI/source combination is a blocked grammar
        // path.  It must not escape the canonical owner as a page exception.
        return null;
      }
    }
    function buildClassicalNahuatlVerbstemLogicProofFrame({
      ...
      const runtimeTarget = getClassicalNahuatlVerbstemRuntimeTarget();
      const lowerPriorAuthorized = Boolean(
        (
          typeof runtimeTarget?.isClassicalNahuatlFiniteVncResult === "function"
          && runtimeTarget.isClassicalNahuatlFiniteVncResult(priorVncFrame)
        )
        || (
          typeof runtimeTarget?.isClassicalNahuatlTransitiveVncObjectFrame === "function"
          && runtimeTarget.isClassicalNahuatlTransitiveVncObjectFrame(priorVncFrame)
        )
      );
      const lowerPriorVncSlotFrame = priorVncFrame?.vncSlotFrame || null;
      const lesson11TypedVncSlotFrame =
        lesson11VncApplicationFrame?.typedVncSlotFrame || null;
      const lesson11TypedSlotAuthorized = Boolean(
        lesson11VncApplicationFrame?.authorizationStatus === "authorized"
        && typeof runtimeTarget?.isClassicalNahuatlVncSlotFrame === "function"
        && runtimeTarget.isClassicalNahuatlVncSlotFrame(
          lesson11TypedVncSlotFrame,
        )
      );
      // The lower finite result remains the source receipt.  A Lesson 11
      // operation may then issue a distinct canonical typed slot (suppletive
      // stem, tense, or number carrier).  Rebuilding that slot as if its
      // selected carriers were ordinary caller options loses exactly the
      // conditioned operation we need to preserve.
      const priorVncSlotFrame = lesson11TypedSlotAuthorized
        ? lesson11TypedVncSlotFrame
        : lowerPriorVncSlotFrame;
      const priorAuthorized = Boolean(
        lowerPriorAuthorized
        && (!lesson11VncApplicationFrame || lesson11TypedSlotAuthorized)
      );
      ...
      const lowerOptionalIrregularFormulaRealizations = optionalIrregularPriorVncFrames
        .filter((frame) => (
          typeof runtimeTarget?.isClassicalNahuatlFiniteVncResult === "function"
          && runtimeTarget.isClassicalNahuatlFiniteVncResult(frame)
        ) || (
          typeof runtimeTarget?.isClassicalNahuatlTransitiveVncObjectFrame === "function"
          && runtimeTarget.isClassicalNahuatlTransitiveVncObjectFrame(frame)
        ))
      ...
      const grammarOperationEvaluationFrame = typeof runtimeTarget?.buildClassicalNahuatlVncOperationEvaluationFrame === "function" ? runtimeTarget.buildClassicalNahuatlVncOperationEvaluationFrame({
        priorVncFrame,
      ...
      return {
        kind: "classical-nahuatl-verbstem-logic-proof-frame",
        lesson: "Andrews Lesson 7",
        ...
        premises: [{
          ...
          operation: "typed-vnc-operation-authority",
          rule: "Grammar operations consume typed VNC slots; curriculum order and printed formula artifacts do not choose the result.",
          ...
          boundaryRealizationKind: finalBoundaryRealizationFrame?.kind || "",
          ...
          paradigmTense: lesson11ParadigmPlan.paradigmTense || "",
          semanticTenseValue: lesson11ParadigmPlan.semanticTenseValue || lesson11ParadigmPlan.requestedSemanticTense || "",
          ...
          rule: "Fuente selection consumes typed Source: one whole stem, explicit internal boundaries, or matching user-supplied embed plus matrix constituents.",
          ...
          typedSourceSelectionAvailable: sourceSelectionFrame?.typedSourceSelectionAvailable === true,
          ...
          priorVncResultKind: priorVncFrame?.kind || "",
        ...
        conclusion: {
          ...
          lesson11ConditionedParadigmCellFrame: authorized ? lesson11VncApplicationFrame?.conditionedParadigmCellFrame || null : null,
          ...
          lesson11ParadigmTense: authorized ? lesson11ParadigmPlan?.paradigmTense || "" : "",
          lesson11SemanticTenseValue: authorized ? lesson11ParadigmPlan?.semanticTenseValue || lesson11ParadigmPlan?.requestedSemanticTense || "" : "",
          ...
          lesson11AlternativeBoundaryFrames: authorized ? lesson11AlternativeBoundaryFrames : [],
          ...
          typedSourceSelectionAvailable: authorized ? sourceSelectionFrame?.typedSourceSelectionAvailable === true : false,
          userSelectionTypedSourceAuthorized: authorized ? sourceSelectionFrame?.userSelectionTypedSourceAuthorized === true : false,
          userSelectionContradictsTypedSource: authorized ? sourceSelectionFrame?.userSelectionContradictsTypedSource === true : false,
          ...
          grammarOperationEvaluationFrame: authorized ? grammarOperationEvaluationFrame : null,
          appliedGrammarOperationIds: authorized ? grammarOperationEvaluationFrame?.appliedOperationIds || [] : [],
          resultOperationId: authorized ? grammarOperationEvaluationFrame?.resultOperationId || "" : "",
    ...
    function buildClassicalNahuatlVerbstemSelectedOutputLogicFrame({
      proofFrame = null,
      structureRuleFrame = null,
      sourceSelectionFrame = null,
      progressiveAssimilationFrame = null,
      ...
      return {
        ...
        outputFillers: authorized ? {
          ...
          ...(lesson11ParadigmPlan?.applies ? {
            ...
            lesson11ParadigmTense: proofFrame.conclusion.lesson11ParadigmTense,
            lesson11SemanticTenseValue: proofFrame.conclusion.lesson11SemanticTenseValue,
          ...
          typedSourceSelectionAvailable: proofFrame.conclusion.typedSourceSelectionAvailable,
          userSelectionTypedSourceAuthorized: proofFrame.conclusion.userSelectionTypedSourceAuthorized,
          userSelectionContradictsTypedSource: proofFrame.conclusion.userSelectionContradictsTypedSource,
          ...
          ...(sentenceSurfaceFrame?.sentenceSurfaceApplies === true ? {
            ...
            consumedVncStatus: sentenceSurfaceFrame.consumedVncStatus,
            consumedVncRole: sentenceSurfaceFrame.consumedVncRole,
            sentenceCompositionOperationId: sentenceSurfaceFrame.sentenceCompositionOperationId,
            curriculumOrderAuthority: sentenceSurfaceFrame.curriculumOrderAuthority,
        ...
        steps: authorized ? [{
          ...
          operation: "irregular-paradigm-selection",
          value: proofFrame.conclusion.lesson11SelectedStem,
          consumedRegularStem: proofFrame.conclusion.stemVariant,
          consumedInputStatus: "complete-stem-candidate",
          resultOperationId: "vnc-irregular-paradigm-selection",
          ...
          operation: "sentence-composition",
    ...
    function buildClassicalNahuatlVerbstemReceiptInventory() {
      return {
        kind: "classical-nahuatl-verbstem-display-receipt-inventory",
        receiptRole: "display-only",
        authorityRole: "not-authority",
        outputableSlots: [...CLASSICAL_NAHUATL_LESSON7_OUTPUTABLE_SLOTS],
    ...
    function buildClassicalNahuatlVerbstemDisplayReceiptFrame({
      proofFrame = null,
    ...
    function buildClassicalNahuatlVerbstemReceiptAuthorityFrame({
      proofFrame = null,
      selectedOutputLogicFrame = null,
      ...
      return {
        kind: "classical-nahuatl-verbstem-receipt-authority-rule-frame",
        lesson: "Andrews Lesson 7",
        section: "7.1-7.10",
    ...
    function buildClassicalNahuatlVerbstemClassFrame(stem = "", options = {}) {
      ...
      const lesson7RuntimeTarget =
        getClassicalNahuatlVerbstemRuntimeTarget();
      const canonicalSourceSelectionFrame = Boolean(
        canonicalSourceSelectionCandidate
        && typeof lesson7RuntimeTarget
          ?.isClassicalNahuatlCanonicalSourceSelectionFrame === "function"
        && lesson7RuntimeTarget
          .isClassicalNahuatlCanonicalSourceSelectionFrame(
            canonicalSourceSelectionCandidate,
          )
        && canonicalSourceSelectionCandidate.canonicalStem
          === normalizedStem
      )
        ? canonicalSourceSelectionCandidate
        : null;
      ...
      const lesson11RuntimeTarget = getClassicalNahuatlVerbstemRuntimeTarget();
      const lesson11ParadigmPlan = typeof lesson11RuntimeTarget?.buildClassicalNahuatlIrregularVncParadigmPlan === "function" ? lesson11RuntimeTarget.buildClassicalNahuatlIrregularVncParadigmPlan(normalizedStem, {
        ...requestedOptions,
        requestedMood: requestedOptions.mood || requestedOptions.sentenceMood,
        requestedSemanticTense: requestedOptions.semanticTense || requestedOptions.tense
      }) : null;
      ...
      const structureRuleFrame = buildClassicalNahuatlVerbstemStructureRuleFrame(normalizedStem);
      const sourceSelectionFrame = buildClassicalNahuatlFuenteSourceSelectionFrame(normalizedStem, sourceOptions);
      const citationRuleFrame = buildClassicalNahuatlCitationRuleFrame(normalizedStem, sourceOptions);
      const tlaFusionRuleFrame = buildClassicalNahuatlTlaFusionRuleFrame(normalizedStem, {
        ...sourceOptions,
      ...
      const progressiveAssimilationFrame = tlaFusionRuleFrame.fused || lesson11SupersedesLesson210 ? null : buildClassicalNahuatlProgressiveBoundaryFrameForVerbstem(preAssimilationClassTargetStem);
      ...
      const classRuleFrame = buildClassicalNahuatlVerbstemClassRuleFrame(classTargetStem, {
        ...options,
      ...
      const predicateFormationRuleFrame = buildClassicalNahuatlPredicateFormationRuleFrame(classTargetStem, {
        ...options,
        classProfile: classRuleFrame.classProfile
      });
      const borrowedFormOptions = predicateFormationRuleFrame
        .finiteFormDiffersFromSemanticSelection ? {
        ...options,
      ...
      const analysisRuleFrame = buildClassicalNahuatlAnalysisRuleFrame(normalizedStem, options);
      ...
      const zeroRootLowerLessonBridgeFrame = explicitSourceZeroRoot ? {
        kind: "classical-nahuatl-irregular-vnc-zero-root-prior-vnc-bridge-frame",
        lesson: "Andrews Lesson 11",
        sourceAuthority: "Andrews transcription",
        ...
        formulaStemRestoredBy: licensedSourceZeroRoot ? "classical-nahuatl-irregular-vnc-zero-root-operation-frame" : "none",
      ...
      const basePriorVncFrame = getClassicalNahuatlPriorVncFrame(lowerLessonPriorVncStem, priorVncOptions);
      const predicateTnsOverride = normalizeClassicalNahuatlVerbstemToken(options.predicateTnsOverride || "");
      // A copied slot/result shape is not an authorized finite result.  A
      // requested carrier that differs from the finite owner's selection must
      // therefore block until a named semantic operation owns that change.
      const predicateEnvironmentPriorVncFrame = !predicateTnsOverride
        || predicateTnsOverride
          === basePriorVncFrame?.vncSlotFrame?.slots?.predicate?.tns
        ? basePriorVncFrame
        : null;
      ...
      // carriers as a fresh ordinary lower request.
      const priorVncFrame = predicateEnvironmentPriorVncFrame;
      const objectRelationshipRuleFrame = buildClassicalNahuatlObjectRelationshipRuleFrame(normalizedStem, {
        ...sourceOptions,
        valence: effectiveSourceValence || citationRuleFrame.valence
      }, priorVncFrame);
      const requestedOutsidePrefixes = getClassicalNahuatlOutsidePrefixes(options);
      ...
      const expandedVncBoundaryFrame = buildClassicalNahuatlExpandedVncBoundaryFrame({
        stem: priorVncStem,
        ...
        directionalIttaContraction: options.directionalIttaContraction ?? options.onIttaContraction ?? options.rareDirectionalContraction ?? "",
        outsidePrefixes,
        caNegativeTrigger: options.caNegativeTrigger || options.negativeTrigger || options.triggerParticle || "",
        hostileFormulaSlots: getClassicalNahuatlHostileFormulaSlots(options)
      });
      ...
      const optionalIrregularPriorVncFrames = optionalIrregularStemVariants.filter(variant => variant && variant !== selectedIrregularStemVariant).map(variant => getClassicalNahuatlPriorVncFrame(variant, tlaFusionRuleFrame.fused ? {
        ...borrowedFormOptions,
        valence: "intransitive",
        transitivity: "intransitive",
        verbClass: classRuleFrame.classId,
      ...
      const proofFrame = buildClassicalNahuatlVerbstemLogicProofFrame({
        structureRuleFrame,
        sourceSelectionFrame,
        zeroRootLowerLessonBridgeFrame,
        ...
        lesson11ConditionedParadigmCellFrame: lesson11VncApplicationFrame?.conditionedParadigmCellFrame || null,
        citationRuleFrame,
        classRuleFrame,
      ...
      const sentenceSurfaceFrame = buildClassicalNahuatlVncSentenceSurfaceFrame({
      ...
      }) : proofFrame.grammarOperationEvaluationFrame || null;
      const selectedOutputLogicFrame = buildClassicalNahuatlVerbstemSelectedOutputLogicFrame({
        proofFrame,
        structureRuleFrame,
        sourceSelectionFrame,
        progressiveAssimilationFrame: activeProgressiveAssimilationFrame,
      ...
      const frame = {
        ...
        lesson11ParadigmPlan: activeLesson11ParadigmPlan,
        lesson11VncApplicationFrame,
        lesson11ConditionedParadigmCellFrame: proofFrame.conclusion.lesson11ConditionedParadigmCellFrame,
        lesson11AlternativeBoundaryFrames: proofFrame.conclusion.lesson11AlternativeBoundaryFrames,
        classTargetStem,
        classTargetRole,
        classTargetValence,
        ...
        ruleRefs: getClassicalNahuatlVerbstemRuleLogicRules(),
        classProfile: classRuleFrame.classProfile,
        classId: classRuleFrame.classId,
    ...
    function isClassicalNahuatlVerbstemClassFrame(frame = null) {
      return Boolean(
        frame
        && classicalNahuatlLesson7IssuedVerbstemClassFrames.has(frame)
        && frame.kind
          === "classical-nahuatl-verbstem-verbstem-class-machinery-frame"
        && frame.version
          === CLASSICAL_NAHUATL_LESSON7_VERBSTEM_CLASSES_VERSION
      );
    }
    function installClassicalNahuatlVerbstemClassesClassicGlobals() {
      ...
      Object.assign(globalTarget, {
        getClassicalNahuatlStructureRules,
        getClassicalNahuatlCitationRules,
        getClassicalNahuatlClassRules,
        getClassicalNahuatlTypeOneCausativeRules,
        getClassicalNahuatlClassBChangeRules,
        getClassicalNahuatlVariableClassRules,
        getClassicalNahuatlGuidelineRules,
        getClassicalNahuatlPredicateRules,
        getClassicalNahuatlAnalysisRules,
        getClassicalNahuatlObjectRelationshipRules,
        getClassicalNahuatlTlaFusionRules,
        getClassicalNahuatlExpandedVncRules,
        getClassicalNahuatlSentenceSurfaceRules,
        getClassicalNahuatlOptativeVncRules,
        getClassicalNahuatlWishCommandSentenceRules,
        getClassicalNahuatlVerbstemReceiptRules,
        getClassicalNahuatlOptionalIrregularPerfectiveRules,
        getClassicalNahuatlVerbstemRuleLogicRules,
        normalizeClassicalNahuatlVerbstem,
        buildClassicalNahuatlVerbstemInitialSupportiveIFrame,
        buildClassicalNahuatlCitationValenceFrame,
        inferClassicalNahuatlLesson7ClassProfile,
        getClassicalNahuatlPerfectiveStem,
        buildClassicalNahuatlPredicateTableFrame,
        getClassicalNahuatlSourceBoundaryRoleRecord,
        getClassicalNahuatlClassBSilentCausativeCarrierRecord,
        getClassicalNahuatlTraditionalSpellingWarningRecord,
        getClassicalNahuatlGuidelineAuthorityRecord,
        buildClassicalNahuatlImperfectiveShapeEligibilityFrame,
        getClassicalNahuatlStemVariant,
        getClassicalNahuatlOptionalIrregularPerfectiveRecord,
        buildClassicalNahuatlOptionalIrregularPerfectiveFrame,
        buildClassicalNahuatlSourceBoundaryRoleFrame,
        ...
        buildClassicalNahuatlAnalysisRuleFrame,
        ...
        getClassicalNahuatlObjectMorphIdentityFrame,
        buildClassicalNahuatlFinalBoundaryRealizationFrame,
        applyClassicalNahuatlLesson81DirectionalToFormula,
        buildClassicalNahuatlVerbstemLogicProofFrame,
        buildClassicalNahuatlVerbstemSelectedOutputLogicFrame,
        buildClassicalNahuatlVerbstemReceiptInventory,
    ...
    installClassicalNahuatlVerbstemClassesClassicGlobals();
    ...
    api.getClassicalNahuatlVerbstemRuntimeTarget = getClassicalNahuatlVerbstemRuntimeTarget;
    api.buildClassicalNahuatlProgressiveBoundaryFrameForVerbstem = buildClassicalNahuatlProgressiveBoundaryFrameForVerbstem;
    ...
    api.getClassicalNahuatlStructureRules = getClassicalNahuatlStructureRules;
    api.getClassicalNahuatlCitationRules = getClassicalNahuatlCitationRules;
    api.getClassicalNahuatlClassRules = getClassicalNahuatlClassRules;
    api.getClassicalNahuatlTypeOneCausativeRules = getClassicalNahuatlTypeOneCausativeRules;
    api.getClassicalNahuatlClassBChangeRules = getClassicalNahuatlClassBChangeRules;
    api.getClassicalNahuatlVariableClassRules = getClassicalNahuatlVariableClassRules;
    api.getClassicalNahuatlGuidelineRules = getClassicalNahuatlGuidelineRules;
    api.getClassicalNahuatlPredicateRules = getClassicalNahuatlPredicateRules;
    api.getClassicalNahuatlAnalysisRules = getClassicalNahuatlAnalysisRules;
    api.getClassicalNahuatlObjectRelationshipRules = getClassicalNahuatlObjectRelationshipRules;
    api.getClassicalNahuatlTlaFusionRules = getClassicalNahuatlTlaFusionRules;
    api.getClassicalNahuatlVerbstemReceiptRules = getClassicalNahuatlVerbstemReceiptRules;
    api.getClassicalNahuatlOptionalIrregularPerfectiveRules = getClassicalNahuatlOptionalIrregularPerfectiveRules;
    api.getClassicalNahuatlVerbstemRuleLogicRules = getClassicalNahuatlVerbstemRuleLogicRules;
    api.normalizeClassicalNahuatlVerbstemToken = normalizeClassicalNahuatlVerbstemToken;
    api.normalizeClassicalNahuatlVerbstem = normalizeClassicalNahuatlVerbstem;
    api.getClassicalNahuatlExplicitTlaFusionDerivedStem = getClassicalNahuatlExplicitTlaFusionDerivedStem;
    api.removeClassicalNahuatlLesson7Marks = removeClassicalNahuatlLesson7Marks;
    api.getClassicalNahuatlStemForShape = getClassicalNahuatlStemForShape;
    api.wrapClassicalNahuatlLesson7Stem = wrapClassicalNahuatlLesson7Stem;
    api.getClassicalNahuatlStructureLookupKey = getClassicalNahuatlStructureLookupKey;
    api.getClassicalNahuatlTlaFusionCanvasBoundaryExample = getClassicalNahuatlTlaFusionCanvasBoundaryExample;
    api.getClassicalNahuatlUnknownInternalMorphRecords = getClassicalNahuatlUnknownInternalMorphRecords;
    api.getClassicalNahuatlContrastiveBoundaryRecord = getClassicalNahuatlContrastiveBoundaryRecord;
    api.buildClassicalNahuatlUserDefinedEmbedMatrixBoundaryRecord = buildClassicalNahuatlUserDefinedEmbedMatrixBoundaryRecord;
    api.getClassicalNahuatlSourceBoundaryRoleRecord = getClassicalNahuatlSourceBoundaryRoleRecord;
    api.getClassicalNahuatlNearbySourceBoundaryRecordKey = getClassicalNahuatlNearbySourceBoundaryRecordKey;
    api.dedupeClassicalNahuatlLesson7RuleRefs = dedupeClassicalNahuatlLesson7RuleRefs;
    api.buildClassicalNahuatlSourceBoundaryRoleFrame = buildClassicalNahuatlSourceBoundaryRoleFrame;
    api.normalizeClassicalNahuatlSourceSelectionKind = normalizeClassicalNahuatlSourceSelectionKind;
    api.buildClassicalNahuatlFuenteSourceSelectionFrame = buildClassicalNahuatlFuenteSourceSelectionFrame;
    api.isClassicalNahuatlFuenteSourceSelectionFrame = isClassicalNahuatlFuenteSourceSelectionFrame;
    api.getClassicalNahuatlClassBSilentCausativeCarrierRecord = getClassicalNahuatlClassBSilentCausativeCarrierRecord;
    api.getClassicalNahuatlTraditionalSpellingWarningRecord = getClassicalNahuatlTraditionalSpellingWarningRecord;
    api.getClassicalNahuatlShapeConsonantUnits = getClassicalNahuatlShapeConsonantUnits;
    api.isClassicalNahuatlFinalVowelAfterConsonantCluster = isClassicalNahuatlFinalVowelAfterConsonantCluster;
    api.isClassicalNahuatlMonosyllabicStem = isClassicalNahuatlMonosyllabicStem;
    api.makeClassicalNahuatlLesson7GeneralClassAuthorityRecord = makeClassicalNahuatlLesson7GeneralClassAuthorityRecord;
    api.getClassicalNahuatlGuidelineAuthorityRecord = getClassicalNahuatlGuidelineAuthorityRecord;
    api.getClassicalNahuatlFinalVowelInfo = getClassicalNahuatlFinalVowelInfo;
    api.normalizeClassicalNahuatlValence = normalizeClassicalNahuatlValence;
    api.getClassicalNahuatlVerbstemInitialVowelKind = getClassicalNahuatlVerbstemInitialVowelKind;
    api.omitClassicalNahuatlLesson7InitialSupportiveI = omitClassicalNahuatlLesson7InitialSupportiveI;
    api.buildClassicalNahuatlVerbstemInitialSupportiveIFrame = buildClassicalNahuatlVerbstemInitialSupportiveIFrame;
    api.getClassicalNahuatlProjectiveCitationRepresentative = getClassicalNahuatlProjectiveCitationRepresentative;
    api.buildClassicalNahuatlCitationValenceFrame = buildClassicalNahuatlCitationValenceFrame;
    api.normalizeClassicalNahuatlClassId = normalizeClassicalNahuatlClassId;
    api.getClassicalNahuatlStemRelationship = getClassicalNahuatlStemRelationship;
    api.getClassicalNahuatlOptionalIrregularPerfectiveRecord = getClassicalNahuatlOptionalIrregularPerfectiveRecord;
    api.normalizeClassicalNahuatlVerbstemSubject = normalizeClassicalNahuatlVerbstemSubject;
    api.isClassicalNahuatlOptionalIrregularEnvironment = isClassicalNahuatlOptionalIrregularEnvironment;
    ...
    api.getClassicalNahuatlClassCTruncatedStem = getClassicalNahuatlClassCTruncatedStem;
    api.isClassicalNahuatlSingularSubject = isClassicalNahuatlSingularSubject;
    api.buildClassicalNahuatlImperfectiveShapeEligibilityFrame = buildClassicalNahuatlImperfectiveShapeEligibilityFrame;
    api.getClassicalNahuatlExactPerfectiveStemOverride = getClassicalNahuatlExactPerfectiveStemOverride;
    api.applyClassicalNahuatlLesson7ClassBChanges = applyClassicalNahuatlLesson7ClassBChanges;
    api.getClassicalNahuatlClassBPerfectiveCarrierFrame = getClassicalNahuatlClassBPerfectiveCarrierFrame;
    api.getClassicalNahuatlPerfectiveStem = getClassicalNahuatlPerfectiveStem;
    api.buildClassicalNahuatlOptionalIrregularPerfectiveFrame = buildClassicalNahuatlOptionalIrregularPerfectiveFrame;
    api.normalizeClassicalNahuatlVerbstemMood = normalizeClassicalNahuatlVerbstemMood;
    api.normalizeClassicalNahuatlVerbstemTense = normalizeClassicalNahuatlVerbstemTense;
    api.getClassicalNahuatlBorrowedIndicativeFormUse = getClassicalNahuatlBorrowedIndicativeFormUse;
    api.getClassicalNahuatlAspectForTense = getClassicalNahuatlAspectForTense;
    api.normalizeClassicalNahuatlPredicateCarrier = normalizeClassicalNahuatlPredicateCarrier;
    api.getClassicalNahuatlPredicateRuleId = getClassicalNahuatlPredicateRuleId;
    api.getClassicalNahuatlPredicateTableSide = getClassicalNahuatlPredicateTableSide;
    api.buildClassicalNahuatlPredicateTableFrame = buildClassicalNahuatlPredicateTableFrame;
    api.getClassicalNahuatlStemVariant = getClassicalNahuatlStemVariant;
    api.buildClassicalNahuatlVerbstemStructureRuleFrame = buildClassicalNahuatlVerbstemStructureRuleFrame;
    api.buildClassicalNahuatlCitationRuleFrame = buildClassicalNahuatlCitationRuleFrame;
    api.buildClassicalNahuatlVerbstemClassRuleFrame = buildClassicalNahuatlVerbstemClassRuleFrame;
    api.buildClassicalNahuatlPredicateFormationRuleFrame = buildClassicalNahuatlPredicateFormationRuleFrame;
    api.buildClassicalNahuatlAnalysisRuleFrame = buildClassicalNahuatlAnalysisRuleFrame;
    api.normalizeClassicalNahuatlObjectRelationshipToken = normalizeClassicalNahuatlObjectRelationshipToken;
    api.normalizeClassicalNahuatlIndefiniteObject = normalizeClassicalNahuatlIndefiniteObject;
    api.normalizeClassicalNahuatlObjectRelationshipKind = normalizeClassicalNahuatlObjectRelationshipKind;
    api.getClassicalNahuatlObjectRelationshipGroup = getClassicalNahuatlObjectRelationshipGroup;
    api.getClassicalNahuatlObjectRelationshipWitness = getClassicalNahuatlObjectRelationshipWitness;
    api.getClassicalNahuatlSpecificProjectiveRelationshipKind = getClassicalNahuatlSpecificProjectiveRelationshipKind;
    api.getClassicalNahuatlSelectedObjectRelationship = getClassicalNahuatlSelectedObjectRelationship;
    api.buildClassicalNahuatlObjectRelationshipRuleFrame = buildClassicalNahuatlObjectRelationshipRuleFrame;
    api.normalizeClassicalNahuatlTlaFusionAnalysisKind = normalizeClassicalNahuatlTlaFusionAnalysisKind;
    api.normalizeClassicalNahuatlAdverbPosition = normalizeClassicalNahuatlAdverbPosition;
    api.getClassicalNahuatlFusedTlaSegment = getClassicalNahuatlFusedTlaSegment;
    api.getClassicalNahuatlExplicitTlaFusionEmbedStem = getClassicalNahuatlExplicitTlaFusionEmbedStem;
    api.getClassicalNahuatlExplicitTlaFusionMatrixStem = getClassicalNahuatlExplicitTlaFusionMatrixStem;
    api.buildClassicalNahuatlConstructiveTlaFusionTargetFrame = buildClassicalNahuatlConstructiveTlaFusionTargetFrame;
    api.buildClassicalNahuatlTlaFusionDerivedStem = buildClassicalNahuatlTlaFusionDerivedStem;
    api.buildClassicalNahuatlTlaFusionSourceStem = buildClassicalNahuatlTlaFusionSourceStem;
    api.buildClassicalNahuatlTlaFusionRuleFrame = buildClassicalNahuatlTlaFusionRuleFrame;
    api.getClassicalNahuatlExpandedVncRules = getClassicalNahuatlExpandedVncRules;
    api.normalizeClassicalNahuatlDirectionalPrefix = normalizeClassicalNahuatlDirectionalPrefix;
    api.normalizeClassicalNahuatlOutsidePrefix = normalizeClassicalNahuatlOutsidePrefix;
    api.getClassicalNahuatlOutsidePrefixes = getClassicalNahuatlOutsidePrefixes;
    api.getClassicalNahuatlHostileFormulaSlots = getClassicalNahuatlHostileFormulaSlots;
    api.getClassicalNahuatlDirectionalPlacement = getClassicalNahuatlDirectionalPlacement;
    api.isClassicalNahuatlThirdSingularProjectiveOnObjectSlot = isClassicalNahuatlThirdSingularProjectiveOnObjectSlot;
    api.getClassicalNahuatlObjectMorphIdentityFrame = getClassicalNahuatlObjectMorphIdentityFrame;
    api.getClassicalNahuatlFirstSound = getClassicalNahuatlFirstSound;
    api.getClassicalNahuatlLastSound = getClassicalNahuatlLastSound;
    api.isClassicalNahuatlVncBoundaryVowelSound = isClassicalNahuatlVncBoundaryVowelSound;
    api.getClassicalNahuatlPers1Carrier = getClassicalNahuatlPers1Carrier;
    api.setClassicalNahuatlLesson81Pers1Carrier = setClassicalNahuatlLesson81Pers1Carrier;
    api.getClassicalNahuatlPers1CarrierFamily = getClassicalNahuatlPers1CarrierFamily;
    api.getClassicalNahuatlNextCarrierAfterPers1 = getClassicalNahuatlNextCarrierAfterPers1;
    api.applyClassicalNahuatlLesson81Pers1FinalSupport = applyClassicalNahuatlLesson81Pers1FinalSupport;
    api.getClassicalNahuatlSpecificObjectSlotFromFormula = getClassicalNahuatlSpecificObjectSlotFromFormula;
    api.getClassicalNahuatlFormulaStemInitialAfterObject = getClassicalNahuatlFormulaStemInitialAfterObject;
    api.getClassicalNahuatlSoundBeforeObject = getClassicalNahuatlSoundBeforeObject;
    ...
    api.getClassicalNahuatlNumberDyadFrame = getClassicalNahuatlNumberDyadFrame;
    api.parseClassicalNahuatlLesson81FinalPredicateNumber = parseClassicalNahuatlLesson81FinalPredicateNumber;
    api.isClassicalNahuatlSilentCarrier = isClassicalNahuatlSilentCarrier;
    api.getClassicalNahuatlNum1LeftSound = getClassicalNahuatlNum1LeftSound;
    api.isClassicalNahuatlNum1KConnectorContext = isClassicalNahuatlNum1KConnectorContext;
    ...
    api.buildClassicalNahuatlLegacyStringBoundaryRealizationFrame = buildClassicalNahuatlLegacyStringBoundaryRealizationFrame;
    api.buildClassicalNahuatlFinalBoundaryRealizationFrame = buildClassicalNahuatlFinalBoundaryRealizationFrame;
    api.applyClassicalNahuatlLesson81DirectionalToFormula = applyClassicalNahuatlLesson81DirectionalToFormula;
    api.normalizeClassicalNahuatlDirectionalIttaContractionSelection = normalizeClassicalNahuatlDirectionalIttaContractionSelection;
    api.buildClassicalNahuatlDirectionalIttaContractionFrame = buildClassicalNahuatlDirectionalIttaContractionFrame;
    api.buildClassicalNahuatlExpandedVncBoundaryFrame = buildClassicalNahuatlExpandedVncBoundaryFrame;
    api.getClassicalNahuatlSentenceSurfaceRules = getClassicalNahuatlSentenceSurfaceRules;
    api.normalizeClassicalNahuatlVncSentenceType = normalizeClassicalNahuatlVncSentenceType;
    api.normalizeClassicalNahuatlParticleSentenceType = normalizeClassicalNahuatlParticleSentenceType;
    api.isClassicalNahuatlNegativeSentenceRequested = isClassicalNahuatlNegativeSentenceRequested;
    api.normalizeClassicalNahuatlIntroductoryParticle = normalizeClassicalNahuatlIntroductoryParticle;
    api.getClassicalNahuatlIntroductoryParticle = getClassicalNahuatlIntroductoryParticle;
    api.normalizeClassicalNahuatlPrefaceParticle = normalizeClassicalNahuatlPrefaceParticle;
    api.normalizeClassicalNahuatlIntroductoryModifier = normalizeClassicalNahuatlIntroductoryModifier;
    api.getClassicalNahuatlPrefaceParticle = getClassicalNahuatlPrefaceParticle;
    api.getClassicalNahuatlIntroductoryModifier = getClassicalNahuatlIntroductoryModifier;
    api.isClassicalNahuatlPrefaceParticleAuthorized = isClassicalNahuatlPrefaceParticleAuthorized;
    api.isClassicalNahuatlIntroductoryModifierAuthorized = isClassicalNahuatlIntroductoryModifierAuthorized;
    api.isClassicalNahuatlNenAuthorized = isClassicalNahuatlNenAuthorized;
    api.getClassicalNahuatlSubjectPersonClass = getClassicalNahuatlSubjectPersonClass;
    api.deriveClassicalNahuatlSentenceTypeFromMood = deriveClassicalNahuatlSentenceTypeFromMood;
    api.getClassicalNahuatlIntroductoryParticleCanvasSentenceRole = getClassicalNahuatlIntroductoryParticleCanvasSentenceRole;
    api.getClassicalNahuatlIntroductoryParticleCanvasSentenceRoleNotice = getClassicalNahuatlIntroductoryParticleCanvasSentenceRoleNotice;
    api.getClassicalNahuatlContrastCanvasSentenceRole = getClassicalNahuatlContrastCanvasSentenceRole;
    api.getClassicalNahuatlContrastCanvasSentenceRoleNotice = getClassicalNahuatlContrastCanvasSentenceRoleNotice;
    api.getClassicalNahuatlSubjectNumberClass = getClassicalNahuatlSubjectNumberClass;
    api.getClassicalNahuatlContrastReadingRequest = getClassicalNahuatlContrastReadingRequest;
    api.getClassicalNahuatlClassContrastProfile = getClassicalNahuatlClassContrastProfile;
    api.getClassicalNahuatlOptativeVncRules = getClassicalNahuatlOptativeVncRules;
    api.getClassicalNahuatlWishCommandSentenceRules = getClassicalNahuatlWishCommandSentenceRules;
    api.getClassicalNahuatlAdmonitionSentenceRules = getClassicalNahuatlAdmonitionSentenceRules;
    api.normalizeClassicalNahuatlQuestionMode = normalizeClassicalNahuatlQuestionMode;
    api.getClassicalNahuatlSentenceHostileFormulaSlots = getClassicalNahuatlSentenceHostileFormulaSlots;
    api.buildClassicalNahuatlSentencePrefixalStack = buildClassicalNahuatlSentencePrefixalStack;
    api.getClassicalNahuatlPriorVncFrame = getClassicalNahuatlPriorVncFrame;
    api.buildClassicalNahuatlVerbstemLogicProofFrame = buildClassicalNahuatlVerbstemLogicProofFrame;
    api.buildClassicalNahuatlVerbstemSelectedOutputLogicFrame = buildClassicalNahuatlVerbstemSelectedOutputLogicFrame;
    api.buildClassicalNahuatlVerbstemReceiptInventory = buildClassicalNahuatlVerbstemReceiptInventory;
