...
export function createPreteritEngineContext(targetObject = globalThis, installationContext = null) {
    ...
    function getPretUniversalPrefixForBase(base, subjectPrefix, objectPrefix, directionalInputPrefix = "", directionalOutputPrefix = "", baseSubjectPrefix = subjectPrefix, baseObjectPrefix = objectPrefix, indirectObjectMarker = "", hasDoubleDash = false) {
    ...
    function buildPretUniversalMCodaSoundSpellingFrame({
      ...
      if (typeof targetObject.buildClassicalNahuatlConsonantPhoneShiftFrame !== "function") {
        return null;
      }
      const frame = targetObject.buildClassicalNahuatlConsonantPhoneShiftFrame({
        ruleId: "cn-l2-213-m-exposed-n",
        sourceConsonant: "m",
        position: "exposed",
        requestedSpelling: "n",
      ...
      return frame?.authorizationStatus === "authorized" ? {
        ...
        sourceSurface: frame.sourceConsonant,
        target: frame.outputSpelling,
        grammarSlot,
        syllablePosition,
      ...
      if (isPlural) {
        ...
        variants.forEach(variant => {
          ...
          const {
            prefix,
            base
          } = getPretUniversalPrefixForBase(variantBase, subjectPrefix, objectPrefix, directionalInputPrefix, directionalOutputPrefix, baseSubjectPrefix, baseObjectPrefix, indirectObjectMarker, hasDoubleDash);
    ...
    function buildClassBasedResult({
      ...
      const context = targetObject.buildPretUniversalContext(verb, analysisTarget, isTransitive, {
        hasSlashMarker,
        hasSuffixSeparator,
        hasLeadingDash,
        hasBoundMarker,
        ...
        if (isPreterit) {
          ...
          bases.forEach(base => {
            const {
              prefix,
              base: baseCore
            } = getPretUniversalPrefixForBase(base, subjectPrefix, objectPrefix, directionalInputPrefix, directionalOutputPrefix, baseSubjectPrefix, baseObjectPrefix, indirectObjectMarker, hasDoubleDash);
            const resolvedCore = typeof targetObject.resolveOptionalSupportiveOutputVerb === "function" ? targetObject.resolveOptionalSupportiveOutputVerb({
              pers1: "",
              obj1: "",
...
export function installPreteritEngineGlobals(targetObject = globalThis, installationContext = null) {
    const api = createPreteritEngineContext(targetObject, installationContext);
