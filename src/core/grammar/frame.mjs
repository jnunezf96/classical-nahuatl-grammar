...
export function createGrammarFrameModule(targetObject = globalThis, installationContext = null) {
    ...
    var issuedGrammarFrames = new WeakSet();
    const issuedGrammarFrameOwnerCapabilities = new WeakMap();
    const issuedGrammarFrameOwners = new WeakMap();
    const issuedGrammarFrameIntegrity = new WeakMap();
    const registerGrammarFrameOwnerCapabilityIssuer =
      installationContext?.registerGrammarFrameOwnerCapabilityIssuer;
    if (typeof registerGrammarFrameOwnerCapabilityIssuer === "function") {
      registerGrammarFrameOwnerCapabilityIssuer((ownerId = "") => {
        const normalizedOwnerId = String(ownerId || "").trim();
        if (!normalizedOwnerId) {
          return null;
        }
        const capability = Object.freeze(Object.create(null));
        issuedGrammarFrameOwnerCapabilities.set(capability, normalizedOwnerId);
        return capability;
      });
    }
    ...
    const ANDREWS_LOGIC_AUTHORITY_POLICY = Object.freeze({
      version: ANDREWS_LOGIC_AUTHORITY_POLICY_VERSION,
      grammarLogicAuthority: "ANDREWS_TRANSCRIPTION_CANVAS.md",
      ...
      contextualClassicalRealizationRequired: false,
    ...
    const ANDREWS_FORMULA_SLOT_SCHEMAS = Object.freeze({
      "vnc-shell": freezeAndrewsFormulaSlotSchema({
        ...
        generationContract: {
          ...
          outputPolicy: "VNC formulas use Andrews as the grammar logic authority; Classical spelling is realized at typed morph boundaries."
        },
        ...
        diagnostics: ["Classical VNC spelling is structural evidence only.", "Displayed Classical surfaces are typed boundary realizations of Andrews-licensed formula logic."]
      }),
      ...
      "possessive-state-nnc": freezeAndrewsFormulaSlotSchema({
        ...
        generationContract: {
          ...
          diagnosticOnlyWhenMissingSource: true,
          unsupportedWhenBlocked: true,
          outputPolicy: "Generate possessive-state structure from Andrews logic and realize Classical spelling at typed morph boundaries."
        },
    ...
    function issueAndrewsCnvTenseLogicAuthorityFrame(frame = null) {
      const clone = cloneAndrewsCnvTenseLogicAuthorityFrame(frame);
      if (!clone) {
        return null;
      }
      Object.freeze(clone.sourceRefs);
      Object.freeze(clone);
      issuedAndrewsCnvTenseLogicAuthorityFrames.add(clone);
      return clone;
    }
    function getAndrewsCnvTenseLogicAuthorityFrame(tenseValue = "") {
      const normalizedTense = String(tenseValue || "").trim();
      const directFrame = issueAndrewsCnvTenseLogicAuthorityFrame(ANDREWS_CNV_TENSE_LOGIC_AUTHORITY_BY_TENSE[normalizedTense]);
      if (directFrame) {
        return directFrame;
      }
      if (typeof targetObject.NONACTIVE_SUFFIX_ORDER !== "undefined" && Array.isArray(targetObject.NONACTIVE_SUFFIX_ORDER) && targetObject.NONACTIVE_SUFFIX_ORDER.includes(normalizedTense)) {
        return issueAndrewsCnvTenseLogicAuthorityFrame({
          scope: "andrews-licensed",
          source: "Andrews",
          ...
          title: "Andrews Lesson 20: nonactive suffix selection is derived-stem logic."
        });
      }
      return issueAndrewsCnvTenseLogicAuthorityFrame({
        scope: "unknown",
        ...
        andrewsTense: "",
        label: "unclassified",
        title: "ANDREWS_TRANSCRIPTION_CANVAS.md must classify this tense before CNV generation."
      });
    }
    function getAndrewsCnvTenseLogicGenerationGateFrame(frameOrTense = "") {
      const frame = frameOrTense && typeof frameOrTense === "object"
        ? issuedAndrewsCnvTenseLogicAuthorityFrames.has(frameOrTense) ? frameOrTense : null
    ...
    function deepFreezeAndrewsFormulaSlotSchema(schema = null) {
      if (!schema || typeof schema !== "object") {
        return null;
      }
      Object.freeze(schema.evidenceRefs);
      schema.slots.forEach(slot => {
        Object.freeze(slot.blockedInterpretations);
        Object.freeze(slot.diagnostics);
        Object.freeze(slot);
      });
      Object.freeze(schema.slots);
      schema.sourceRequirements.forEach(Object.freeze);
      Object.freeze(schema.sourceRequirements);
      Object.freeze(schema.generationContract);
      Object.freeze(schema.diagnostics);
      return Object.freeze(schema);
    }
    function issueAndrewsFormulaSlotSchema(schema = null) {
      const clone = deepFreezeAndrewsFormulaSlotSchema(cloneAndrewsFormulaSlotSchema(schema));
      if (!clone) {
        return null;
      }
      issuedAndrewsFormulaSlotSchemas.add(clone);
      return clone;
    }
    function getAndrewsFormulaSlotSchema(schemaId = "") {
      const key = String(schemaId || "").trim();
      return issueAndrewsFormulaSlotSchema(ANDREWS_FORMULA_SLOT_SCHEMAS[key] || null);
    }
    ...
    function resolveAndrewsFormulaSlotSchema(schemaOrId = "") {
      if (schemaOrId && typeof schemaOrId === "object") {
        return issuedAndrewsFormulaSlotSchemas.has(schemaOrId) ? schemaOrId : null;
      }
    ...
    function evaluateAndrewsFormulaGenerationAuthority(schemaOrId = "", {
      ...
      return {
        ...
        authority: policy.grammarLogicAuthority || "ANDREWS_TRANSCRIPTION_CANVAS.md",
        logicAuthority: policy.grammarLogicAuthority || "ANDREWS_TRANSCRIPTION_CANVAS.md",
        orthographyAuthority: policy.orthographyAuthority || "Classical Andrews transcription",
    ...
    function normalizeGrammarFormulaSegmentFrame(segment = null) {
      ...
      return {
        ...
        classicalRealizationAuthority: String(segment.classicalRealizationAuthority || "Classical Andrews transcription"),
    ...
    function buildGrammarFormulaRealizationRecord({
      ...
      return Object.freeze({
        ...
        classicalRealizationAuthority: String(classicalRealizationAuthority || "Classical Andrews transcription"),
    ...
    function buildGrammarAuthorityFrame({
      ...
      return {
        ...
        spellingAuthority: String(spellingAuthority || "Classical Andrews transcription"),
    ...
    function getGrammarFrameIntegritySignature(value = null) {
      const seen = new WeakMap();
      let nextReferenceId = 1;
      const visit = node => {
        if (node === null) return "null";
        const nodeType = typeof node;
        if (nodeType === "string") return `string:${JSON.stringify(node)}`;
        if (nodeType === "number") return `number:${Number.isNaN(node) ? "NaN" : String(node)}`;
        if (nodeType === "boolean") return `boolean:${node ? "1" : "0"}`;
        if (nodeType === "undefined") return "undefined";
        if (nodeType === "bigint") return `bigint:${String(node)}`;
        if (nodeType === "symbol") return `symbol:${String(node.description || "")}`;
        if (nodeType === "function") return `function:${String(node.name || "")}`;
        if (seen.has(node)) return `reference:${seen.get(node)}`;
        const referenceId = nextReferenceId;
        nextReferenceId += 1;
        seen.set(node, referenceId);
        if (Array.isArray(node)) {
          return `array:${referenceId}:[${node.map(visit).join(",")}]`;
        }
        ...
        const entries = Reflect.ownKeys(descriptors).map(key => {
          ...
          if (!Object.prototype.hasOwnProperty.call(descriptor, "value")) {
            return `${keyText}=accessor:${descriptor.get ? String(descriptor.get.name || "get") : ""}:${descriptor.set ? String(descriptor.set.name || "set") : ""}`;
          }
          return `${keyText}=${visit(descriptor.value)}`;
        }).sort();
        return `object:${referenceId}:{${entries.join(",")}}`;
      };
      return visit(value);
    }
    function buildGrammarFrame(overrides = {}, ownerCapability = null) {
      const ownerId = issuedGrammarFrameOwnerCapabilities.get(ownerCapability) || "";
      if (!ownerId) {
        return null;
      }
      ...
      GRAMMAR_FRAME_KEYS.forEach(key => {
        const normalizedLayer = normalizeGrammarFrameObject(source[key]);
        frame[key] = normalizedLayer ? Object.freeze(normalizedLayer) : null;
      });
      Object.freeze(frame.layerOrder);
      Object.freeze(frame);
      issuedGrammarFrames.add(frame);
      issuedGrammarFrameOwners.set(frame, ownerId);
      issuedGrammarFrameIntegrity.set(frame, getGrammarFrameIntegritySignature(frame));
      return frame;
    }
    function isIssuedGrammarFrame(frame = null) {
      return Boolean(
        frame
        && typeof frame === "object"
        && !Array.isArray(frame)
        && issuedGrammarFrames.has(frame)
        && issuedGrammarFrameIntegrity.get(frame) === getGrammarFrameIntegritySignature(frame)
        && frame.version === GRAMMAR_FRAME_VERSION
        && Array.isArray(frame.layerOrder)
        && frame.layerOrder.join("|") === GRAMMAR_FRAME_LAYER_ORDER.join("|")
        && GRAMMAR_FRAME_KEYS.every(key => (
          Object.prototype.hasOwnProperty.call(frame, key)
          && (
            frame[key] === null
            || typeof frame[key] === "object" && !Array.isArray(frame[key])
          )
        ))
        && Object.keys(frame).every(key => (
          key === "version"
          || key === "layerOrder"
          || GRAMMAR_FRAME_KEYS.includes(key)
        ))
      );
    }
    function getIssuedGrammarFrameOwnerId(frame = null) {
      return isIssuedGrammarFrame(frame)
        ? String(issuedGrammarFrameOwners.get(frame) || "")
        : "";
    }
    function isIssuedCanonicalGrammarResultFrame(frame = null) {
      if (!isIssuedGrammarFrame(frame)) {
        return false;
      }
      const resultFrame = frame.resultFrame;
      if (!resultFrame || resultFrame.ok !== true) {
        return false;
      }
      const formulaSurfacePairs =
        getGrammarCanonicalFormulaSurfacePairsFromResultFrame(resultFrame);
      if (!formulaSurfacePairs.length) {
        return false;
      }
      const canonicalSurfaceForms =
        getGrammarResultFrameCanonicalSurfaceForms(resultFrame);
      const primarySurface = normalizeGrammarSurfaceValue(
        resultFrame.surface || canonicalSurfaceForms[0] || ""
      );
      return Boolean(
        primarySurface
        && canonicalSurfaceForms.includes(primarySurface)
        && formulaSurfacePairs.some(pair => pair.surface === primarySurface)
      );
    }
    function getIssuedGrammarFrameCanonicalSurfaceForms(frame = null) {
      return isIssuedCanonicalGrammarResultFrame(frame)
        ? getGrammarResultFrameCanonicalSurfaceForms(frame.resultFrame)
        : Object.freeze([]);
    }
    function getIssuedGrammarFrameCanonicalFormulaSurfacePairs(frame = null) {
      return isIssuedCanonicalGrammarResultFrame(frame)
        ? getGrammarCanonicalFormulaSurfacePairsFromResultFrame(frame.resultFrame)
        : Object.freeze([]);
    }
    ...
    function getGrammarMetadataContractFrame(record = null, options = {}) {
      ...
      const candidate = optionFrame || recordFrame || null;
      return isIssuedGrammarFrame(candidate) ? candidate : null;
    }
    ...
    function buildGrammarResultContract({
      ...
      const frameCandidate = grammarFrame && typeof grammarFrame === "object"
        ? grammarFrame
        : output.grammarFrame && typeof output.grammarFrame === "object"
          ? output.grammarFrame
          : output.frames && typeof output.frames === "object"
            ? output.frames
            : null;
      const frames = isIssuedGrammarFrame(frameCandidate) ? frameCandidate : null;
      ...
      const ok = frameOk === null ? false : frameOk;
    ...
    function getGrammarDiagnosticLayerContract(entry = null) {
      ...
      if (/orthography|spelling|letter|classical|classical-letter|classical-spelling|phonolog|syllable/i.test(layerId)) {
        return {
    ...
    function buildGrammarAstContractFrame(ast = null, options = {}, ownerCapability = null) {
      ...
      const formulaRecord = node.formulaRecord
        && typeof node.formulaRecord === "object"
        && node.formulaRecord.kind === "grammar-formula-record"
        ? node.formulaRecord
        : null;
      const formulaRealizationRecord = node.formulaRealizationRecord
        && typeof node.formulaRealizationRecord === "object"
        && node.formulaRealizationRecord.kind === "grammar-formula-realization-record"
        ? node.formulaRealizationRecord
        : null;
    ...
    function attachGrammarAstContract(ast = null, options = {}, ownerCapability = null) {
      const node = ast && typeof ast === "object" ? ast : {};
      const grammarFrame = buildGrammarAstContractFrame(node, options, ownerCapability);
      if (!grammarFrame) {
        return null;
      }
    ...
    function buildGrammarMetadataContractFrame(record = null, options = {}, ownerCapability = null) {
      const node = record && typeof record === "object" ? record : {};
      ...
      const inboundGrammarFrame = getGrammarMetadataContractFrame(node, options);
      if (!issuedGrammarFrameOwnerCapabilities.has(ownerCapability)) {
        return inboundGrammarFrame;
      }
    ...
    function attachGrammarMetadataContract(record = null, options = {}, ownerCapability = null) {
      const node = record && typeof record === "object" ? record : {};
      const grammarFrame = buildGrammarMetadataContractFrame(node, options, ownerCapability);
      if (!grammarFrame) {
        return null;
      }
      const resultContract = buildGrammarResultContract({
    ...
    api.isIssuedGrammarFrame = isIssuedGrammarFrame;
    api.getIssuedGrammarFrameOwnerId = getIssuedGrammarFrameOwnerId;
    api.isIssuedCanonicalGrammarResultFrame = isIssuedCanonicalGrammarResultFrame;
    api.getIssuedGrammarFrameCanonicalSurfaceForms = getIssuedGrammarFrameCanonicalSurfaceForms;
    api.getIssuedGrammarFrameCanonicalFormulaSurfacePairs = getIssuedGrammarFrameCanonicalFormulaSurfacePairs;
    ...
    [
      ...
      "getAndrewsCnvTenseLogicGenerationGateValue",
      "isAndrewsCnvTenseLogicGenerationAllowed",
      "cloneAndrewsFormulaSlotDefinition",
      "cloneAndrewsFormulaSlotSchema",
      "getAndrewsFormulaSlotSchema",
      "getAndrewsFormulaSlotSchemas",
      "resolveAndrewsFormulaSlotSchema",
      "getAndrewsFormulaSlotDefinition",
      "getAndrewsFormulaGenerationContract",
      "getAndrewsFormulaSourceRequirements",
      "renderAndrewsFormulaSlotTemplate",
      "isAndrewsFormulaSlotOmitted",
      "renderAndrewsFormulaTemplate",
      "getAndrewsFormulaEchoSlotValue",
      "renderAndrewsFormulaEchoFromSchema",
      "normalizeAndrewsFormulaRequirementValue",
      "getAndrewsFormulaSourceRequirementValue",
...
export function installGrammarFrameGlobals(targetObject = globalThis, installationContext = null) {
    const api = createGrammarFrameModule(targetObject, installationContext);
