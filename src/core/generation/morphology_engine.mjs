...
export function createMorphologyEngineApi(targetObject = globalThis, installationContext = null) {
    ...
    function getMorphologyApplicationSourceSurfaceForms(result = null) {
      ...
      return grammarFrame ? frameFirstForms : [];
    }
    function getMorphologyApplicationSoundSpellingFrames(result = null) {
      const output = result && typeof result === "object" ? result : {};
      const grammarFrame = getMorphologyApplicationResultFrame(output);
      return [...(Array.isArray(grammarFrame?.orthographyFrame?.soundSpellingFrames) ? grammarFrame.orthographyFrame.soundSpellingFrames : [])].map(frame => ({
        ...frame
      }));
    }
    function buildMorphologySoundSpellingFrame(frameInput = {}, beforeValue = "", afterValue = "", role = "") {
    ...
    function pushMorphologyLesson2SoundSpellingFrame(frames = [], frameInput = {}, beforeValue = "", afterValue = "", role = "") {
      ...
      const frame = buildMorphologySoundSpellingFrame(frameInput, beforeValue, afterValue, role);
      if (!frame) {
        return;
      }
    ...
    function buildPotencialActiveForms({
      ...
      if (isTroncoPatientivoCoreProfile) {
        const patientivoTroncoForms = buildTroncoActivePatientivoCoreForms({
          ...
          isTransitive: (isTransitive || forceTransitiveBase) && !hasImpersonalTlaPrefix,
          directionalPrefix,
          boundPrefix,
          boundPrefixes,
          boundExplicitFlags,
    ...
    function applyMorphologyRules({
      ...
      const isIntransitiveVerb = objectPrefix === "" && !isTlaFusion && !indirectObjectMarker && !thirdObjectMarker && !isUnderlyingTransitive;
      const forceTransitiveBase = isTlaFusion || isUnderlyingTransitive;
      ...
      if (isPatientivoAdjectiveProfile) {
        ...
        const isTransitive = !isIntransitiveVerb && !hasImpersonalTlaPrefix;
      ...
      const pretSurfaceRuleMeta = null;
      ...
      if (tense === "activo-tronco-preterito" || tense === "activo-tronco-perfecto") {
        ...
        if (isTroncoActiveWrapper && baseStemCandidates.length) {
          if (activeWrapperSourceTense === "perfecto" && patientivoSource === "perfectivo") {
            ...
            const troncoIsTransitive = forceTransitiveBase && !hasImpersonalTlaPrefix;
            baseStemCandidates.forEach(candidate => {
              const sourceVerb = String(candidate?.verb || "").trim();
              if (!sourceVerb) {
                return;
      ...
      if (isSustantivoVerbalLikeTense) {
        ...
        const getSustantivoVerbalSuffixVariants = (stemValue = "", {
          ...
          const longNominalizer = nominalizerContract?.surfaceSuffixes?.long || "liz";
          const shortNominalizer = nominalizerContract?.surfaceSuffixes?.short || "z";
    ...
    api.buildMorphologySoundSpellingFrame = buildMorphologySoundSpellingFrame;
...
export function installMorphologyEngineGlobals(targetObject = globalThis, installationContext = null) {
    const api = createMorphologyEngineApi(targetObject, installationContext);
