...
export function createAgreementDisplayApi(targetObject = globalThis) {
    ...
    function getDefaultOutputToggleSelection({
      ...
      if (context === "verb-extra-object") {
        return resolveDefaultToggleId(values, preferredId ?? "qui", ["qui", targetObject.OBJECT_TOGGLE_ALL, "", ...fallbackIds]);
      }
      if (context === "noun-primary-object") {
        return resolveDefaultToggleId(values, preferredId ?? "tla", ["tla", "qui", "", ...fallbackIds]);
      }
      if (context === "noun-extra-object") {
        ...
        return resolveDefaultToggleId(normalizedValues, preferredId ?? "tla", ["tla", "", ...fallbackIds]);
      }
      if (context === "noun-possessor") {
        return resolveDefaultToggleId(values, preferredId ?? "ī", ["ī", ...fallbackIds]);
      }
      if (context === "noun-nonactive-primary") {
        return resolveDefaultToggleId(values, preferredId ?? "obj:tla", ["obj:tla", ...fallbackIds]);
      }
    ...
    function getCurrentObjectPrefix() {
      ...
      const directGroup = groups.find(group => group.prefixes.includes("qui")) || groups[0];
      if (!directGroup) {
        return "";
      }
    ...
    function getObjectLabel(prefix) {
      if (!prefix) {
        return targetObject.getLocalizedLabel(targetObject.OBJECT_LABELS.intransitive, false, "intransitivo");
      }
    ...
    function getObjectRoleLabel(role) {
      if (!role) {
        return "";
      }
    ...
    function getValence3PlusSlotRoleLabel(slotId) {
      ...
      if (entry && typeof entry === "object") {
        return entry.labelEs || VALENCE3_PLUS_SLOT_ROLE_FALLBACKS[slotId];
      }
      return VALENCE3_PLUS_SLOT_ROLE_FALLBACKS[slotId] || "";
    }
    function getObjectComboLabel(prefix) {
      return getObjectLabelShort(prefix);
    }
    function getNounObjectLabel(prefix) {
      if (!prefix) {
        return targetObject.getLocalizedLabel(targetObject.NOUN_OBJECT_LABELS.intransitive, false, "a él/ella/eso");
      }
      return getObjectLabel(prefix);
    }
    function getNounObjectLabelShort(prefix) {
      return getNounObjectLabel(prefix).replace(/\s*\([^)]*\)/g, "").trim();
    }
    function getNounObjectComboLabel(prefix) {
      if (!prefix) {
        return getNounObjectLabelShort(prefix);
      }
      return getObjectComboLabel(prefix);
    }
    function getNounZeroObjectComboLabel(options = {}) {
      if (options.isImpersonalDummy) {
        return targetObject.getVerbBlockLabel("impersonal", false, "impersonal");
      }
      return getNounObjectComboLabel("");
    }
    function getPossessorLabel(prefix) {
    ...
    function getPossessorPersonLabel(prefix) {
      ...
      return targetObject.getLocalizedLabel(entry, false, "");
    }
    ...
    function getDummySubjectSubLabel() {
      return "ello";
    }
    function formatActiveValence3PlusPersonSub(basePersonSub = "", objectParts = []) {
    ...
    function getSubjectSubLabel(selection, options = {}) {
      ...
      if (isNonanimateContext && targetObject.isNonanimatePers1Pers2(selection.subjectPrefix, selection.subjectSuffix)) {
        return getDummySubjectSubLabel();
      }
      if (selection.subjectPrefix === "" && ["h", "eh", "ān", "in"].includes(selection.subjectSuffix)) {
        return "ellos/ellas/esos";
      }
      return targetObject.getPersonSubLabel(selection);
    }
