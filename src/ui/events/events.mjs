...
export function createUiEventsApi(targetObject = globalThis) {
    ...
    function applyVerbEntryInputNow() {
      ...
      if (typeof targetObject.commitClassicalSourcePartsEvaluation === "function") {
        targetObject.commitClassicalSourcePartsEvaluation({
          force: true,
          source: "manual-entry"
        });
        const sourceInput = targetObject.document.querySelector(".classical-source-parts__input:not(:disabled)");
        if (sourceInput && typeof sourceInput.focus === "function") {
          sourceInput.focus();
        }
        return;
      }
    ...
    async function loadStaticBootstrapData() {
      const runtimeConfig = typeof globalThis !== "undefined" && globalThis.__CLASSICAL_RUNTIME_CONFIG__ && typeof globalThis.__CLASSICAL_RUNTIME_CONFIG__ === "object" ? globalThis.__CLASSICAL_RUNTIME_CONFIG__ : null;
      const loaderGroups = runtimeConfig?.bootstrapLoaderGroups || {
        ...
        deferred: ["loadStaticOptions", "loadStaticGroups", "loadStaticOrders", "loadStaticRules", "loadStaticDerivationalRules", "loadStaticValenceNeutral", "loadStaticPhonology", "loadStaticAllomorphyRules", "loadStaticModes", "loadStaticMisc", "loadStaticRedup"]
      };
      ...
      uiRuntimeInitializationPromise = (async () => {
        ...
        if (verbEl) {
          ...
          verbEl.addEventListener("input", () => {
            ...
            targetObject.rememberNonSearchValue(searchParts);
            // Typing only updates the editable source. The expensive output pass
            // is committed explicitly with Enter/Return or #verb-entry-apply.
            targetObject.cancelScheduledVerbInputRefresh();
          });
    ...
    function wrapInlineComposerStateProjection() {
      if (typeof targetObject.window !== "undefined" && targetObject.window.__CLASSICAL_INLINE_FORMULA_RENDER_WRAP__) {
        return;
      }
      if (typeof targetObject.window !== "undefined") {
        targetObject.window.__CLASSICAL_INLINE_FORMULA_RENDER_WRAP__ = true;
      }
    ...
    if (!(typeof targetObject.window !== "undefined" && targetObject.window.__CLASSICAL_BOOTSTRAP_MANAGED__)) {
